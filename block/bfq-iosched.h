 ...
 * 공유하도록 보장한다. NVMe SSD 관점에서 보면, 이 파일의 bfq_queue/bfq_entity가
 * 최종적으로 NVMe Submission Queue(SQ, 커맨드 제출 큐)에 어떤 request를 언제
 * 밀어 넣을지를 결정하는 상위 정책의 데이터 모델이다.
 *
 * === 전체 아키텍처에서의 위치 ===
 ...
 * bfq_data(디바이스 전역 상태이자 spinlock의 소유자)이며, NVMe 관점에서는
 * bfq_data->rq_in_driver[actuator]가 각 actuator(Independent Access Range)별
 * NVMe SQ in-flight 개수를 추적해 SQ 슬롯 고갈을 막는 역할을 한다.
 *
 * === 주요 함수/구조체 요약 ===
 ...
 */
#define BFQ_MIN_WEIGHT			1 // 최소 weight; NVMe SQ 자원 분배 비율 하한 (이보다 작으면 굶주림 방지 로직이 무의미해짐)
#define BFQ_MAX_WEIGHT			1000 // 최대 weight; NVMe SQ 자원 분배 비율 상한 (cgroup2 io.weight의 [1,10000]과 별개로 BFQ 내부 계산 정밀도 한계)
#define BFQ_WEIGHT_CONVERSION_COEFF	10 // weight 변환 계수; ioprio(0~7) -> BFQ weight 매핑 시 곱해지는 배율 (ioprio -> NVMe SQ 우선순위 매핑에 사용)
 ...
 */
#define BFQ_DEFAULT_QUEUE_IOPRIO	4 // 기본 queue ioprio; IOPRIO_CLASS_BE 내에서 중간값 (NVMe SQ 내 상대 우선순위 결정)

#define BFQ_DEFAULT_GRP_IOPRIO	0 // 기본 그룹 ioprio; ioprio_class가 IDLE이 아닌 한 사실상 사용되지 않는 placeholder (NVMe SQ cgroup 기본 우선순위)
#define BFQ_DEFAULT_GRP_CLASS	IOPRIO_CLASS_BE // 기본 그룹 클래스; cgroup을 명시적으로 설정하지 않은 경우 Best-Effort로 취급 (NVMe SQ BE 클래스 기본값)
 ...
 * 길이로 정해졌다.
 */
#define MAX_BFQQ_NAME_LENGTH 16 // bfq_queue 이름 최대 길이; NVMe trace/로그 식별자 버퍼 크기 (bfq_log_bfqq 매크로의 pid_str 배열 크기와 동일)
...
struct bfq_entity {
	 ...
	 * entity가 NVMe SQ로 디스패치될 상대적 순서를 뜻한다.
	 ...
	 * 서비스되는 것이 곧 NVMe SQ로 먼저 나갈 우선순위를 뜻한다.
	 ...
	 * 허용된 예산(섹터 수). NVMe doorbell 빈도, 즉 한 번에 얼마나 많은
	 * request를 몰아서 내보낼지를 결정하는 핵심 값이다.
	 ...
	 * request의 총 개수. cgroup 단위로 NVMe SQ depth 사용량을 집계하는 데 쓰인다.
	 ...
	 * entity라면 상위 bfq_group의 entity를 가리킨다. NVMe SQ 자원이 cgroup
	 ...
	 * 가능성을 판단하는 stable-merge 휴리스틱의 입력으로 쓰여, NVMe SQ 경로를
	 * 하나로 합칠지 예측하는 데 사용된다.
...
struct bfq_ttime {
	 ...
	 * 프로세스가 NVMe CQ 완료 인터럽트 직후 거의 바로 다음 요청(SQ
	 * doorbell)을 낸다는 뜻이므로, BFQQF_has_short_ttime 판정을 통해 다른
	 * 큐의 request injection을 억제하고 이 큐의 sequential locality를
	 * 지켜준다.
...
struct bfq_queue {
	 ...
	 * 들어갈지를 결정하며, NVMe SQ로 나갈 우선순위 그룹을 좌우한다.
	 ...
	 * 연결하는 노드. 인접한 LBA를 다루는 큐(잠재적 cooperator)를 찾는 데
	 * 쓰여 NVMe PRP/SGL이 순차적으로 준비될 후보를 빠르게 탐색하게 한다.
	 ...
	 * 정렬해 담는 rb-tree. 순차 접근 패턴을 유지해 디스크 헤드 이동(HDD) 또는
	 * NVMe PRP/SGL 구성 시 sequential locality를 극대화한다.
	 ...
	 * blk_mq_dispatch_rq_list() -> nvme_queue_rq() 경로로 전달될 다음 후보다.
	 ...
	 * bfq_io_cq.bfqq 행렬의 첫 인덱스와 동일한 관례). NVMe SQ backlog 크기를
	 ...
	 * 허용되는 최대 예산. 한 큐가 NVMe SQ를 독점하지 못하도록 하는 상한이며,
	...
	int dispatched;
	/* [한국어] 이미 스케줄러 밖(dispatch 리스트 또는 드라이버/NVMe SQ)으로
	 ...
	 * NVMe SQ/CQ 흐름 제어에 직접 영향을 주는 상태 비트들의 모음이다.
	 ...
	 * 누적 시간(ns). NVMe SQ 관점에서 이 큐가 실제로 SQ를 사용한 비율을
	 * 역산하는 데 쓰인다.
	 ...
	 * 거리 차로 순차성(sequentiality)을 판단해 seek_history 갱신 및 NVMe
	 * PRP/SGL이 연속된 세그먼트로 이어질지 예측하는 데 쓰인다.
	 ...
	 * 있으며, NVMe CQ 완료 직후 다음 doorbell이 올 확률(예측 신뢰도)을 높이는
	 * 근거가 되어 결국 IO_bound로 승격시킬지 판단한다.
	 ...
	 * 된다. NVMe SQ 경로가 태스크별로 분리되어 있는지(bic != NULL) 아니면
	 * 공유되는지를 구분하는 값이다.
	 ...
	 * 일시적으로 높은 weight를 주는 기간의 상한으로, NVMe SQ 상에서
	 ...
	 * 있지 않다는 뜻이고, bfqd->bfq_wr_coeff까지 커질 수 있어 이 큐가 NVMe SQ
	 ...
	 * 서비스량. WR 기간 동안 실제로 얼마나 많은 NVMe SQ 서비스를 받았는지
...
struct bfq_iocq_bfqq_data {
	 ...
	 */
	bool saved_IO_bound;
	/* [한국어] merge 직전 BFQQF_IO_bound 플래그의 스냅샷. NVMe SQ
	 ...
	 * NVMe SQ 자원 분배 비율이 원래 큐와 달라지므로 정확한 복원이 중요하다.
	 * 설정자/읽는 자: save/resume 경로.
	...
	/* [한국어] merge 직전 ttime(think time 통계) 전체 스냅샷. split 후 NVMe
	 * doorbell 도착 간격 예측을 처음부터 다시 학습하지 않도록 보존한다.
	 * 설정자/읽는 자: save/resume 경로.
	 * 값 범위: struct bfq_ttime 참고.
	 * 동기화: bfqd->lock. */
	...
	/* [한국어] merge 직전 inject_limit 스냅샷. NVMe SQ injection 한계를
	 * split 후에도 이어가기 위함.
	 * 설정자/읽는 자: save/resume 경로.
	 * 값 범위: 0 이상.
	 * 동기화: bfqd->lock. */
...
struct bfq_io_cq {
	 ...
	 * 프로세스라도 actuator별로 별도의 NVMe SQ 경로를 갖게 한다.
	 ...
	 * 계산 시 이 태스크가 이미 얼마나 NVMe SQ 슬롯을 쓰고 있는지 참조된다.
...
struct bfq_data {
	 ...
	 * 실제 드라이버에 넘어가지 않은 request들의 대기열. NVMe SQ로 전달되기
	 * 직전 마지막 홀딩 지점이다.
	 ...
	 * idling 중이라도 서비스 중인 큐 포함) bfq_queue 개수. NVMe SQ에 후보로
	 ...
	 * NVMe SQ에서 latency-sensitive(대화형/soft-RT)로 취급되어 우선 처리
	 ...
	 * 총 개수. 모든 bfq_queue.queued[]의 합에 해당하며 NVMe SQ에 아직
	 ...
	 * 최댓값. 드라이버가 실제로 여러 태그를 동시에 처리하는지(NVMe SQ의 실질
	 ...
	 * 기대하며 device idling을 수행할 때 쓰는 hrtimer. NVMe에서는 이 타이머가
	 * 만료되기 전 request가 도착하면 doorbell을 다시 눌러 batching 효과를
	 * 보고, 만료되면 다른 큐로 서비스를 전환한다.
	...
	/* [한국어] 현재 실제로 디스패치가 진행 중인 최상위 bfq_queue. NVMe SQ로
	 ...
	 * request와의 거리로 전역적인 순차성/탐색 비용을 추정해 NVMe PRP/SGL
	 * 연속성 판단이나 peak_rate 계산에 참고된다.
	 ...
	 * 소비 속도를 측정해 NVMe SQ 서비스 슬라이스 길이를 검증하는 데 쓰인다.
	...
	/* [한국어] 마지막으로 device idling을 시작한 시각(ktime). idling
	 * 지속시간을 측정해 doorbell batching 대비 latency 손실을 평가하는 데
	 * 쓰인다.
	 ...
	 * 샘플 수. 전체 샘플 대비 이 값의 비율로 NVMe PRP/SGL이 순차 최적화를
	 * 활용할 수 있는 워크로드인지 판단한다.
	 ...
	 * NVMe PRP/SGL이 한 커맨드에 담을 수 있는 세그먼트 한계를 추정하거나
	 * peak_rate 계산의 보정치로 쓰인다.
	 ...
	 * 큐의 max_budget 상한이 되어 NVMe SQ 독점을 막는다.
	 ...
	 * 무장(arm) 길이로 쓰이며, NVMe doorbell batching 이득과 latency 손실
	 * 사이의 트레이드오프를 조절하는 핵심 튜너블이다.
	 ...
	 * 계산의 상한으로 강제된다(NVMe SQ depth의 수동 상한 설정에 해당).
	 ...
	 * 시간(jiffies). seeky 큐가 예산을 오래 붙들어 순차적인 큐의 NVMe SQ
	 ...
	 * 버스트로 간주해 NVMe SQ activation 폭주를 감지한다.
	 ...
	 * bfq_large_burst_thresh를 넘으면 large_burst로 승격되어 NVMe SQ
	 ...
	 * 판단해 NVMe SQ cgroup 자원 공유 정책에 반영한다.
	 ...
	 * 새로 생성되는 큐들에 대해 WR을 억제하는 등, NVMe SQ 스케줄러가
	 ...
	 * (각 큐의 burst_list_node로 연결). NVMe SQ queue lifetime과 폭주 관리를
	 ...
	 * 이 값까지 오를 수 있으며, NVMe SQ에서 latency-sensitive 큐가 받을 수
	 * 있는 최대 우선도를 제한한다.
	 ...
	 * 시간(jiffies). NVMe SQ 상에서 soft-RT QoS를 보장하는 시간 창의
	 * 상한이다.
	 ...
	 * 도착 간격이 이만큼은 벌어져 있어야 한다는 최소 시간(jiffies). NVMe SQ
	 ...
	 * 보장되지 않지만 최소한 NVMe SQ로 request가 나갈 경로 자체는
	 * 유지된다(가용성 우선).
	 ...
	 * 이루어지며, NVMe completion(인터럽트/소프트IRQ 컨텍스트)과 submit(프로세스
	 * 컨텍스트) 경로가 이 락으로 상호 배제되어 doorbell 관련 자료구조의
	 * 가시성을 보장한다.
	 ...
	 * bfq_queue 캐시. NVMe SQ에 들어갈 request가 기존 큐로 병합될 수
	 ...
	 * 계열에 돌려준다. NVMe SQ가 넘치거나 특정 종류의 request가 SQ를
	 * 독점하는 것을 막는 1차 방어선이다.
	 ...
	 * request를 injection해 NVMe SQ 슬롯을 최대한 채우되, in-service
	 * 큐로부터 훔치는 슬롯 수는 최소화한다(원본 주석 참고, NCQ 32슬롯 가정).
	 * 설정자: bfq_init_queue()가 경험적 상수로 초기화.
	 * 읽는 자: bfq_select_queue()의 injection 대상 판단 로직.
	 * 값 범위: 0 초과.
	 * 동기화: 초기화 후 불변(추정). */
};
 ...
 * 직접 영향을 준다. 아래 각 값은 bfq_queue.flags의 비트 인덱스이며,
...
enum bfqq_state_flags {
	 ...
	 * 카운트에 반영되는 기준 플래그다. (NVMe SQ 활성 후보) */
	BFQQF_wait_request,	/* waiting for a request */
	/* [한국어] idling 중이며 다음 request 도착을 기다리고 있음을 표시.
	 * idle_slice_timer가 무장된 상태와 대응된다. (NVMe SQ doorbell 지연 상태) */
	BFQQF_non_blocking_wait_rq, /*
				     * waiting for a request
				     * without idling the device
				     */
	/* [한국어] request를 기다리되 device idling(디스크를 놀리는 대기)은
	 * 하지 않는 특수 대기 모드. NVMe에서 busy-polling과 유사하게, SQ를
	 * 비우지 않고도 다음 request를 예약하는 경로에 쓰인다. */
	BFQQF_fifo_expire,	/* FIFO checked in this slice */
	/* [한국어] 현재 서비스 슬라이스 동안 이미 FIFO 만료 검사를 한 번
	 * 수행했음을 표시(중복 검사 방지). (NVMe timeout/abort 준비) */
	BFQQF_has_short_ttime,	/* queue has a short think time */
	/* [한국어] bfq_ttime.ttime_mean이 짧다고 판정된 상태. NVMe CQ 완료
	 * 직후 곧바로 다음 doorbell이 올 것으로 예상되어 injection을 억제한다. */
	BFQQF_sync,		/* synchronous queue */
	/* [한국어] O_DIRECT/동기 쓰기 등 동기(sync) I/O를 내는 큐임을 표시.
	 * bfq_io_cq.bfqq[1][*] 행에 대응하며, async 큐와 다른 idling/우선순위
	 * 정책이 적용된다. (NVMe SQ sync I/O 경로) */
	BFQQF_IO_bound,		/*
	 ...
	 * I/O-bound(지속적으로 I/O를 내는)" 큐로 분류됨을 표시. (NVMe SQ
	 * timeout/abort 패턴) */
...
void bfq_clear_bfqq_##name(struct bfq_queue *bfqq);			\
int bfq_bfqq_##name(const struct bfq_queue *bfqq);
BFQ_BFQQ_FNS(just_created); // BFQQF_just_created의 mark/clear/query 3함수 선언; NVMe SQ 초기화 상태 전환
BFQ_BFQQ_FNS(busy); // BFQQF_busy의 mark/clear/query 3함수 선언; NVMe SQ 활성/비활성화
BFQ_BFQQ_FNS(wait_request); // BFQQF_wait_request의 mark/clear/query 3함수 선언; NVMe SQ doorbell 지연 상태
BFQ_BFQQ_FNS(non_blocking_wait_rq); // BFQQF_non_blocking_wait_rq의 mark/clear/query 3함수 선언; NVMe SQ busy-polling 유사 경로
BFQ_BFQQ_FNS(fifo_expire); // BFQQF_fifo_expire의 mark/clear/query 3함수 선언; NVMe timeout/abort 준비 플래그
BFQ_BFQQ_FNS(has_short_ttime); // BFQQF_has_short_ttime의 mark/clear/query 3함수 선언; NVMe CQ 후속 doorbell 밀접성
BFQ_BFQQ_FNS(sync); // BFQQF_sync의 mark/clear/query 3함수 선언; NVMe SQ sync/async 경로 구분
BFQ_BFQQ_FNS(IO_bound); // BFQQF_IO_bound의 mark/clear/query 3함수 선언; NVMe SQ timeout/abort 이력 플래그
BFQ_BFQQ_FNS(in_large_burst); // BFQQF_in_large_burst의 mark/clear/query 3함수 선언; NVMe SQ activation 폭주 플래그
BFQ_BFQQ_FNS(coop); // BFQQF_coop의 mark/clear/query 3함수 선언; NVMe SQ 경로 병합 상태
BFQ_BFQQ_FNS(split_coop); // BFQQF_split_coop의 mark/clear/query 3함수 선언; NVMe SQ 경로 분리 예고
BFQ_BFQQ_FNS(softrt_update); // BFQQF_softrt_update의 mark/clear/query 3함수 선언; NVMe latency 정책 갱신 필요
#undef BFQ_BFQQ_FNS
...
enum bfqq_expiration {
	 ...
	 * request가 오지 않아 만료된 경우. NVMe SQ doorbell이 예상보다 늦게
	 * 트리거되어 idling 이득보다 손실이 커졌다고 판단된 상황이다. */
	BFQQE_BUDGET_TIMEOUT,	/* budget took too long to be used */
	/* [한국어] budget_timeout(jiffies)이 지나도록 예산을 다 쓰지 못해
	 * 강제로 만료된 경우. NVMe SQ latency 보장을 위해 seeky 큐를
	 * 강제 교체하는 시나리오다. */
	BFQQE_BUDGET_EXHAUSTED,	/* budget consumed */
	/* [한국어] 할당된 예산을 정상적으로 다 소비해 만료된 경우. NVMe SQ
	 * fairness 규칙에 따른 정상적인 큐 교체다. */
	BFQQE_NO_MORE_REQUESTS,	/* the queue has no more requests */
	/* [한국어] 큐에 더 이상 대기 중인 request가 없어(자연스럽게 비어)
	 * 만료된 경우. NVMe SQ 스케줄러가 해당 큐를 후보에서 제거하는 가장
	 * 흔한 경로다. */
	BFQQE_PREEMPTED		/* preemption in progress */
	/* [한국어] 더 높은 우선순위(RT 등)의 큐가 활성화되어 현재 서비스 중인
	 * 큐가 선점(preemption)당한 경우. NVMe SQ에서 latency-critical
	 * 트래픽이 끼어들 때 발생한다. */
};
struct bfq_stat {
	 ...
	 * blkio 통계(NVMe SQ throughput 측정 등)처럼 정확도보다 갱신 성능이
	 * 중요한 카운터에 쓰인다.
...
struct bfqg_stats {
	 ...
	 * 통계). merge가 많을수록 실제 NVMe SQ에 나가는 커맨드 수가 줄어드는
	 * 효과를 보여준다.
	...
	/* [한국어] BFQ 스케줄러 큐 안에서 대기한 총 시간(ns, NVMe SQ에
	...
	/* [한국어] 이 그룹에 큐잉되어 있던 IO 개수의 누적/스냅샷 통계(NVMe SQ
	 ...
	 * (bfq_stat = percpu_counter + aux_cnt 조합). NVMe SQ 서비스 시간의
	 * cgroup별 누적치다.
	 ...
	 * avg_queue_size_samples로 나누면 평균 큐 크기(NVMe SQ 평균 깊이)가
	 * 된다.
	 ...
	 * 총 시간(NVMe SQ cgroup 레벨 latency).
	...
	/* [한국어] 이 blkcg_gq(그룹)를 위해 device idling한 누적 시간(NVMe
	 * doorbell batching으로 인한 지연 시간의 cgroup별 분해).
...
struct bfq_group_data {
	 ...
	 * 되어, NVMe SQ 자원 분배 비율을 cgroup 단위로 결정한다.
...
struct bfq_group {
	 ...
	 * bfq_group)들을 스케줄링하는 자신만의 B-WF2Q+ 스케줄러. NVMe SQ
	 ...
	 * 두지 않고 그룹 단위로 공유해 NVMe SQ에 나가는 커맨드 수를 줄인다.
	 ...
	 * bfq_find_close_cooperator()의 탐색 대상이며, NVMe SQ에서 여러 작은
	 ...
	 * cgroupfs를 통해 사용자에게 노출되는 NVMe SQ cgroup 모니터링 데이터의
	 * 원천이다.
 ...
 * NVMe SQ 후보 트리가 아무 큐도 갖지 않은 최초 상태를 표현한다.
 */
 ...
 * 기본값을 참조할 수 있게 한다. NVMe SQ latency 보장의 기본 시간
 * 파라미터다. */
 ...
 * 찾아 NVMe SQ merge 후보를 판단하는 데 사용된다. bfqd->lock 하에서 호출.
 ...
 * @return: 없음(void).
 *
 * merge 해제/큐 소멸 시 협력 체인 전체의 참조를 정리해 NVMe SQ 병합
 ...
 * @return: 없음(void).
 *
 * 그룹 단위로 async I/O의 WR을 일괄 종료해 NVMe SQ async 경로의
 ...
 * 호출되어 NVMe SQ 경로에서 태스크를 분리(detach)한다. bfqd->lock 하에서 호출.
 ...
 * 다시 시도해야 할 때, blk_mq_run_hw_queue() 재실행을 예약해 NVMe SQ로
 ...
 * 공유 async 큐들의 NVMe SQ 자원을 회수한다. bfqd->lock 하에서 호출.
 ...
 * @return: 없음(void).
 *
 * merge가 많을수록 실제 NVMe SQ에 나가는 커맨드 수가 줄어드는 효과를
 ...
 * @return: 없음(void).
 *
 * NVMe SQ 스케줄링 이벤트(그룹이 트리에서 빠지는 빈도)를 통계로
 ...
 * B-WF2Q+ 트리로 옮겨 NVMe SQ 자원 분배를 새 cgroup 기준으로
 ...
 * @return: 없음(void).
 *
 * NVMe SQ backlog 크기를 cgroup 단위로 기록하는 디버그 전용 경로.
 * bfqd->lock 하에서 호출.
 ...
 * @return: 없음(void).
 *
 * NVMe SQ 활용도 저하 구간을 통계로 남기기 위한 시작 마커 기록.
 * bfqd->lock 하에서 호출.
 ...
 * @return: 없음(void).
 *
 * NVMe SQ 평균 깊이 통계(avg_queue_size_sum/samples)를 갱신한다.
 * bfqd->lock 하에서 호출.
 ...
 * @return: 없음(void).
 *
 * 새 bfq_queue/bfq_group이 생성될 때 호출되어, 이 entity를 NVMe SQ
 ...
 * 호출, NVMe SQ cgroup 분류를 최신 상태로 유지한다. bfqd->lock 하에서 호출.
 ...
 * @return: 없음(void).
 *
 * low_latency가 꺼지거나 전역 정책이 바뀔 때, NVMe SQ async 경로의
 ...
 * 처음 I/O를 낼 때 그 cgroup에 대응하는 bfq_group 트리를 구축해 NVMe SQ
 ...
 * @return: 없음(void).
 *
 * bfq_group.ref와 대응 blkg의 참조를 짝 지어 해제해 NVMe SQ cgroup
 ...
 * (blkio.bfq.weight 등). blkcg_policy_bfq 등록 시 함께 등록되어 NVMe SQ
 ...
 * 통해 bfq_group_data/bfq_group 생성이 blkcg 코어와 연결된다(NVMe SQ
 ...
 * 적용"해야 하는 코드에서 쓰인다. NVMe SQ 자원이 cgroup 계층을 따라
 ...
 * bfq_queue로 "내려가야" NVMe SQ dispatch(next_rq 접근 등)를 할 수 있다.
 * bfqd->lock 하에서 호출.
 ...
 *
 * NVMe SQ 병렬도(동시에 후보가 될 수 있는 큐 수)를 나타내며, 0이면
 ...
 * 데 쓰인다(RT/BE/IDLE 중 하나, NVMe SQ 클래스 선택).
 ...
 * 지정했을 때, 이를 B-WF2Q+가 이해하는 weight 수치로 바꿔 NVMe SQ
 * 우선순위 계산에 반영한다.
 ...
 * NVMe SQ 후보에서 최종 제외되는 처리다. bfqd->lock 하에서 호출.
 ...
 * 상태로 전이할 때만 호출되어야 트리 정합성이 깨지지 않는다. NVMe SQ
 ...
 * (누가 얼마나 NVMe SQ를 썼는지)을 갱신하고 vtime을 전진시킨다.
 * bfqd->lock 하에서 호출.
 ...
 * 시간만큼 fairness 계산에 반영해야 NVMe SQ latency 보장이 무너지지
 ...
 *
 * NVMe SQ 후보에서 이 entity(및 그 하위 bfq_queue/bfq_group)를 제외하는
 ...
 *
 * 더 높은 우선순위 큐가 활성화되었을 때 NVMe SQ 우선순위 전환(선점)을
 ...
 * NVMe SQ로 전달되는, BFQ 스케줄링의 핵심 진입점이다. bfqd->lock 하에서 호출.
 ...
 * 되돌리는 NVMe SQ 완료 후 정리 단계다. bfqd->lock 하에서 호출.
 ...
 * @return: 없음(void).
 *
 * NVMe SQ 후보에서 큐를 빼는 상위 레벨 API(entity 레벨의
 ...
 * @return: 없음(void).
 *
 * request가 새로 도착해 큐가 backlogged로 전이할 때 호출되어, NVMe SQ
 ...
 * NVMe SQ 후보로 유지한다. bfqd->lock 하에서 호출.
 ...
 * NVMe SQ 병렬도 카운트를 낮춘다. bfqd->lock 하에서 호출.
 ...
 * @return: 없음(void).
 *
 * bfq_del_bfqq_busy()의 역연산으로, 큐가 backlogged로 전이할 때 NVMe SQ
 ...
 * 큐를 가리키도록 NVMe SQ merge 예측 상태를 복원한다. bfqd->lock 하에서 호출.
...
{
	char type = bfq_bfqq_sync(bfqq) ? 'S' : 'A'; // bfq_bfqq_sync()로 BFQQF_sync 비트를 읽어 sync면 'S', async면 'A' 문자를 결정 - 이후 이름 문자열의 접미사로 쓰여 NVMe SQ sync/async 경로를 로그에서 구분
	if (bfqq->pid != -1) // pid가 -1이 아니면 이 큐는 특정 프로세스 전용(공유되지 않음) - merge되어 소유자가 사라진 공유 큐와 구분하는 조건
		snprintf(str, len, "bfq%d%c", bfqq->pid, type); // "bfq<pid><S|A>" 형태로 포맷 - PID 기반 이름이라 NVMe blktrace에서 어느 프로세스의 큐인지 바로 식별 가능
	else
		snprintf(str, len, "bfqSHARED-%c", type); // pid가 -1이면(merge로 공유 상태) "bfqSHARED-<S|A>"로 포맷 - 특정 프로세스에 귀속되지 않는 공유 NVMe SQ 경로임을 명시
}
 ...
 * ->bfqg_to_blkg()->blkcg->css)까지 태깅해 메시지를 남긴다. NVMe SQ/CQ
 ...
 * 큐 단위가 아닌 전역 NVMe SQ 이벤트를 기록할 때 쓰인다.
