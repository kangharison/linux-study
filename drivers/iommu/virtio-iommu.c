...
struct viommu_event {
	union {
	/* [한국어] 이벤트를 헤더로 볼지 폴트로 볼지를 같은 메모리에 겹친 창.
	 * 왜 필요한가: 이벤트 큐로 어떤 종류가 올지 미리 알 수 없다. 아직 모르는
	 *   종류가 와도 첫 워드(헤더)만 보고 걸러 내야 하므로, 종류를 확정하기 전에
	 *   안전하게 읽을 수 있는 관점이 하나 있어야 한다.
	 * 설정자: 장치(가상 IOMMU)가 큐에 이벤트를 쓴다.
	 * 읽는 자: viommu_event_handler() 가 먼저 head 로 예약 비트를 검사해 알 수
	 *   없는 이벤트를 버리고, 통과한 것만 fault 로 해석한다.
	 * 이 구조가 앞으로 정의될 이벤트 종류에 대한 전방 호환을 준다 — 모르는
	 *   것을 조용히 무시하는 것이 규격이 요구하는 동작이다. */
		u32			head;
