...
struct sysmmu_v1_fault_info {
	...
	unsigned int type;	/* fault type for report_iommu_fault */
	/* [한국어] (원 주석: fault type for report_iommu_fault) 이 폴트를 어느 접근 방향으로 보고할지.
	 * 설정자: sysmmu_v1_faults 표의 정적 초기화.
	 * 읽는 자: 폴트를 iommu 코어에 보고할 때 report_iommu_fault() 에 그대로 넘긴다.
	 * 값 범위: IOMMU_FAULT_READ 또는 IOMMU_FAULT_WRITE.
	 * 왜 표에 미리 적어 두는가: v1~v3 세대는 폴트 종류와 접근 방향이 일대일로
	 *   묶여 있다 — 인터럽트 비트 자체가 "읽기 페이지 폴트" 처럼 방향을 함의한다.
	 *   그래서 별도의 레지스터를 읽지 않고 표에서 꺼내면 된다.
	 * v7 세대는 이 관계가 끊겨 fault_info 레지스터를 따로 읽어야 하며, 그 차이가
	 *   variant 의 get_fault_info 콜백으로 흡수된다. */
};
...
struct sysmmu_variant {
	...
	/* [한국어] (원 주석 참고) 1단계 페이지 테이블의 물리 주소를 쓰는 레지스터의 오프셋.
	 * 설정자: 세대별 sysmmu_variant 표의 정적 초기화.
	 * 읽는 자: SYSMMU_REG 매크로. 그 매크로가 data 의 variant 를 따라가 이 값을
	 *   더해 실제 주소를 만든다.
	 * 이 레지스터에 쓰는 순간부터 하드웨어가 그 테이블로 변환을 시작한다.
	 *   그래서 쓰기 전에 테이블이 완성되어 있어야 하고, 쓴 뒤에는 TLB 를 비워야
	 *   옛 테이블의 항목이 남지 않는다.
	 * 값이 0 이면 그 세대에 이 레지스터가 없다는 뜻이며, 쓰는 쪽이 미리 확인한다.
	 * 왜 표로 모으는가: 세대마다 오프셋이 다른데 하는 일은 같다. 표로 모아 두면
	 *   나머지 코드가 세대를 의식하지 않고 이름만으로 레지스터에 닿는다. */

	u32 flush_all;		/* invalidate all TLB entries */
	/* [한국어] (원 주석 참고) TLB 전체 무효화를 명령하는 레지스터의 오프셋.
	 * 설정자: 세대별 sysmmu_variant 표의 정적 초기화.
	 * 읽는 자: SYSMMU_REG 매크로. 그 매크로가 data 의 variant 를 따라가 이 값을
	 *   더해 실제 주소를 만든다.
	 * 가장 무딘 무효화다. 도메인 하나의 매핑을 바꿔도 그 컨트롤러의 모든 항목이
	 *   사라져 이후 접근이 모두 TLB 미스가 된다. 그래도 범위 무효화가 없는
	 *   세대에서는 항목을 하나씩 지우는 것보다 이쪽이 빠른 경우가 있다.
	 * 값이 0 이면 그 세대에 이 레지스터가 없다는 뜻이며, 쓰는 쪽이 미리 확인한다.
	 * 왜 표로 모으는가: 세대마다 오프셋이 다른데 하는 일은 같다. 표로 모아 두면
	 *   나머지 코드가 세대를 의식하지 않고 이름만으로 레지스터에 닿는다. */

	u32 flush_entry;	/* invalidate specific TLB entry */
	/* [한국어] (원 주석 참고) IOVA 하나에 해당하는 TLB 항목만 무효화하는 레지스터의 오프셋.
	 * 설정자: 세대별 sysmmu_variant 표의 정적 초기화.
	 * 읽는 자: SYSMMU_REG 매크로. 그 매크로가 data 의 variant 를 따라가 이 값을
	 *   더해 실제 주소를 만든다.
	 * 범위 무효화가 없는 세대(v1~v3)에서 여러 페이지를 풀 때는 이 레지스터에
	 *   주소를 하나씩 써 넣으며 반복한다. 구간이 넓으면 그 반복이 비싸져,
	 *   일정 크기를 넘으면 위 flush_all 로 바꾸는 판단이 코드에 들어 있다.
	 * 값이 0 이면 그 세대에 이 레지스터가 없다는 뜻이며, 쓰는 쪽이 미리 확인한다.
	 * 왜 표로 모으는가: 세대마다 오프셋이 다른데 하는 일은 같다. 표로 모아 두면
	 *   나머지 코드가 세대를 의식하지 않고 이름만으로 레지스터에 닿는다. */
	...
	/* [한국어] (원 주석 참고) 범위 무효화의 끝 주소를 쓰는 레지스터의 오프셋.
	 * 설정자: 세대별 sysmmu_variant 표의 정적 초기화.
	 * 읽는 자: SYSMMU_REG 매크로. 그 매크로가 data 의 variant 를 따라가 이 값을
	 *   더해 실제 주소를 만든다.
	 * 위 flush_start 와 짝이며, 둘을 모두 쓴 뒤 flush_range 에 명령을 써야
	 *   실제로 무효화가 일어난다. 세 레지스터를 쓰는 사이에 다른 CPU 가 끼어들면
	 *   범위가 뒤섞이므로, 그 구간 전체가 락으로 보호된다.
	 * 값이 0 이면 그 세대에 이 레지스터가 없다는 뜻이며, 쓰는 쪽이 미리 확인한다.
	 * 왜 표로 모으는가: 세대마다 오프셋이 다른데 하는 일은 같다. 표로 모아 두면
	 *   나머지 코드가 세대를 의식하지 않고 이름만으로 레지스터에 닿는다. */

	u32 int_status;		/* interrupt status information */
	/* [한국어] (원 주석 참고) 어떤 폴트가 났는지 비트로 알려 주는 레지스터의 오프셋.
	 * 설정자: 세대별 sysmmu_variant 표의 정적 초기화.
	 * 읽는 자: SYSMMU_REG 매크로. 그 매크로가 data 의 variant 를 따라가 이 값을
	 *   더해 실제 주소를 만든다.
	 * 인터럽트 처리기가 가장 먼저 읽는 레지스터다. 선 비트의 번호가 곧
	 *   sysmmu_v1_faults 배열의 첨자가 되어, 폴트 이름과 주소 레지스터와 접근
	 *   방향을 한꺼번에 얻는다 — 그 대응이 이 드라이버의 폴트 처리 골격이다.
	 * 값이 0 이면 그 세대에 이 레지스터가 없다는 뜻이며, 쓰는 쪽이 미리 확인한다.
	 * 왜 표로 모으는가: 세대마다 오프셋이 다른데 하는 일은 같다. 표로 모아 두면
	 *   나머지 코드가 세대를 의식하지 않고 이름만으로 레지스터에 닿는다. */

	u32 int_clear;		/* clear the interrupt */
	/* [한국어] (원 주석 참고) 처리한 폴트 비트를 지우는 레지스터의 오프셋.
	 * 설정자: 세대별 sysmmu_variant 표의 정적 초기화.
	 * 읽는 자: SYSMMU_REG 매크로. 그 매크로가 data 의 variant 를 따라가 이 값을
	 *   더해 실제 주소를 만든다.
	 * 이 레지스터에 되쓰지 않으면 인터럽트가 계속 걸려 시스템이 멈춘다.
	 *   폴트 정보를 다 읽은 뒤에 지워야 하며, 순서를 바꾸면 지운 직후 새 폴트가
	 *   나서 읽은 정보와 어긋날 수 있다.
	 * 값이 0 이면 그 세대에 이 레지스터가 없다는 뜻이며, 쓰는 쪽이 미리 확인한다.
	 * 왜 표로 모으는가: 세대마다 오프셋이 다른데 하는 일은 같다. 표로 모아 두면
	 *   나머지 코드가 세대를 의식하지 않고 이름만으로 레지스터에 닿는다. */
	...
	/* [한국어] (원 주석 참고) 폴트가 난 IOVA 를 담고 있는 레지스터의 오프셋.
	 * 설정자: 세대별 sysmmu_variant 표의 정적 초기화.
	 * 읽는 자: SYSMMU_REG 매크로. 그 매크로가 data 의 variant 를 따라가 이 값을
	 *   더해 실제 주소를 만든다.
	 * v7 세대에서만 이 자리가 통일되어 있다. v1~v3 는 폴트 종류마다 주소
	 *   레지스터가 달라, 그 오프셋을 sysmmu_v1_fault_info 표에 종류별로 적어 둔다.
	 *   그 차이가 아래 get_fault_info 콜백이 필요한 이유의 절반이다.
	 * 값이 0 이면 그 세대에 이 레지스터가 없다는 뜻이며, 쓰는 쪽이 미리 확인한다.
	 * 왜 표로 모으는가: 세대마다 오프셋이 다른데 하는 일은 같다. 표로 모아 두면
	 *   나머지 코드가 세대를 의식하지 않고 이름만으로 레지스터에 닿는다. */

	u32 fault_info;		/* fault transaction info */
	/* [한국어] (원 주석 참고) 폴트 트랜잭션의 부가 정보(접근 방향 등)를 담은 레지스터의 오프셋.
	 * 설정자: 세대별 sysmmu_variant 표의 정적 초기화.
	 * 읽는 자: SYSMMU_REG 매크로. 그 매크로가 data 의 variant 를 따라가 이 값을
	 *   더해 실제 주소를 만든다.
	 * v7 전용이다. 이 세대는 폴트 종류와 접근 방향이 분리되어, 방향을 알려면
	 *   이 레지스터를 따로 읽어야 한다. v1~v3 가 종류만으로 방향을 알 수 있었던
	 *   것과 대비되며, 그것이 get_fault_info 콜백이 필요한 이유의 나머지 절반이다.
	 * 값이 0 이면 그 세대에 이 레지스터가 없다는 뜻이며, 쓰는 쪽이 미리 확인한다.
	 * 왜 표로 모으는가: 세대마다 오프셋이 다른데 하는 일은 같다. 표로 모아 두면
	 *   나머지 코드가 세대를 의식하지 않고 이름만으로 레지스터에 닿는다. */

	int (*get_fault_info)(struct sysmmu_drvdata *data, unsigned int itype,
	/* [한국어] 인터럽트 비트 번호를 세대 무관한 폴트 정보로 옮기는 콜백.
	 * 설정자: 세대별 variant 표.
	 * 읽는 자: exynos_sysmmu_irq() 가 int_status 를 읽은 뒤 이 콜백을 부른다.
	 * @data:  이 컨트롤러의 상태 — 레지스터를 읽을 기준 주소를 얻는다.
	 * @itype: int_status 에서 선 비트의 번호.
	 * @fault: 채워 넣을 결과 — 주소, 접근 방향, 폴트 이름이 담긴다.
	 * @return: 0 이면 성공, 음수면 알 수 없는 폴트다.
	 * 왜 표만으로는 안 되는가: 위 fault_va 와 fault_info 설명대로, 세대마다
	 *   비트 배치도 주소를 얻는 방법도 다르다. v1~v3 는 종류별 표를 뒤져야 하고
	 *   v7 은 고정된 레지스터 둘을 읽는다. 오프셋의 차이가 아니라 절차의 차이라
	 *   값의 표로는 표현할 수 없다. */
...
struct sysmmu_drvdata {
	...
	/* [한국어] (원 주석: SYSMMU's pclk clock) 주변장치 클럭 — aclk 와 짝을 이룬다.
	 * 설정자: 프로브가 장치 트리에서 선택적으로 얻는다.
	 * 읽는 자: 컨트롤러를 켜고 끌 때 clk_enable/disable 하는 자리.
	 * 왜 클럭이 여럿인가: SoC 마다 SYSMMU 에 클럭을 대는 방식이 다르다. 하나로
	 *   묶어 주는 SoC 도 있고(clk), 버스 클럭과 주변장치 클럭을 따로 주는 SoC 도
	 *   있다(aclk + pclk). 드라이버는 있는 것만 켠다.
	 * 값 범위: NULL 일 수 있다. 다만 clk 이 없으면 aclk 와 pclk 가 모두 있어야
	 *   하며, 그 검사를 프로브가 한다. */
	...
	/* [한국어] (원 주석: node for owner controllers list) 마스터의 controllers 목록에 매다는 고리.
	 * 설정자/읽는 자: 마스터(실제 DMA 를 하는 장치)를 프로브하고 해제할 때.
	 * 왜 이 목록이 필요한가: 마스터 하나가 여러 SYSMMU 컨트롤러 뒤에 있을 수
	 *   있다. 예컨대 멀티미디어 IP 는 읽기 포트와 쓰기 포트가 각각 다른 SYSMMU 에
	 *   매인다. 그 장치의 도메인을 바꾸면 관련된 컨트롤러 전부를 함께 설정해야
	 *   하므로, 마스터 쪽에서 컨트롤러들을 훑을 수 있어야 한다.
	 * 위 domain_node 와 방향이 다르다 — 그쪽은 도메인이 컨트롤러들을 훑는 목록이고,
	 *   이쪽은 마스터가 자기 컨트롤러들을 훑는 목록이다. 같은 컨트롤러가 두 목록에
	 *   동시에 매달린다. */
