...
enum dart_type {
	DART_T8020,
	/* [한국어] M1 이전 세대(T8103 등)의 DART.
	 * 설정자: of_device_id 표가 고른 apple_dart_hw 안에 이 값이 들어 있다.
	 * 읽는 자: 세대마다 절차가 다른 소수의 자리 — 레지스터 오프셋 차이는 hw 표가
	 *   흡수하지만, TLB 무효화 명령의 형태처럼 절차 자체가 다른 곳은 이 값으로 가른다.
	 * 특징: 36비트 물리 주소, APPLE_DART 페이지 테이블 포맷.
	 * 아래 DART_T6000 과 레지스터 배치가 같고 주소 폭과 포맷만 다르다. 반면
	 *   DART_T8110 은 배치 자체가 달라, 이 enum 이 사실상 "배치가 다른 두 부류 +
	 *   같은 부류 안의 변종" 을 표현한다. */
...
struct apple_dart_hw {
	...
	u32 tcr_enabled;
	/* [한국어] "변환을 수행하라"에 해당하는 TCR 레지스터 값.
	 * 설정자: 세대별 apple_dart_hw 표의 정적 초기화.
	 * 읽는 자: apple_dart_hw_enable_translation() 이 이 값에서 시작해 필요하면
	 *   tcr_4level 비트를 얹은 뒤 스트림의 TCR 에 쓴다.
	 * 왜 값이 세대마다 다른가: 같은 뜻의 설정이라도 비트 배치가 세대마다 달라,
	 *   "변환 켜기"를 나타내는 워드가 통째로 다르다. 비트 위치를 각각 정의하는
	 *   대신 완성된 값을 표에 담아 두는 편이 짧다.
	 * 이 드라이버의 격리가 이 레지스터 하나에 달려 있다 — 스트림별 TCR 이 곧
	 *   그 스트림의 변환 여부이자 권한이다. */
	...
	u32 ttbr_valid;
	/* [한국어] TTBR 레지스터에서 "이 항목이 유효하다"를 뜻하는 비트.
	 * 설정자: 세대별 hw 표.
	 * 읽는 자: TTBR 을 쓸 때 주소와 함께 OR 하고, 지울 때는 이 비트를 내린다.
	 * 왜 세대마다 위치가 다른가: T8020/T6000 계열과 T8110 은 레지스터 배치가
	 *   달라 유효 비트의 자리도 다르다. 상수로 박아 두면 한 세대에서만 동작한다.
	 * 값이 0 이 아님이 보장된다 — 유효 비트가 없는 세대는 없다.
	 * 아래 ttbr_addr_field_shift 와 짝을 이룬다: 주소를 그만큼 밀어 넣고 이
	 *   비트를 세우는 것이 TTBR 을 쓰는 절차의 전부다. */
...
struct apple_dart_atomic_stream_map {
	struct apple_dart *dart;
	/* [한국어] 이 원자적 스트림 맵이 가리키는 DART.
	 * 설정자: 도메인을 완성할 때(init_lock 아래에서) 한 번만 설정된다.
	 * 읽는 자: 이 맵의 비트에 해당하는 스트림을 실제로 프로그램할 때.
	 * 값 범위: NULL 이면 이 슬롯이 아직 쓰이지 않는다는 뜻이다.
	 * 동기화가 이 구조체의 요점이다: 아래 sidmap 은 atomic_long_t 배열이라
	 *   attach/detach 가 락 없이 병렬로 비트를 세우고 지울 수 있다. 그런데 이
	 *   dart 포인터 자체는 보호되지 않는다 — 도메인이 완성될 때 한 번 정해지고
	 *   다시 바뀌지 않으므로, 원자적으로 다룰 필요가 없기 때문이다.
	 * 원본 영어 주석이 밝히는 것도 바로 이 비대칭이다. */
