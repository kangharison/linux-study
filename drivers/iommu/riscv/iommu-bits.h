...
enum riscv_iommu_ddtp_modes {
	...
	/* [한국어] 2단계 DDT — 중간 테이블 하나를 거친다. */

	RISCV_IOMMU_DDTP_IOMMU_MODE_3LVL = 4,
	/* [한국어] 3단계 DDT — 24비트 device_id 전체를 덮는 최대 구성이다. */
...
enum riscv_iommu_dc_iohgatp_modes {
	...
	/* [한국어] 48비트 게스트 물리 주소 공간(4단계 테이블). */

	RISCV_IOMMU_DC_IOHGATP_MODE_SV57X4 = 10
	/* [한국어] 57비트 게스트 물리 주소 공간(5단계 테이블). */
};
...
enum riscv_iommu_dc_fsc_atp_modes {
	...
	/* [한국어] 48비트 가상 주소(4단계 테이블). */

	RISCV_IOMMU_DC_FSC_IOSATP_MODE_SV57 = 10,
	/* [한국어] 57비트 가상 주소(5단계 테이블). */
...
enum riscv_iommu_fq_causes {
	...
	/* [한국어] 명령어 인출 접근 폴트 — 물리 메모리에 접근할 수 없었다. */
	...
	/* [한국어] 쓰기/원자적 연산의 주소 정렬 오류. */

	RISCV_IOMMU_FQ_CAUSE_WR_FAULT = 7,
	/* [한국어] 쓰기/원자적 연산의 접근 폴트. */
	...
	/* [한국어] DDTP 모드가 OFF라 모든 인바운드 트랜잭션이 차단됐다. */
	...
	/* [한국어] MSI 페이지 테이블 엔트리를 읽는 데 실패했다. */

	RISCV_IOMMU_FQ_CAUSE_MSI_INVALID = 262,
	/* [한국어] MSI 페이지 테이블 엔트리의 V 비트가 0이다. */
