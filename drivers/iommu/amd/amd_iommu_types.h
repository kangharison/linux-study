...
struct amd_iommu {
	struct list_head list;
	/* [한국어] 전역 amd_iommu_list 에 이 유닛을 매다는 고리.
	 * 설정자: 부팅 중 IVRS 표를 파싱하며 유닛을 하나씩 발견할 때마다 매단다.
	 * 읽는 자: for_each_iommu 매크로. 모든 유닛에 같은 일을 해야 하는 경로 —
	 *   플러시 전파, 서스펜드/레주메, 초기화 단계 진행 — 가 이 목록을 훑는다.
	 * 한 시스템에 IOMMU 유닛이 여럿 있을 수 있고(소켓마다, 또는 IO 허브마다),
	 *   각각이 자기 장치 표와 명령 버퍼를 따로 갖는다. 그래서 "모든 유닛에" 라는
	 *   동작이 자주 필요하고, 그것을 위한 목록이다.
	 * 동기화: 목록 자체는 부팅 중에만 바뀌므로 이후에는 락 없이 훑는다. */
	...
	u64 mmio_phys;
	/* [한국어] (원 주석: physical address of MMIO space) 이 유닛의 레지스터 창 물리 시작 주소.
	 * 설정자: IVRS 표의 IVHD 항목이 알려 준 값을 부팅 중에 저장한다.
	 * 읽는 자: iommu_map_mmio_space() 가 ioremap 할 때, 그리고 debugfs 가 이 창의
	 *   위치를 보여 줄 때.
	 * 왜 물리 주소를 계속 들고 있는가: 매핑한 뒤에는 아래 mmio_base 만 있으면
	 *   되지만, 레주메 경로에서 다시 매핑해야 하고 진단에도 쓰이기 때문이다.
	 * 값 범위: 펌웨어가 정한 값. 4KB 정렬되어 있다. */
	/* [한국어] MMIO 영역의 물리 시작 주소 (원 주석). */

	/* physical end address of MMIO space */
	u64 mmio_phys_end;
	/* [한국어] (원 주석: physical end address of MMIO space) 그 창의 끝 주소.
	 * 설정자: 위 mmio_phys 와 함께 IVRS 파싱에서.
	 * 읽는 자: 매핑할 크기를 정할 때 (end - phys).
	 * 왜 크기가 아니라 끝 주소인가: IVRS 표가 그렇게 알려 주기 때문이다. 표의
	 *   형식을 그대로 담아 두고 필요할 때 크기를 계산한다.
	 * 값 범위: mmio_phys 보다 크다. 하드웨어 세대에 따라 창 크기가 달라
	 *   (성능 카운터 유무 등) 이 값도 달라진다. */
	...
	/* Extended features 2 */
	u64 features2;
	/* [한국어] (원 주석: Extended features 2) 두 번째 확장 기능 비트맵.
	 * 설정자: 초기화 중 EXT_FEATURES2 레지스터를 읽어 채운다.
	 * 읽는 자: FEATURE_* 상수로 이 유닛의 능력을 검사하는 모든 자리.
	 * 왜 두 번째 워드가 있는가: 위 features 의 64비트를 다 써 버려 늘린 것이다.
	 *   하드웨어가 세대를 거듭하며 기능이 늘어난 흔적이라, 새 기능일수록 이쪽에 있다.
	 * 값 범위: 하드웨어가 알린 값. 이 레지스터 자체를 지원하지 않는 구형 유닛에서는 0 이다.
	 * 동기화: 초기화 뒤 바뀌지 않는다. */
	...
	/* [한국어] (원 주석: length of exclusion range of that IOMMU) 제외 범위의 길이.
	 * 설정자: 위 exclusion_start 와 함께 IVRS 파싱에서.
	 * 읽는 자: 제외 범위 레지스터를 프로그램할 때 끝 주소를 계산한다.
	 * 값 범위: 0 이면 제외 범위가 없다는 뜻이라, 그 레지스터를 아예 켜지 않는다.
	 * 제외 범위가 위험한 이유: 그 구간의 DMA 는 변환을 전혀 거치지 않는다. 즉
	 *   IOMMU 의 보호가 그 구간에는 미치지 않는다. 그래서 펌웨어가 꼭 필요하다고
	 *   알린 경우에만 쓰고, 요즘은 대신 항등 매핑(unity map)을 선호한다. */
	/* [한국어] 그 범위의 길이 (원 주석). */
	...
	/* [한국어] (원 주석: Name for PPR log interrupt) PPR 로그 인터럽트에 붙일 이름.
	 * 설정자: 인터럽트를 등록하기 직전에 유닛 번호를 넣어 만든다.
	 * 읽는 자: request_threaded_irq() 가 그대로 가져가고, 사용자에게는 /proc/interrupts
	 *   에 이 이름으로 보인다.
	 * 왜 구조체 안에 배열로 두는가: request_irq 는 이름 문자열을 복사하지 않고
	 *   포인터만 들고 있는다. 스택이나 임시 버퍼에 만들면 등록 뒤 사라져 버려,
	 *   /proc/interrupts 가 쓰레기를 읽는다. 유닛과 수명이 같은 이 자리에 둬야 안전하다.
	 * 값 범위: 16바이트에 들어가는 문자열. 널 종료를 포함한다. */
	...
	/* [한국어] (원 주석: Name for GA log interrupt) GA 로그 인터럽트에 붙일 이름.
	 * 설정자/읽는 자: 위 ppr_irq_name 과 같다.
	 * 구조체 안에 두어야 하는 이유도 같다 — request_irq 가 문자열을 복사하지 않는다.
	 * GA 로그가 무엇인가: 게스트에게 직접 전달하지 못한 인터럽트가 여기 쌓이고,
	 *   KVM 이 그것을 보고 대신 주입한다. 그 처리기의 인터럽트가 이 이름으로 보인다.
	 * 값 범위: 16바이트 문자열. */
	...
	u32 stored_l2[0x83];
	/* [한국어] (원 주석: The l2 indirect registers) L2 간접 레지스터들의 사본.
	 * 설정자: 서스펜드 직전 iommu_apply_resume_quirks() 계열이 전부 읽어 담는다.
	 * 읽는 자: 레주메 때 같은 순서로 되쓴다.
	 * 왜 사본이 필요한가: 이 레지스터들은 MMIO 창에 직접 보이지 않고, 주소
	 *   레지스터에 오프셋을 쓴 뒤 데이터 레지스터를 읽는 간접 방식으로만 접근한다.
	 *   전원이 끊기면 내용이 사라지는데 펌웨어가 되살려 주지 않는 기종이 있어,
	 *   드라이버가 직접 보관했다가 복원한다.
	 * 값 범위: 0x83 개. 위 stored_l1 은 여섯 벌인데 이쪽은 한 벌인 것이
	 *   하드웨어의 구성 차이다. */
	/* [한국어] L2 간접 레지스터의 사본 (원 주석). */

	/* The maximum PC banks and counters/bank (PCSup=1) */
	u8 max_banks;
	/* [한국어] (원 주석: PCSup=1 일 때) 이 유닛이 가진 성능 카운터 뱅크의 수.
	 * 설정자: 초기화 중 능력 레지스터를 읽어 정한다. PCSup 기능이 없으면 0 이다.
	 * 읽는 자: perf 이벤트를 카운터에 배정할 때, 그리고 뱅크마다의 레지스터
	 *   주소를 계산할 때.
	 * 값 범위: 0 이면 이 유닛에 성능 카운터가 없어 PMU 를 등록하지 않는다.
	 * 뱅크와 카운터로 나뉘어 있는 이유: 하드웨어가 카운터를 뱅크 단위로 묶어
	 *   두고, 뱅크마다 레지스터 블록이 따로 있다. 그래서 카운터 하나를 지목하려면
	 *   뱅크 번호와 뱅크 안 번호가 둘 다 필요하다. */
	u8 max_counters;
	/* [한국어] 뱅크 하나에 든 성능 카운터의 수.
	 * 설정자: 위 max_banks 와 함께 능력 레지스터에서.
	 * 읽는 자: 전체 카운터 수(max_banks * max_counters)를 계산할 때, 그리고 카운터
	 *   번호를 뱅크 번호와 뱅크 안 번호로 나눌 때.
	 * 값 범위: 하드웨어가 알린 값. max_banks 가 0 이면 의미가 없다.
	 * 왜 곱해 두지 않는가: 카운터 하나를 프로그램하려면 뱅크 번호가 따로 필요하다
	 *   — 레지스터 주소가 뱅크마다 다른 블록에 있기 때문이다. 곱해 두면 그 정보가
	 *   사라져 다시 나눠야 한다. */
	/* [한국어] 뱅크당 카운터 수 (원 주석). */
	...
	int dbg_mmio_offset;
	/* [한국어] debugfs 가 다음에 읽을 MMIO 레지스터의 오프셋.
	 * 설정자: 사용자가 debugfs 의 mmio 노드에 값을 쓰면 그 값이 여기 담긴다.
	 * 읽는 자: 같은 노드를 읽을 때. 그 오프셋의 레지스터 값을 보여 준다.
	 * 왜 상태를 들고 있는가: debugfs 파일 하나로 "어디를 볼지"와 "그 값"을 모두
	 *   다루기 위한 관용구다. 쓰기로 주소를 정하고 읽기로 값을 얻는다.
	 * 값 범위: MMIO 창 안의 오프셋. 범위를 벗어난 값은 읽기 때 거부된다.
	 * 동기화: 진단용이라 락이 없다. 두 사용자가 동시에 쓰면 서로 덮어쓴다. */
	int dbg_cap_offset;
	/* [한국어] debugfs 가 다음에 읽을 PCI 능력 공간의 오프셋.
	 * 설정자/읽는 자: 위 dbg_mmio_offset 과 같은 방식 — 쓰기로 정하고 읽기로 본다.
	 * 왜 MMIO 와 따로 두는가: IOMMU 의 설정은 MMIO 창과 PCI 설정 공간의 IOMMU
	 *   능력 구조체 두 곳에 나뉘어 있다. 진단하려면 양쪽을 다 볼 수 있어야 해서
	 *   각각의 현재 위치를 따로 기억한다.
	 * 값 범위: 그 능력 구조체 안의 오프셋. */
...
struct acpihid_map_entry {
	struct list_head list;
	/* [한국어] 전역 acpihid_map 목록에 이 대응을 매다는 고리.
	 * 설정자: IVRS 표 파싱 또는 커널 명령줄(ivrs_acpihid=) 처리에서.
	 * 읽는 자: ACPI 장치를 IOMMU 에 붙일 때 이 목록을 훑어 HID/UID 가 맞는 항목을 찾는다.
	 * 왜 이 대응이 필요한가: PCI 장치는 BDF 가 곧 요청자 id 라 별도 대응이 필요
	 *   없지만, ACPI 로 기술되는 장치(일부 USB 컨트롤러, 센서 허브 등)는 BDF 가
	 *   없다. 펌웨어가 "이 HID/UID 장치는 IOMMU 에 이 id 로 보인다"고 알려 줘야 한다.
	 * 동기화: 부팅 중에만 바뀐다. */
...
struct devid_map {
	struct list_head list;
	/* [한국어] ioapic_map 또는 hpet_map 목록에 이 대응을 매다는 고리.
	 * 설정자: IVRS 파싱 또는 커널 명령줄(ivrs_ioapic=, ivrs_hpet=) 처리에서.
	 * 읽는 자: IOAPIC 이나 HPET 의 인터럽트를 재매핑할 때, 그 장치의 요청자 id 를
	 *   찾으려 이 목록을 훑는다.
	 * 두 목록을 같은 구조체로 쓰는 이유: IOAPIC 과 HPET 모두 "번호 하나로 지목되는,
	 *   PCI 가 아닌 인터럽트 원천"이라는 점에서 같은 문제를 갖는다. 담을 정보가
	 *   같아 구조체를 나눌 이유가 없다.
	 * 동기화: 부팅 중에만 바뀐다. */
	u8 id;
	/* [한국어] IOAPIC 번호 또는 HPET 블록 번호 — 이 목록을 찾을 때의 키.
	 * 설정자: IVRS 표나 명령줄이 알려 준 값.
	 * 읽는 자: 인터럽트 재매핑 항목을 만들 때, 그 인터럽트가 어느 IOAPIC/HPET 에서
	 *   오는지로 이 값을 대조한다.
	 * 값 범위: ACPI 가 그 IOAPIC 이나 HPET 에 붙인 번호. 시스템 안에서 유일하다.
	 * u8 인 이유: IOAPIC 은 시스템에 많아야 수십 개, HPET 은 몇 개뿐이라 충분하다. */
...
struct iommu_dev_data {
	...
	/* [한국어] (원 주석: PASID state) 이 장치의 PASID 능력을 실제로 켰는가.
	 * 설정자: 장치를 붙일 때 pdev_pasid_enable() 이 성공하면 1 로 둔다.
	 * 읽는 자: 뗄 때 되돌릴지 정할 때, 그리고 PASID 를 쓰는 부착이 가능한지 볼 때.
	 * 값 범위: 0 이면 이 장치는 PASID 없이 RID 하나로만 DMA 한다.
	 * 왜 능력 유무와 따로 기억하는가: 장치가 PASID 를 지원하더라도(위 pasid_supported)
	 *   실제로 켜는 것은 별개다. 켜기가 실패할 수 있고, 켠 것만 정확히 되돌려야
	 *   참조와 하드웨어 상태가 어긋나지 않는다.
	 * 동기화: 그 장치의 부착 경로에서만 바뀌며, iommu 그룹 뮤텍스가 지킨다. */
	u8 pri_tlp      :1;		  /* PASID TLB required for
					     PPR completions */
	/* [한국어] (원 주석: PASID TLB required for PPR completions) PPR 응답에 PASID 를 실어야 하는 장치인가.
	 * 설정자: 장치를 붙일 때 PCI PRI 능력 구조체의 해당 비트를 읽어 담는다.
	 * 읽는 자: 페이지 폴트에 응답하는 명령을 만들 때. 이 값이 1 이면 응답 TLP 에
	 *   PASID 접두를 붙여야 한다.
	 * 왜 장치마다 다른가: PCIe 규격이 두 형식을 모두 허용한다. 장치가 어느 쪽을
	 *   기대하는지는 그 장치의 능력 구조체만 알고 있다.
	 * 틀리면 어떻게 되는가: 장치가 응답을 자기 것으로 알아보지 못해, 폴트를 낸
	 *   요청이 영원히 완료되지 않는다. 조용히 멈추는 종류의 고장이라 진단이 어렵다. */
...
struct dev_table_entry {
	union {
	/* [한국어] 같은 256비트 항목을 두 가지 폭으로 보는 창.
	 * 왜 union 인가: 이 항목은 대부분의 경우 64비트 단위로 필드를 고치면 되지만
	 *   (data[4]), 어떤 갱신은 128비트를 한 번에 원자적으로 바꿔야 한다(data128[2]).
	 *   두 시각이 모두 필요한데 저장 공간은 하나다.
	 * 읽는 자: 필드 마스크(DTE_*)를 쓰는 코드는 data 로 보고, 원자적 갱신 경로는
	 *   data128 로 본다. 하드웨어는 그저 256비트 항목으로 읽는다.
	 * 동기화: 항목이 256비트라 어떤 방법으로도 한 번에 다 쓸 수 없다. 하드웨어가
	 *   중간 상태를 보지 않도록 쓰는 순서를 지켜야 하고, 그 순서를 지키는 동안
	 *   다른 CPU 가 끼어들지 못하게 dte_lock 이 있다. */
		u64 data[4];
...
struct ivhd_dte_flags {
	struct list_head list;
	/* [한국어] 전역 amd_ivhd_dev_flags_list 에 이 지시를 매다는 고리.
	 * 설정자: 부팅 중 IVRS 표의 IVHD 항목을 파싱하며 장치 플래그 지시를 만날 때마다.
	 * 읽는 자: 장치의 DTE 를 처음 만들 때 이 목록을 훑어, 그 장치에 해당하는
	 *   지시가 있으면 그 내용을 DTE 에 미리 반영한다.
	 * 왜 목록으로 모아 두는가: IVRS 표는 부팅 초기에 한 번만 읽을 수 있는데, 장치는
	 *   나중에(핫플러그 포함) 나타난다. 표의 지시를 미리 담아 두었다가 그때 꺼내 쓴다.
	 * 동기화: 부팅 중에만 채워지고, 이후에는 읽기만 한다. */
	u16 segid;
	/* [한국어] 이 지시가 적용되는 PCI 세그먼트 번호.
	 * 설정자: IVRS 파싱에서. 그 IVHD 가 담당하는 세그먼트다.
	 * 읽는 자: 장치를 대조할 때 devid 범위와 함께 확인한다.
	 * 왜 세그먼트까지 봐야 하는가: 장치 id(BDF)는 세그먼트 안에서만 유일하다.
	 *   세그먼트가 여럿인 큰 시스템에서는 같은 BDF 가 여러 개 있을 수 있어,
	 *   세그먼트를 빼면 엉뚱한 장치에 지시가 적용된다.
	 * 값 범위: 0 이 보통이며, 다중 세그먼트 시스템에서만 0 이 아닌 값이 나온다. */
	u16 devid_first;
	/* [한국어] 이 지시가 적용되는 장치 id 범위의 시작(포함).
	 * 설정자: IVRS 파싱에서.
	 * 읽는 자: 장치 대조. devid_first 이상 devid_last 이하이면 이 지시가 적용된다.
	 * 왜 범위인가: IVRS 표가 장치를 하나씩이 아니라 범위로 지시할 수 있다.
	 *   브리지 아래 전체나 특정 버스 전체에 같은 플래그를 걸어야 하는 경우가 흔해,
	 *   펌웨어가 범위로 적어 두면 표가 훨씬 짧아진다.
	 * 값 범위: 16비트 BDF. 단일 장치 지시면 devid_last 와 같은 값이다. */
	u16 devid_last;
...
struct unity_map_entry {
	...
	u16 devid_start;
	/* [한국어] (원 주석: including) 이 항등 매핑이 적용되는 첫 장치 id.
	 * 설정자: IVRS 표의 IVMD 항목 파싱에서.
	 * 읽는 자: 도메인을 만들 때 그 도메인에 속한 장치가 이 범위에 드는지 보고,
	 *   들면 아래 주소 구간의 항등 매핑을 그 도메인에 넣는다.
	 * 왜 항등 매핑이 필요한가: 펌웨어나 다른 장치가 이미 쓰고 있는 메모리 구간이
	 *   있고, 그 구간에 대한 DMA 는 IOVA 와 물리 주소가 같아야 동작한다.
	 *   IOMMU 를 켠 뒤에도 그 관계를 유지하려면 명시적으로 항등으로 매핑해 둔다.
	 * 값 범위: 16비트 BDF. 포함 관계라 이 값 자체도 대상이다. */
	/* end device id this entry is used for (including) */
	u16 devid_end;
	/* [한국어] (원 주석: including) 그 범위의 마지막 장치 id.
	 * 설정자/읽는 자: 위 devid_start 와 같다.
	 * 값 범위: devid_start 이상. 두 값이 같으면 장치 하나만 가리킨다.
	 * 포함(including)이라는 점이 중요하다 — 반열린 구간으로 오해하면 마지막
	 *   장치가 항등 매핑을 받지 못해, 그 장치만 부팅 중에 DMA 오류를 낸다. */
	...
	u64 address_end;
	/* [한국어] (원 주석: including) 항등 매핑할 주소 구간의 끝.
	 * 설정자: IVMD 파싱에서.
	 * 읽는 자: 매핑 크기를 계산할 때. 포함 관계이므로 크기는 end - start + 1 이다.
	 * 왜 크기가 아니라 끝 주소인가: IVRS 표가 그 형식으로 알려 주기 때문이다.
	 *   표의 값을 그대로 담고, 계산은 쓰는 쪽에서 한다.
	 * 값 범위: address_start 이상. 페이지 정렬은 보장되지 않아, 매핑할 때
	 *   페이지 경계로 넓혀야 한다. */
...
union irte {
	...
	struct {
		u32 valid	: 1,
		/* [한국어] 이 재매핑 항목이 유효한가 — 32비트 형식.
		 * 설정자: 인터럽트를 설정할 때 1 로, 해제할 때 0 으로.
		 * 읽는 자: 하드웨어. 0 이면 이 인덱스로 오는 인터럽트 요청을 거부한다.
		 * 값 범위: 0 또는 1. 0 이면 아래 no_fault 가 폴트를 낼지 조용히 버릴지를 가른다.
		 * 왜 첫 비트인가: 하드웨어가 항목을 읽자마자 판정할 수 있게 규격이 그렇게 정했다.
		 * 동기화: 이 형식은 32비트라 val 을 통째로 쓰는 것만으로 원자적 갱신이 된다. */
		    no_fault	: 1,
		    /* [한국어] 유효하지 않은 항목에 인터럽트가 왔을 때 폴트를 낼지, 조용히 버릴지.
		     * 설정자: 항목을 만들 때 정책에 따라.
		     * 읽는 자: 하드웨어.
		     * 값 범위: 0 이면 폴트를 기록해 관리자가 알 수 있게 하고, 1 이면 말없이 버린다.
		     * 왜 버리는 선택지가 있는가: 장치를 떼는 중이거나 재설정하는 사이에 남은
		     *   인터럽트가 도착할 수 있는데, 그것까지 오류로 쌓으면 로그가 넘친다. */
		    int_type	: 3,
		    /* [한국어] 인터럽트 전달 방식 — 고정, 하위 우선순위, SMI, NMI, INIT, ExtINT.
		     * 설정자: 상위 인터럽트 계층이 요청한 전달 방식을 옮겨 담는다.
		     * 읽는 자: 하드웨어가 이 값대로 CPU 에 전달한다.
		     * 값 범위: 3비트라 여덟 가지. APIC 규격의 delivery mode 인코딩을 그대로 쓴다.
		     * 왜 재매핑 항목에 있는가: 재매핑은 목적지만 바꾸는 것이 아니라 인터럽트
		     *   메시지 전체를 다시 쓰는 것이라, 원래 메시지에 있던 정보를 모두 담아야 한다. */
		    rq_eoi	: 1,
		    /* [한국어] 레벨 트리거 인터럽트에서 EOI 를 요구하는지.
		     * 설정자: 인터럽트가 레벨 트리거일 때 1 로 둔다.
		     * 읽는 자: 하드웨어.
		     * 값 범위: 0 이면 에지 트리거로 다룬다.
		     * 왜 필요한가: 레벨 트리거는 원인이 사라질 때까지 계속 어서션 상태라, CPU 가
		     *   처리를 끝냈음을 알려야(EOI) 다음 인터럽트가 성립한다. 재매핑을 거치면
		     *   그 EOI 도 재매핑 하드웨어를 지나므로 항목이 그 사실을 알아야 한다. */
		    dm		: 1,
		    /* [한국어] 목적지 모드 — 논리(logical)인지 물리(physical)인지.
		     * 설정자: 상위 계층이 정한 목적지 표현 방식.
		     * 읽는 자: 하드웨어가 아래 destination 필드를 어떻게 해석할지 정할 때.
		     * 값 범위: 0 이면 물리 모드(destination 이 APIC id 그 자체), 1 이면 논리 모드
		     *   (destination 이 CPU 들의 비트마스크).
		     * 왜 두 모드가 있는가: 물리는 CPU 하나를 정확히 지목하고, 논리는 여러 CPU 중
		     *   하나가 받게 한다. 부하 분산이 필요하면 논리를 쓴다. */
		    rsvd_1	: 1,
		    /* [한국어] 예약 비트 — 0 이어야 한다.
		     * 설정자: 아무도 쓰지 않는다. 구조체를 0 으로 초기화하면 그대로 0 이 된다.
		     * 읽는 자: 하드웨어가 무시한다(현재 세대에서는).
		     * 왜 자리를 잡아 두는가: 이 비트를 빼면 그 뒤의 필드가 한 칸씩 밀려 하드웨어가
		     *   읽는 배치와 어긋난다. 예약 비트는 자리를 지키는 것이 존재 이유다.
		     * 0 을 지켜야 하는 이유: 미래 세대가 이 비트에 의미를 부여할 수 있고, 그때
		     *   1 이 들어 있으면 뜻하지 않은 동작을 한다. */
		    destination	: 8,
		    /* [한국어] 목적지 APIC id. 8비트라 이 형식으로는 CPU 255개를 넘을 수 없다 —
		     *   그것이 128비트 형식이 필요해진 이유 중 하나다. */
		    vector	: 8,
		    rsvd_2	: 8;
		    /* [한국어] 32비트를 채우는 나머지 예약 비트들.
		     * 설정자/읽는 자: 위 rsvd_1 과 같다 — 아무도 쓰지 않고 0 을 지킨다.
		     * 왜 8비트나 남는가: 이 형식은 목적지를 8비트로만 담아(위 destination) CPU
		     *   255개까지밖에 가리키지 못한다. 그 한계 때문에 128비트 형식(irte_ga)이
		     *   생겼고, 이 형식은 남는 자리를 예약으로 둔 채 굳어졌다.
		     * 세미콜론으로 끝나는 것이 이 비트필드 묶음의 마지막임을 뜻한다. */
	} fields;
};
...
union irte_ga_lo {
	 ...
	 * 설정자/읽는 자: 항목을 통째로 읽고 쓰는 코드, 그리고 cmpxchg 계열.
	 * 왜 필요한가: 아래 두 struct 는 같은 64비트를 서로 다르게 해석한 것이다.
	 *   해석을 가리지 않고 값 자체를 옮기거나 비교해야 할 때는 이 창을 쓴다.
	 * 128비트 형식이라 이 워드 하나를 쓰는 것만으로는 원자적 갱신이 되지 않는다 —
	 *   상위 워드와 함께 cmpxchg16b 로 한 번에 바꿔야 한다(struct irte_ga 참고). */

	/* For int remapping */
	struct {
		u64 valid	: 1,
		/* [한국어] (재매핑 해석) 이 항목이 유효한가.
		 * 설정자: 인터럽트를 설정하고 해제할 때.
		 * 읽는 자: 하드웨어.
		 * 값 범위: 0 또는 1. 뜻은 위 32비트 형식의 valid 와 같다.
		 * 이 struct 는 아래 fields_vapic 과 같은 64비트를 다르게 해석한 것이다.
		 *   어느 해석이 유효한지는 guest_mode 비트가 정한다 — 0 이면 이쪽(재매핑),
		 *   1 이면 게스트 vAPIC 해석이다. */
		    no_fault	: 1,
		    /* [한국어] (재매핑 해석) 유효하지 않은 항목에 온 인터럽트를 조용히 버릴지.
		     * 설정자/읽는 자: 위 32비트 형식의 같은 이름 필드와 같다.
		     * 값 범위: 0 이면 폴트를 기록, 1 이면 말없이 버린다.
		     * 128비트 형식에서도 이 비트의 자리와 뜻이 32비트 형식과 같게 유지된다 —
		     *   두 형식을 오가는 코드가 필드 이름만으로 옮겨 쓸 수 있게 하기 위해서다. */
		    /* ------ */
		    int_type	: 3,
		    /* [한국어] (재매핑 해석) 인터럽트 전달 방식.
		     * 설정자/읽는 자: 위 32비트 형식의 같은 이름 필드와 같다.
		     * 값 범위: APIC 규격의 delivery mode 3비트 인코딩.
		     * 게스트 해석(fields_vapic)에서는 이 자리에 rsvd1 이 온다 — 게스트 모드에서는
		     *   하드웨어가 CPU 에 직접 전달하지 않고 vAPIC 페이지에 기록하므로, 전달 방식을
		     *   고를 여지가 없기 때문이다. */
		    rq_eoi	: 1,
		    /* [한국어] (재매핑 해석) 레벨 트리거의 EOI 요구 여부.
		     * 설정자/읽는 자: 위 32비트 형식의 같은 이름 필드와 같다.
		     * 값 범위: 0 이면 에지, 1 이면 레벨.
		     * 게스트 해석에서는 이 자리도 rsvd1 에 흡수된다. */
		    dm		: 1,
		    /* [한국어] (재매핑 해석) 목적지 모드 — 논리인지 물리인지.
		     * 설정자/읽는 자: 위 32비트 형식의 같은 이름 필드와 같다.
		     * 값 범위: 0 이면 물리, 1 이면 논리.
		     * 아래 destination 이 24비트로 넓어진 것이 이 형식의 요점이다. 32비트
		     *   형식의 8비트로는 x2APIC 의 큰 id 를 담을 수 없었다. */
		    /* ------ */
		    guest_mode	: 1,
	...
	struct {
		u64 valid	: 1,
		/* [한국어] (게스트 vAPIC 해석) 항목이 유효한가.
		 * 설정자: KVM 이 게스트 전달을 켤 때.
		 * 읽는 자: 하드웨어.
		 * 값 범위: 0 또는 1. 위 재매핑 해석의 같은 자리와 뜻이 같다 — 두 해석이
		 *   겹쳐 있어도 유효 비트만은 공통이어야, 어느 해석이든 항목을 끌 수 있다.
		 * 각 필드의 자세한 뜻은 이 struct 바로 위의 블록 주석에 정리해 두었다. */
		    no_fault	: 1,
		    /* [한국어] (게스트 vAPIC 해석) 유효하지 않을 때 조용히 버릴지.
		     * 설정자/읽는 자: 위 재매핑 해석의 같은 자리와 같다.
		     * 값 범위: 0 이면 폴트 기록, 1 이면 버림.
		     * 이 비트도 두 해석에서 자리와 뜻이 같게 유지된다. */
		    /* ------ */
		    ga_log_intr	: 1,
		    /* [한국어] (게스트 vAPIC 해석) GA 로그에 항목이 쌓였을 때 인터럽트를 낼지.
		     * 설정자: KVM 이 게스트 전달을 켤 때.
		     * 읽는 자: 하드웨어.
		     * 값 범위: 1 이면 목적지 vCPU 가 실행 중이 아니어서 직접 전달하지 못했을 때
		     *   GA 로그에 적고 호스트에 인터럽트를 낸다. 그 인터럽트를 받은 KVM 이
		     *   vCPU 를 깨워 인터럽트를 주입한다.
		     * 이 비트가 없으면 vCPU 가 잠든 사이 온 인터럽트가 그대로 묻힌다. */
		    rsvd1	: 3,
		    /* [한국어] (게스트 vAPIC 해석) 예약 3비트.
		     * 설정자/읽는 자: 아무도 쓰지 않는다.
		     * 왜 여기가 예약인가: 재매핑 해석에서 이 자리는 int_type(전달 방식)이다.
		     *   게스트 모드에서는 하드웨어가 CPU 에 직접 전달하지 않고 vAPIC 백킹 페이지에
		     *   비트를 세우므로 전달 방식을 고를 필요가 없어, 그 자리가 남는다.
		     * 두 해석이 같은 비트를 다르게 쓰는 대표적인 예다. */
		    is_run	: 1,
		    /* [한국어] (게스트 vAPIC 해석) 목적지 vCPU 가 지금 물리 CPU 위에서 실행 중인가.
		     * 설정자: KVM 이 vCPU 를 스케줄 인/아웃 할 때마다 갱신한다.
		     * 읽는 자: 하드웨어가 인터럽트를 어떻게 처리할지 정할 때.
		     * 값 범위: 1 이면 그 vCPU 가 실행 중이라 하드웨어가 곧바로 그 물리 CPU 에
		     *   전달한다 — 호스트 개입이 전혀 없어 가장 빠른 경로다. 0 이면 전달할 곳이
		     *   없으므로 GA 로그에 적고 위 ga_log_intr 에 따라 호스트를 부른다.
		     * 이 비트가 AMD AVIC 의 핵심이다. 실행 중인 vCPU 에게는 하이퍼바이저를
		     *   거치지 않고 인터럽트가 도달한다.
		     * 동기화: KVM 이 자주 갱신하므로 cmpxchg16b 로 항목 전체를 원자적으로 바꾼다. */
		    /* ------ */
		    guest_mode	: 1,
		    /* [한국어] (게스트 vAPIC 해석) 이 항목을 어느 해석으로 읽을지 정하는 열쇠.
		     * 설정자: KVM 이 게스트 전달을 켤 때 1, 끌 때 0 으로.
		     * 읽는 자: 하드웨어. 그리고 드라이버 코드도 이 값을 보고 두 struct 중
		     *   어느 쪽을 읽을지 정한다.
		     * 값 범위: 1 이면 이 게스트 vAPIC 해석이 유효하고, 0 이면 위 재매핑 해석이 유효하다.
		     * 이 한 비트가 union 의 어느 멤버가 의미 있는지를 혼자 결정한다 — union 에
		     *   태그가 필요할 때 하드웨어가 그 태그를 항목 안에 두는 방식이다. */
		    destination	: 24,
		    /* [한국어] (게스트 vAPIC 해석) 목적지 — 게스트 vAPIC 백킹 페이지를 가리킨다.
		     * 설정자: KVM 이 그 vCPU 의 백킹 페이지 주소로 정한다.
		     * 읽는 자: 하드웨어가 인터럽트 비트를 세울 자리를 찾을 때.
		     * 값 범위: 24비트. 위 hi 워드의 destination 8비트와 합쳐 32비트 APIC id 를
		     *   이룬다 — 두 자리가 이어져 있지 않아 APICID_TO_IRTE_DEST_LO/HI 매크로가
		     *   나누고 합치는 일을 맡는다.
		     * 왜 24비트로 넓혔는가: 32비트 형식의 8비트로는 x2APIC 의 큰 id 를 담을 수 없다. */
		    ga_tag	: 32;
		    /* [한국어] (게스트 vAPIC 해석) KVM 이 이 항목에 붙인 태그.
		     * 설정자: KVM 이 게스트 전달을 켤 때, 그 vCPU 를 식별하는 값으로.
		     * 읽는 자: GA 로그에 쌓인 항목에도 이 태그가 실려 온다. 호스트는 그 태그로
		     *   "어느 게스트의 어느 vCPU 에게 갈 인터럽트였는지"를 되짚는다.
		     * 값 범위: KVM 이 정한 32비트 값. 하드웨어는 뜻을 모르고 그대로 실어 나른다.
		     * 왜 필요한가: GA 로그 항목만 보고는 그것이 어느 vCPU 의 것인지 알 수 없다.
		     *   하드웨어가 되돌려 주는 이 태그가 유일한 단서다. */
...
union irte_ga_hi {
	u64 val;
	/* [한국어] 상위 64비트를 통째로 보는 창.
	 * 설정자/읽는 자: 항목을 통째로 옮기거나 비교하는 코드.
	 * 아래 fields 와 같은 64비트를 다르게 본 것이다. 하위 워드와 달리 이쪽은
	 *   해석이 하나뿐이라 union 의 갈래가 둘밖에 없다 — 게스트 모드에서도 벡터와
	 *   백킹 페이지 주소는 같은 자리에 있다.
	 * 동기화: 하위 워드와 함께 cmpxchg16b 로 한 번에 바꾼다. */
	struct {
	/* [한국어] 상위 64비트를 필드 단위로 보는 창.
	 * 왜 union 인가: 값 전체를 다루는 코드(위 val)와 필드를 고치는 코드가 모두
	 *   필요한데 저장 공간은 하나다.
	 * 이 struct 의 필드 배치는 하드웨어 규격이 정한 것이라 순서를 바꿀 수 없다.
	 *   특히 destination 이 하위 워드와 나뉘어 있는 것이 그렇다. */
		u64 vector	: 8,
		/* [한국어] 게스트 안에서 쓰일 인터럽트 벡터 번호.
		 * 설정자: KVM 이 게스트에게 보이는 벡터를 정해 담는다.
		 * 읽는 자: 하드웨어가 vAPIC 백킹 페이지의 어느 비트를 세울지 정할 때.
		 * 값 범위: 8비트. 게스트의 IDT 항목 번호다 — 호스트의 벡터와는 다른 값이며,
		 *   그 대응을 KVM 이 관리한다.
		 * 왜 상위 워드에 있는가: 하위 64비트는 이미 destination 24비트와 ga_tag 32비트
		 *   로 가득 차 자리가 없었다. */
		    rsvd_1	: 4,
		    /* [한국어] 예약 4비트 — 0 이어야 한다.
		     * 설정자/읽는 자: 아무도 쓰지 않는다.
		     * 왜 여기 있는가: 아래 ga_root_ptr 이 정렬된 자리에서 시작해야 해서, 벡터
		     *   8비트 뒤에 채움이 필요하다. 페이지 정렬 주소를 담는 필드는 하위 12비트를
		     *   버리고 시작하므로 그 경계에 맞춰야 한다. */
		    ga_root_ptr	: 40,
...
struct irte_ga {
	union {
	/* [한국어] 128비트 항목을 두 가지로 보는 창 — 두 워드로 나눠 보거나, 하나로 묶어 보거나.
	 * 왜 필요한가: 필드를 고칠 때는 lo/hi 로 나눠 보는 편이 편하지만, 갱신은
	 *   128비트를 한 번에 원자적으로 해야 한다. 두 시각이 모두 필요하다.
	 * 동기화: 하드웨어와 KVM 이 동시에 이 항목을 본다. 반쯤 갱신된 상태를 하드웨어가
	 *   읽으면 존재하지 않는 목적지로 인터럽트가 가므로, cmpxchg16b 로 128비트를
	 *   한 번에 바꾼다 — 이것이 이 union 이 존재하는 유일한 이유다. */
		struct {
		/* [한국어] 두 워드를 나란히 놓아, 하위와 상위를 각각의 union 으로 다루는 형태.
		 * 읽는 자: 필드를 고치는 거의 모든 코드가 이 시각을 쓴다.
		 * 왜 익명 struct 인가: 이름을 붙이면 접근할 때마다 그 이름을 거쳐야 해서
		 *   코드가 길어진다. 익명이면 바깥에서 그냥 .lo / .hi 로 닿는다.
		 * 배치가 중요하다: lo 가 먼저 오고 hi 가 뒤에 온다. 이 순서가 아래 u128 로
		 *   볼 때의 비트 배치를 결정하므로, 바꾸면 하드웨어가 읽는 값이 달라진다. */
			union irte_ga_lo lo;
			/* [한국어] 항목의 하위 64비트.
			 * 설정자: 인터럽트를 설정할 때, 그리고 KVM 이 게스트 전달을 켜고 끌 때.
			 * 읽는 자: 하드웨어와 드라이버 양쪽.
			 * 값 범위: 재매핑 해석과 게스트 vAPIC 해석이 겹쳐 있고, guest_mode 비트가
			 *   어느 쪽이 유효한지 정한다.
			 * 동기화: 이 워드만 따로 쓰면 안 된다 — hi 와 함께 128비트로 한 번에 바꿔야
			 *   하드웨어가 중간 상태를 보지 않는다. */
			union irte_ga_hi hi;
			/* [한국어] 항목의 상위 64비트.
			 * 설정자/읽는 자: 위 lo 와 같다.
			 * 값 범위: 해석이 하나뿐이다 — 벡터, 게스트 vAPIC 백킹 페이지 주소, 그리고
			 *   목적지 APIC id 의 상위 8비트.
			 * 목적지가 lo 와 hi 로 나뉘어 있다는 점이 이 구조에서 가장 헷갈리는 부분이다.
			 *   APICID_TO_IRTE_DEST_LO/HI 매크로를 반드시 짝으로 써야 한다.
			 * 동기화: lo 와 함께 원자적으로 바꾼다. */
		};
		u128 irte;
...
struct amd_ir_data {
	...
	int ga_vector;
	u64 ga_root_ptr;
	/* [한국어] 게스트 vAPIC 백킹 페이지의 물리 주소를 보관해 둔 사본.
	 * 설정자: 게스트 전달을 켤 때 KVM 이 알려 준 값을 여기 담는다.
	 * 읽는 자: 게스트 전달을 껐다가 다시 켤 때, 또는 인터럽트 친화도가 바뀌어
	 *   항목을 다시 쓸 때. 매번 KVM 에 다시 묻지 않고 이 사본을 쓴다.
	 * 왜 사본이 필요한가: 항목을 다시 쓰는 일은 KVM 이 아니라 인터럽트 계층에서
	 *   시작될 수 있다(친화도 변경 등). 그때 게스트 쪽 정보를 알 방법이 없어,
	 *   켤 때 받은 값을 드라이버가 들고 있어야 한다.
	 * 값 범위: 페이지 정렬된 물리 주소. */
	u32 ga_tag;
	/* [한국어] KVM 이 이 인터럽트에 붙인 태그의 사본.
	 * 설정자: 게스트 전달을 켤 때 KVM 이 알려 준 값.
	 * 읽는 자: 항목을 다시 쓸 때 그대로 싣는다.
	 * 왜 보관하는가: 위 ga_root_ptr 과 같은 이유다. 항목을 다시 쓰는 경로가
	 *   KVM 밖에서 시작될 수 있어, 게스트 쪽 값을 드라이버가 기억해야 한다.
	 * 값 범위: KVM 이 정한 32비트 값. GA 로그 항목에 실려 되돌아와 짝을 찾는 데 쓰인다. */
};
