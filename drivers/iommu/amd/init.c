...
struct ivhd_entry {
	...
	u64 cid;
	/* [한국어] ACPI CID(Compatible ID) 문자열.
	 * 설정자: IVRS 표의 ACPI HID 항목을 파싱할 때 표의 바이트를 그대로 담는다.
	 * 읽는 자: ACPI 장치를 IOMMU 에 대응시킬 때. HID 로 못 찾으면 CID 로도 찾는다.
	 * HID 와 CID 의 차이: HID 는 그 장치의 고유 이름이고, CID 는 "이것과 호환된다"는
	 *   더 일반적인 이름이다. 펌웨어가 새 HID 를 쓰면서 옛 드라이버도 붙게 하려고
	 *   CID 를 함께 적어 두는 일이 흔하다.
	 * 값 범위: 8바이트에 담긴 ASCII 문자열. 널 종료가 아니라 자리를 다 채운다.
	 * u64 로 선언한 이유: 표의 필드 폭이 8바이트라 그대로 옮긴 것이며, 문자열로
	 *   쓸 때는 바이트 배열로 다시 본다. */
	u8 uidf;
...
struct ivmd_header {
	u8 type;
	/* [한국어] 이 IVMD 항목이 적용될 대상 — 모든 장치, 장치 하나, 또는 장치 범위.
	 * 설정자: IVRS 표 파싱이 표의 바이트를 그대로 옮긴다.
	 * 읽는 자: init_unity_map_range() 가 이 값으로 갈래를 나눠, 아래 devid 와 aux 를
	 *   어떻게 해석할지 정한다.
	 * 값 범위: ACPI_IVMD_TYPE_ALL 이면 devid/aux 를 보지 않고 그 세그먼트의 모든
	 *   장치에 적용하고, ACPI_IVMD_TYPE 이면 devid 하나만, ACPI_IVMD_TYPE_RANGE 면
	 *   devid 부터 aux 까지의 범위다.
	 * 이 필드가 나머지 필드의 해석을 혼자 결정하므로, 파싱 순서상 가장 먼저 본다. */
	u8 flags;
	 ...
	 * 설정자: 표 파싱.
	 * 읽는 자: 이 IVMD 가 어느 장치에 적용되는지 판정하는 곳.
	 * 언제 유효한가: 위 type 이 ALL 이면 이 값을 보지 않는다. 그 경우 세그먼트의
	 *   모든 장치가 대상이므로 특정 id 를 지목할 필요가 없다.
	 * 값 범위: 16비트 BDF(버스/장치/기능). 아래 pci_seg 와 함께 봐야 장치가
	 *   유일하게 정해진다. */
	u16 aux;
	/* [한국어] 범위 타입에서 끝 장치의 id.
	 * 설정자: 표 파싱.
	 * 읽는 자: 범위 타입일 때 devid 부터 이 값까지를 순회하는 코드.
	 * 이름이 aux(보조)인 이유: 표의 이 자리가 type 에 따라 다른 뜻으로 쓰인다.
	 *   범위 타입에서는 끝 장치 id 지만, 다른 타입에서는 의미가 없거나 다른 값이
	 *   들어갈 수 있어 중립적인 이름을 붙였다.
	 * 값 범위: 범위 타입이면 devid 이상. 그 밖의 타입에서는 읽지 않는다. */
	u16 pci_seg;
	/* [한국어] 이 IVMD 가 적용되는 PCI 세그먼트 번호.
	 * 설정자: 표 파싱.
	 * 읽는 자: 장치를 대조할 때 devid 와 함께 확인한다.
	 * 왜 세그먼트가 필요한가: BDF 는 세그먼트 안에서만 유일하다. 세그먼트가 여럿인
	 *   큰 서버에서는 같은 BDF 가 여러 개 있어, 세그먼트를 빼면 엉뚱한 장치에
	 *   예약 구간이 적용된다.
	 * 값 범위: 대부분의 시스템에서 0 이다. */
	u8  resv[6];
	/* [한국어] 예약 — 표의 자리를 맞추기 위한 공백이다.
	 * 설정자/읽는 자: 아무도 쓰지 않는다.
	 * 왜 구조체에 넣는가: 이 구조체는 ACPI 표의 바이트 배치를 그대로 본뜬 것이다.
	 *   표에서 이 여섯 바이트가 비어 있으므로, 구조체에도 같은 자리를 남겨 두어야
	 *   뒤따르는 range_start 가 올바른 오프셋에 놓인다.
	 * 이 배열을 빼면 그 뒤의 모든 필드가 6바이트씩 밀려, 파싱이 조용히 쓰레기를 읽는다.
	 * 값 범위: 펌웨어가 0 으로 채워 두지만, 드라이버는 확인하지 않는다. */
	u64 range_start;
...
enum iommu_init_state {
	IOMMU_START_STATE,
	/* [한국어] 아무것도 시작하지 않은 상태 — 이 상태 기계의 출발점이다.
	 * 설정자: 전역 변수의 초기값이 이것이다(0).
	 * 읽는 자: state_next() 가 다음에 무엇을 할지 정할 때.
	 * 왜 상태 기계인가: AMD IOMMU 초기화는 ACPI 파싱, 하드웨어 켜기, PCI 연결,
	 *   인터럽트 등록이 서로 다른 부팅 단계에서 일어난다. 한 함수로 이어 쓸 수 없어,
	 *   각 단계가 끝날 때마다 상태를 한 칸 밀고 다음 단계에서 이어받는다.
	 * 되감기의 어려움: 어느 단계에서 실패했느냐에 따라 되돌려야 할 것이 다르다.
	 *   그래서 실패해도 상태를 남겨 두어, 무엇까지 진행했는지 알 수 있게 한다. */
	IOMMU_IVRS_DETECTED,
	 ...
	 * 설정자: IVRS 표를 찾지 못했을 때.
	 * 읽는 자: 이후의 모든 진입점이 이 상태를 보면 조용히 물러난다.
	 * 왜 오류와 구별하는가: IOMMU 가 없는 기계는 정상적으로 동작해야 한다.
	 *   이것을 오류로 다루면 부팅 로그에 무의미한 경고가 쌓이고, 되감기 코드가
	 *   존재하지 않는 자원을 되돌리려 한다.
	 * 이 값부터 뒤의 셋은 진행 단계가 아니라 종착점이다 — 여기서 상태가 더
	 *   나아가지 않는다. */
	IOMMU_INIT_ERROR,
	/* [한국어] IOMMU 는 있는데 초기화에 실패했다 — 되감기가 필요한 상태다.
	 * 설정자: 어느 단계든 실패하면 이 상태로 옮긴다.
	 * 읽는 자: 이후 진입점들이 이 상태를 보면 물러나고, 되감기 경로가 지금까지
	 *   잡은 자원을 반납한다.
	 * 위 NOT_FOUND 와 나누는 이유가 여기 있다. 없는 것은 되돌릴 것도 없지만,
	 *   실패한 것은 이미 표를 파싱하고 메모리를 잡고 어쩌면 하드웨어를 켰을 수도
	 *   있어 반드시 되감아야 한다.
	 * 아래 CMDLINE_DISABLED 와도 다르다 — 사용자가 끈 경우는 실패가 아니므로
	 *   경고를 내지 않는다. */
...
union intcapxt {
	...
	struct {
		u64	reserved_0		:  2,
		/* [한국어] 예약 비트 두 개 — 0 이어야 한다.
		 * 설정자/읽는 자: 아무도 쓰지 않는다.
		 * 왜 자리를 잡아 두는가: 이 비트필드 묶음은 하드웨어 레지스터의 배치를 그대로
		 *   본뜬 것이다. 예약 자리를 빼면 그 뒤의 모든 필드가 밀려, 조립한 값이
		 *   하드웨어가 기대하는 배치와 어긋난다.
		 * 0 을 지켜야 하는 이유: 미래 세대가 이 비트에 의미를 부여할 수 있고, 그때
		 *   1 이 들어 있으면 뜻하지 않은 동작을 한다. 구조체를 0 으로 초기화하면
		 *   자동으로 지켜진다. */
			dest_mode_logical	:  1,
			 ...
			 * 설정자: 인터럽트를 설정할 때 대상 CPU 의 APIC id 에서 잘라 넣는다.
			 * 읽는 자: 하드웨어가 인터럽트를 보낼 CPU 를 정할 때.
			 * 아래 destid_24_31 과 나뉘어 있는 것이 이 union 의 핵심 문제다: x2APIC 의
			 *   32비트 id 를 담을 연속된 자리가 이 레지스터에 없어 두 조각으로 흩어졌다.
			 *   값을 넣고 꺼낼 때 반드시 두 조각을 함께 다뤄야 한다.
			 * 값 범위: APIC id 의 하위 24비트. 대부분의 시스템에서는 상위 8비트가 0 이라
			 *   이 필드만으로 충분하지만, 아주 큰 시스템에서는 그렇지 않다. */
			vector			:  8,
			/* [한국어] CPU 를 깨울 인터럽트 벡터 번호.
			 * 설정자: 인터럽트를 설정할 때 커널이 배정한 벡터를 넣는다.
			 * 읽는 자: 하드웨어가 인터럽트 메시지를 만들 때.
			 * 값 범위: 8비트. x86 의 IDT 항목 번호이며, 0~31 은 예외용으로 예약되어 있어
			 *   장치 인터럽트에는 32 이상이 쓰인다.
			 * 이 필드와 위 destid 가 함께 "어느 CPU 의 어느 벡터" 를 이룬다 — 그 둘이
			 *   MSI 메시지의 전부다. */
			reserved_2		: 16,
			/* [한국어] 예약 비트 열여섯 개 — 0 이어야 한다.
			 * 설정자/읽는 자: 아무도 쓰지 않는다.
			 * 왜 이렇게 큰 공백이 있는가: 이 자리에 위 destid_0_23 의 나머지 8비트를 이어
			 *   놓았다면 문제가 없었겠지만, 레지스터 배치가 그렇게 정해져 있지 않다.
			 *   그래서 아래 destid_24_31 이 이 공백 뒤로 밀려나, 목적지 id 가 두 조각으로
			 *   흩어지는 결과가 되었다.
			 * 이 필드는 그 흩어짐의 원인을 눈으로 보여 주는 자리다. */
			destid_24_31		:  8;
			/* [한국어] 목적지 id 의 상위 8비트.
