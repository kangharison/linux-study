...
enum {
	...
	/* [한국어] Dell Inspiron 7375 — 이 기종의 quirk 목록을 가리키는 배열 첨자.
	 * 설정자: 이 enum 자체가 정의다.
	 * 읽는 자: ivrs_ioapic_quirks 배열의 지정 초기화 첨자, 그리고 아래 DMI 표의
	 *   driver_data 가 그 목록을 가리킬 때.
	 * 이 기종의 펌웨어는 IVRS 표에 IOAPIC 항목 두 개를 빠뜨렸다. 그대로 두면
	 *   그 IOAPIC 들의 인터럽트를 재매핑할 수 없어 부팅이 실패하거나 인터럽트가
	 *   전달되지 않는다. 원 주석의 ivrs_ioapic[4]/[5] 지정이 그 두 항목이다.
	 * 0 으로 명시한 이유: 이 값이 배열 첨자로 쓰이므로 시작이 0 임을 분명히 한다.
	 * 왜 숫자 대신 이름인가: 배열 첨자를 숫자로 쓰면 어느 기종의 목록인지 알 수
	 *   없고, DMI 표와 quirk 목록이 어긋나도 눈에 띄지 않는다. 이름으로 이으면
	 *   둘이 같은 상수를 쓰므로 어긋날 여지가 준다. */
	DELL_LATITUDE_5495,
	/* [한국어] Dell Latitude 5495 — 이 기종의 quirk 목록을 가리키는 배열 첨자.
	 * 설정자: 이 enum 자체가 정의다.
	 * 읽는 자: ivrs_ioapic_quirks 배열의 지정 초기화 첨자, 그리고 아래 DMI 표의
	 *   driver_data 가 그 목록을 가리킬 때.
	 * 위 Inspiron 과 같은 계열의 펌웨어 결함을 갖는다. 기종마다 빠진 IOAPIC 의
	 *   장치 id 가 달라 목록을 따로 둔다.
	 * 왜 숫자 대신 이름인가: 배열 첨자를 숫자로 쓰면 어느 기종의 목록인지 알 수
	 *   없고, DMI 표와 quirk 목록이 어긋나도 눈에 띄지 않는다. 이름으로 이으면
	 *   둘이 같은 상수를 쓰므로 어긋날 여지가 준다. */
	LENOVO_IDEAPAD_330S_15ARR,
	/* [한국어] Lenovo IdeaPad 330S-15ARR — 이 기종의 quirk 목록을 가리키는 배열 첨자.
	 * 설정자: 이 enum 자체가 정의다.
	 * 읽는 자: ivrs_ioapic_quirks 배열의 지정 초기화 첨자, 그리고 아래 DMI 표의
	 *   driver_data 가 그 목록을 가리킬 때.
	 * 역시 IVRS 의 IOAPIC 항목이 불완전한 기종이다.
	 * 이 enum 의 마지막 값이며, 같은 결함을 가진 기종이 발견되면 여기 이어
	 *   붙이고 아래 두 표에 항목을 더하면 된다 — 그것이 이 구조의 목적이다.
	 * 왜 숫자 대신 이름인가: 배열 첨자를 숫자로 쓰면 어느 기종의 목록인지 알 수
	 *   없고, DMI 표와 quirk 목록이 어긋나도 눈에 띄지 않는다. 이름으로 이으면
	 *   둘이 같은 상수를 쓰므로 어긋날 여지가 준다. */
};
