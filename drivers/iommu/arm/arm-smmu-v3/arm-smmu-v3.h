...
struct arm_smmu_ll_queue {
	union {
	/* [한국어] 같은 8바이트를 네 가지 관점으로 보는 창.
	 * 어떤 관점들인가: 한 워드(val), 두 개의 u32(prod/cons), 두 개의 atomic_t,
	 *   그리고 캐시 줄을 채우는 바이트 배열이다.
	 * 왜 이렇게까지 겹치는가: 락 없는 명령 큐 삽입이 세 가지를 모두 요구한다.
	 *   가득 참을 판정하려면 두 포인터를 한 번에 읽어야 하고(val), 값을 계산할
	 *   때는 각각의 수로 봐야 하며(prod/cons), 밀 때는 원자 연산이 필요하다(atomic_t).
	 *   타입만 바꿔 겹쳐 두면 형변환 없이 세 가지를 모두 자연스럽게 쓸 수 있다.
	 * 동기화: union 바깥의 ____cacheline_aligned_in_smp 가 시작 주소를 캐시 줄에
	 *   맞추고, 안의 __pad 가 끝까지 채워 줄 하나를 온전히 차지한다. */
		u64			val;
		...
		struct {
		/* [한국어] 위 두 워드를 atomic_t 로 본 두 번째 관점.
		 * 읽는 자: 원자 연산이 필요한 경로 — 명령 큐의 소유권 이양 알고리즘이
		 *   atomic_fetch_inc 계열로 이 이름들을 쓴다.
		 * 바로 위 struct 와 같은 자리를 가리킨다. 값을 그냥 읽으면 되는 곳은 u32
		 *   이름을, 원자적으로 다뤄야 하는 곳은 atomic_t 이름을 쓰도록 나눠 둔 것이다.
		 * 이름이 같고 타입만 다른 두 struct 를 겹쳐 두는 이 방식은, 어느 접근이
		 *   원자적이어야 하는지를 코드에 드러내 준다 — 이름을 고르는 순간 그 결정이
		 *   기록된다. */
			atomic_t	prod;
