...
enum arm_smmu_s2cr_privcfg {
	...
	S2CR_PRIVCFG_DIPAN,
	/* [한국어] 모든 접근을 권한 없는 것으로 강제한다. */
	S2CR_PRIVCFG_UNPRIV,
	/* [한국어] 모든 접근을 특권으로 강제한다. */
	S2CR_PRIVCFG_PRIV,
};
...
enum arm_smmu_implementation {
	...
	GENERIC_SMMU,
	/* [한국어] ARM 의 MMU-500. 알려진 문제를 우회하는 리셋 절차가 있다. */
	ARM_MMU500,
...
enum arm_smmu_context_fmt {
	/* [한국어] 아직 정하지 않았다. 능력에 맞춰 고르라는 뜻. */
	ARM_SMMU_CTX_FMT_NONE,
	/* [한국어] 64비트 긴 서술자 형식. 오늘날 대부분 이것이다. */
	ARM_SMMU_CTX_FMT_AARCH64,
	/* [한국어] 32비트 긴 서술자(LPAE) 형식.
	 *  32비트 커널이 40비트 물리 주소를 쓸 때의 형식이다. */
	ARM_SMMU_CTX_FMT_AARCH32_L,
	/* [한국어] 32비트 짧은 서술자 형식. 가장 오래된 방식이다. */
...
struct arm_smmu_context_fault_info {
	/* [한국어] 오류가 난 주소(FAR 레지스터). */
	unsigned long iova;
	/* [한국어] 오류 상태 비트들(FSR). 어떤 종류의 오류인지 담는다. */
	u32 fsr;
	/* [한국어] 부가 정보(FSYNR0). 읽기였는지 쓰기였는지 등. */
	u32 fsynr;
