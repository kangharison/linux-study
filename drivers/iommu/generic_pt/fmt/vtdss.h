...
enum {
	...
	PT_MAX_TOP_LEVEL = 4,	/* [한국어] SS-PML5E 까지 — 0 기반이라 4 가 5단계다 */
	PT_GRANULE_LG2SZ = 12,	/* [한국어] 최소 페이지 4KB */
	PT_TABLEMEM_LG2SZ = 12,	/* [한국어] 표 하나가 4KB */

	/* SSPTPTR is 4k aligned and limited by HAW */
	PT_TOP_PHYS_MASK = GENMASK_ULL(63, 12),	/* [한국어] (원 주석: SSPTPTR 은 4K 정렬이고 HAW 가 상한을 정한다) */
};
 ...
 */
enum {
	VTDSS_FMT_R = BIT(0),	/* [한국어] 읽기 허용. 이것이 존재 표시를 겸한다 */
	VTDSS_FMT_W = BIT(1),	/* [한국어] 쓰기 허용 */
	VTDSS_FMT_A = BIT(8),	/* [한국어] 접근됨 — 하드웨어가 세운다 */
	VTDSS_FMT_D = BIT(9),	/* [한국어] 더티 — 쓰기가 있었다는 표시 */
	VTDSS_FMT_SNP = BIT(11),	/* [한국어] 장치 접근이 CPU 캐시를 스누프하게 한다 */
	VTDSS_FMT_OA = GENMASK_ULL(51, 12),	/* [한국어] 출력 주소(페이지 번호) */
};
 ...
 */
enum {
	VTDSS_FMT_PS = BIT(7),	/* [한국어] 아래 표가 아니라 큰 페이지라는 표시 */
};
