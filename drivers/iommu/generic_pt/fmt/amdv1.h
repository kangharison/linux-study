...
enum {
	...
	PT_MAX_OUTPUT_ADDRESS_LG2 = 51,	/* [한국어] 시험용 출력 주소 폭 */
	PT_MAX_TOP_LEVEL = 4,	/* [한국어] 시험용 최대 단계 */
	PT_GRANULE_LG2SZ = 11,	/* [한국어] 2KB — 커널 페이지와 일부러 어긋나게 한다 */
#else
	PT_MAX_VA_ADDRESS_LG2 = 64,	/* [한국어] 실제 하드웨어: 입력 주소 전 범위 */
	PT_MAX_OUTPUT_ADDRESS_LG2 = 52,	/* [한국어] 출력 물리 주소 52비트 */
	PT_MAX_TOP_LEVEL = 5,	/* [한국어] 최대 6단계(0~5) */
	...
	PT_TOP_PHYS_MASK = GENMASK_ULL(51, 12),	/* [한국어] (원 주석: DTE 의 최상위 물리 주소 필드는 이 비트뿐이다) */
};
 ...
 */
enum {
	AMDV1PT_FMT_PR = BIT(0),	/* [한국어] 존재 비트 — 0 이면 빈 항목 */
	AMDV1PT_FMT_D = BIT(6),	/* [한국어] 더티 — 하드웨어가 쓰기를 했다는 표시 */
	AMDV1PT_FMT_NEXT_LEVEL = GENMASK_ULL(11, 9),	/* [한국어] 다음 단계 코드: 0=잎, 7=크기 인코딩 잎, 그 밖=표 */
	AMDV1PT_FMT_OA = GENMASK_ULL(51, 12),	/* [한국어] 출력 주소(페이지 번호). 큰 페이지에서는 하위에 크기가 인코딩된다 */
	AMDV1PT_FMT_FC = BIT_ULL(60),	/* [한국어] 강제 일관 — 장치의 비일관 접근을 무시한다 */
	AMDV1PT_FMT_IR = BIT_ULL(61),	/* [한국어] 읽기 허용 */
	AMDV1PT_FMT_IW = BIT_ULL(62),	/* [한국어] 쓰기 허용 */
};
