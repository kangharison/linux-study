 ...
 * x86_64_pt_write_attrs: 항목 하나를 쓸 때 필요한 값 묶음.
 * pt_write_attrs: 공통 코드가 부르는 이름. 형식마다 다른 구조체를 같은
 *   이름으로 보이게 해 템플릿이 성립한다.
 */
#ifndef __GENERIC_PT_FMT_DEFS_X86_64_H	/* [한국어] 중복 포함 방지 */
#define __GENERIC_PT_FMT_DEFS_X86_64_H	/* [한국어] 같은 이름으로 표시 */

#include <linux/generic_pt/common.h>	/* [한국어] 형식과 무관한 공통 정의 */
#include <linux/types.h>	/* [한국어] u32/u64 등 고정 폭 정수 */

typedef u64 pt_vaddr_t;	/* [한국어] 입력 주소 64비트 — 상위는 부호 확장으로 해석된다 */
typedef u64 pt_oaddr_t;	/* [한국어] 출력 물리 주소 */

struct x86_64_pt_write_attrs {	/* [한국어] 항목 하나를 쓸 때 필요한 값 묶음 — 공통 코드가 내용을 모른 채 들고 다닌다 */
	u64 descriptor_bits;
	/* [한국어] 항목에 얹을 권한·속성 비트(P, RW, US, XD 등).
	 * 설정자: 형식 구현이 공통 권한을 x86 페이지 테이블 비트로 옮겨 채운다.
	 * 읽는 자: 항목을 쓰는 코드가 물리 주소와 OR 한다.
	 * 값 범위: 주소 필드를 뺀 나머지 비트.
	 * 동기화: 호출 스택 값이라 공유되지 않는다. */
	gfp_t gfp;
...
#define pt_write_attrs x86_64_pt_write_attrs	/* [한국어] 공통 코드가 부르는 이름으로 */
