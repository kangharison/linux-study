...
enum {
	...
	/* [한국어] 이 표의 캐시 플러시가 끝났음을 표시하는 소프트웨어 비트의 번호.
	 * 설정자: 비일관 플랫폼에서 표를 캐시에서 밀어낸 뒤, 그 표를 가리키는 항목에 세운다.
	 * 읽는 자: 같은 표를 다시 만나는 다른 CPU. 이 비트가 서 있으면 플러시를
	 *   다시 하지 않는다.
	 * 왜 필요한가: 비일관 IOMMU 에서는 표를 만들 때마다 캐시를 밀어내야 하는데,
	 *   그 비용이 크다. 여러 CPU 가 같은 표에 매핑을 추가할 때 각자 다시 밀어내면
	 *   낭비이므로, 한 번 끝났다는 사실을 항목 안에 기록해 둔다.
	 * 동기화가 핵심이다: 이 비트를 세울 때는 해제(release), 읽을 때는 획득(acquire)
	 *   순서를 붙인다. 그래야 이 비트가 서 있는 것을 본 CPU 가 그 표의 내용도
	 *   확실히 볼 수 있다. 순서를 빼면 비트만 보이고 내용은 옛것인 창이 생긴다.
	 * 값 범위: 0 — 형식이 제공하는 소프트웨어 비트 중 첫 번째를 쓴다. */
};
