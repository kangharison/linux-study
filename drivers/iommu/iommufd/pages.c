...
struct pfn_reader {
	...
	union {
		...
		/* [한국어] 사용자 메모리 또는 파일이 원천일 때의 읽기 상태.
		 * 설정자: pfn_reader_init() 이 pages 의 종류를 보고 이쪽을 초기화한다.
		 * 읽는 자: pfn_reader_next() 가 다음 묶음의 PFN 을 가져올 때.
		 * 무엇이 들어 있는가: 지금까지 어디까지 읽었는지, 고정해 둔 페이지 배열,
		 *   그리고 사용자 주소 공간이나 파일에서 다음 페이지를 얻는 데 필요한 상태다.
		 * 아래 dmabuf 와 union 인 이유: 한 pfn_reader 는 하나의 pages 를 읽고,
		 *   그 pages 의 원천은 하나로 정해져 있다. 둘을 동시에 쓸 일이 없다.
		 * 어느 쪽이 유효한지 따로 표시하지 않는 이유는 위 주석에 있다 — pages 의
		 *   종류가 이미 그것을 말해 준다. */
		struct pfn_reader_dmabuf dmabuf;
		/* [한국어] dma-buf 가 원천일 때의 읽기 상태.
		 * 설정자/읽는 자: 위 user 와 같은 자리들.
		 * 왜 별도 구조가 필요한가: dma-buf 의 메모리에는 struct page 가 없을 수
		 *   있다. 페이지 배열을 채우는 보통의 방식을 쓸 수 없어, 스캐터리스트에서
		 *   직접 물리 주소를 꺼내는 다른 절차를 밟는다.
		 * 수명 관리도 다르다: dma-buf 는 첨부(attach)와 매핑(map)의 단계가 있고,
		 *   공급자가 메모리를 옮길 수 있어 이동 통지(move notify)를 처리해야 한다.
		 *   그 상태가 여기 들어 있다. */
	};
};
