...
struct iommu_test_cmd {
	...
	union {
		...
		struct {
			__u32 out_stdev_id;
			/* [한국어] (MOCK_DOMAIN_FLAGS) 만들어진 모의 장치의 selftest 객체 id.
			 * 설정자: 커널의 selftest.c 가 명령을 처리하며 채운다 — 출력이다.
			 * 읽는 자: 테스트 프로그램. 나중에 이 장치를 지울 때 IOMMU_DESTROY 에 넘긴다.
			 * 위 mock_domain 갈래의 같은 이름 필드와 뜻이 같다. 이 갈래는 그것에
			 *   장치 성격 플래그를 더한 확장판이라, 앞의 세 출력 필드를 그대로 되풀이한다.
			 * 왜 되풀이하는가: 두 명령이 같은 배치를 공유해야 사용자 공간 코드가
			 *   구조체를 갈아 끼우지 않고 플래그만 더해 쓸 수 있다. */
			__u32 out_hwpt_id;
			/* [한국어] (MOCK_DOMAIN_FLAGS) 그 모의 장치에 자동으로 붙은 페이지 테이블의 id.
			 * 설정자: 커널이 채운다 — 출력이다.
			 * 읽는 자: 테스트 프로그램이 이 도메인의 매핑을 검사하거나 교체할 때 쓴다.
			 * 왜 자동으로 붙는가: 장치를 만들자마자 어딘가에 붙어 있어야 DMA 를 흉내 낼
			 *   수 있다. 테스트가 매번 도메인을 따로 만들어 붙이는 수고를 덜어 준다.
			 * 위 mock_domain 갈래와 같은 자리에 있어야 하는 이유는 out_stdev_id 와 같다. */
			__u32 out_idev_id;
			/* [한국어] (MOCK_DOMAIN_FLAGS) 그 장치의 iommufd 장치 객체 id.
			 * 설정자: 커널이 채운다 — 출력이다.
			 * 읽는 자: 테스트 프로그램이 이 장치를 정규 iommufd 명령에 쓸 때.
			 * 왜 중요한가: 이것은 정규 IOMMU_DEVICE_BIND 가 돌려주는 것과 같은 종류의
			 *   객체다. 즉 모의 장치를 만든 뒤에는 시험 전용 명령이 아니라 진짜 uAPI 로
			 *   다룰 수 있어, 테스트가 실제 사용자 코드와 같은 경로를 밟는다.
			 * 위 out_stdev_id 와 나뉘어 있는 이유: stdev 는 selftest 가 만든 가짜 장치이고,
			 *   idev 는 그것을 iommufd 에 등록한 결과다. 수명과 소유자가 다르다. */
		...
		struct {
			__aligned_u64 iova;
			/* [한국어] (MD_CHECK_MAP) 검사를 시작할 IOVA.
			 * 설정자: 테스트 프로그램 — 입력이다.
			 * 읽는 자: selftest.c 가 이 주소부터 아래 length 만큼 도메인을 따라 내려간다.
			 * __aligned_u64 인 이유: 이 헤더는 사용자 공간과 공유하는 uAPI 다. 32비트
			 *   사용자 공간과 64비트 커널이 같은 구조체를 봐야 하는데, 보통의 u64 는
			 *   32비트에서 4바이트 정렬이라 배치가 어긋난다. 이 타입이 양쪽에서 8바이트
			 *   정렬을 강제해 그 문제를 없앤다.
			 * 값 범위: 검사할 도메인에 실제로 매핑된 구간이어야 한다. */
			__aligned_u64 length;
		...
		struct {
			__aligned_u64 length;
			/* [한국어] (MD_CHECK_REFS) 참조 수를 검사할 사용자 메모리의 길이.
			 * 설정자: 테스트 프로그램 — 입력이다.
			 * 읽는 자: selftest.c 가 아래 uptr 부터 이 길이만큼의 페이지를 훑는다.
			 * 무엇을 검사하는가: 그 페이지들의 참조 수가 기대값과 같은지 본다. iommufd 가
			 *   고정(pin)을 새게 하면 참조가 남아 값이 크고, 이중으로 놓았으면 작다.
			 *   둘 다 조용히 지나가면 나중에 메모리 누수나 해제된 페이지 사용으로
			 *   나타나므로, 테스트가 그 자리에서 잡아내려는 것이다.
			 * 값 범위: 페이지 크기의 배수여야 의미가 있다. */
			__aligned_u64 uptr;
		...
		struct {
			...
			__aligned_u64 iova;
			/* [한국어] (ACCESS_PAGES) 고정할 구간의 시작 IOVA.
			 * 설정자: 테스트 프로그램 — 입력이다.
			 * 읽는 자: selftest.c 가 이 주소부터 아래 length 만큼의 페이지를 고정한다.
			 * 무엇을 흉내 내는가: 실제 드라이버가 iommufd 의 페이지를 직접 참조해 쓰는
			 *   경로(access)다. 그 경로가 페이지를 올바르게 고정하고 놓는지 시험한다.
			 * 이 명령의 결과인 out_access_pages_id 를 DESTROY_ACCESS_PAGES 에 넣어야
			 *   고정이 풀린다 — 짝을 맞추지 않으면 참조가 남아 위 MD_CHECK_REFS 가 잡아낸다. */
			__aligned_u64 length;
		...
		struct {
			__aligned_u64 iova;
			/* [한국어] (ACCESS_RW) 읽거나 쓸 구간의 시작 IOVA.
			 * 설정자: 테스트 프로그램 — 입력이다.
			 * 읽는 자: selftest.c 가 이 주소부터 아래 length 만큼을 접근자(access)를 통해
			 *   읽거나 쓴다.
			 * 위 access_pages 와의 차이: 그쪽은 페이지를 고정해 두고 직접 접근하는 방식이고,
			 *   이쪽은 고정 없이 커널이 대신 복사해 주는 방식이다. iommufd 는 두 경로를
			 *   모두 제공하며, 짧은 접근에는 후자가 싸다.
			 * 아래 flags 가 방향과 어느 경로를 쓸지를 정한다. */
			__aligned_u64 length;
			/* [한국어] 반대편 버퍼가 되는 사용자 메모리 주소.
			 *  읽기면 여기로 담고, 쓰기면 여기서 가져간다. */
			__aligned_u64 uptr;
			__u32 flags;
			/* [한국어] (ACCESS_RW) MOCK_ACCESS_RW_* 조합 — 방향과 경로 선택을 담는다.
			 * 설정자: 테스트 프로그램 — 입력이다.
			 * 읽는 자: selftest.c 가 읽기인지 쓰기인지, 그리고 느린 경로를 강제할지 정할 때.
			 * 왜 경로를 고를 수 있어야 하는가: 커널의 복사 구현에는 빠른 경로(연속된
			 *   페이지를 한 번에)와 느린 경로(페이지마다 따로)가 있다. 실제 데이터로는
			 *   빠른 경로만 타기 쉬워, 느린 경로를 강제하는 플래그가 없으면 그 코드가
			 *   시험되지 않는다.
			 * 값 범위: MOCK_ACCESS_RW_WRITE, MOCK_ACCESS_RW_SLOW_PATH 등의 OR. */
		} access_rw;
		...
		struct {
			...
			__u32 flags;
			__aligned_u64 iova;
			/* [한국어] (DIRTY) 더럽혔다고 표시할 구간의 시작 IOVA.
			 * 설정자: 테스트 프로그램 — 입력이다.
			 * 읽는 자: selftest.c 가 이 주소부터 아래 length 만큼의 페이지 테이블 항목에
			 *   더티 비트를 세운다.
			 * 왜 이런 명령이 필요한가: 더티 추적을 시험하려면 장치가 실제로 메모리를
			 *   써야 하는데, 모의 장치는 DMA 를 하지 않는다. 그래서 하드웨어가 세웠을
			 *   비트를 소프트웨어가 대신 세워 두고, 수확 경로(GET_DIRTY_BITMAP)가 그것을
			 *   올바르게 읽어 내는지 본다.
			 * 아래 page_size 가 이 구간을 어떤 단위로 나눌지 정한다. */
			__aligned_u64 length;
		...
		struct {
			...
			__u32 id;
			__u32 cache;
			/* [한국어] (DEV_CHECK_CACHE) 그 캐시 항목이 가져야 할 값 — 기대값이다.
			 * 설정자: 테스트 프로그램 — 입력이다.
			 * 읽는 자: selftest.c 가 모의 장치의 캐시 배열에서 위 id 번째 값을 꺼내
			 *   이 값과 견주고, 다르면 명령을 실패시킨다.
			 * 무엇을 시험하는가: 게스트가 보낸 무효화 명령이 실제로 그 장치의 캐시에
			 *   적용되었는지다. 모의 장치의 "캐시"는 정수 배열일 뿐이지만, 무효화 명령이
			 *   올바른 장치의 올바른 항목에 닿았는지를 그것으로 확인할 수 있다.
			 * 값 범위: 무효화가 적용되면 커널이 정해 둔 초기값으로 돌아간다. */
		} check_dev_cache;
		...
		struct {
			...
			__u32 pasid;
			__u32 pt_id;
			/* [한국어] (PASID_ATTACH) 그 PASID 를 붙일 페이지 테이블 또는 IOAS 의 id.
			 * 설정자: 테스트 프로그램 — 입력이다.
			 * 읽는 자: selftest.c 가 모의 장치의 그 PASID 를 이 객체에 붙일 때.
			 * HWPT 와 IOAS 를 모두 받는 이유: 정규 uAPI 의 부착 명령도 둘 다 받는다.
			 *   IOAS 를 주면 커널이 알맞은 HWPT 를 알아서 만들거나 재사용한다.
			 * 바로 아래 주석의 "@id is stdev_id" 가 말하듯, 어느 장치인지는 이 갈래가
			 *   아니라 바깥 구조체의 id 필드가 정한다 — 모든 시험 명령이 공유하는 관례다. */
			/* @id is stdev_id */
		} pasid_attach;
		...
		struct {
			__u32 pasid;
			/* [한국어] (PASID_DETACH) 뗄 PASID 번호.
			 * 설정자: 테스트 프로그램 — 입력이다.
			 * 읽는 자: selftest.c 가 모의 장치의 그 PASID 부착을 해제할 때.
			 * 위 pasid_attach 와 짝을 이룬다. 붙인 것을 떼지 않고 장치를 지우면 참조가
			 *   남는지, 아니면 커널이 알아서 정리하는지를 시험이 확인할 수 있다.
			 * 이 갈래에 pt_id 가 없는 이유: 뗄 때는 무엇에 붙어 있었는지 알 필요가 없다.
			 *   커널이 그 PASID 의 현재 부착을 찾아 해제한다.
			 * 어느 장치인지는 바깥 구조체의 id 가 정한다. */
			/* @id is stdev_id */
		} pasid_detach;
