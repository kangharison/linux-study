...
struct interval_tree_double_span_iter {
	...
	union {
	/* [한국어] 구간의 끝 주소를, 구멍일 때와 쓰인 곳일 때 이름만 달리 읽는 창.
	 * 왜 union 인가: 순회가 내놓는 구간은 언제나 하나이고 그 끝 주소도 하나다.
	 *   다만 그것이 구멍인지 쓰인 곳인지에 따라 읽는 코드의 문맥이 달라, 이름을
	 *   달리 하면 잘못된 문맥에서 읽는 실수가 눈에 띈다.
	 * 어느 이름이 유효한가: is_used 가 0 이면 last_hole, 1 이나 2 면 last_used 다.
	 *   값 자체는 같은 자리에 있으므로 둘 중 어느 이름으로 읽어도 같은 수가 나온다 —
	 *   이 union 은 저장 공간을 아끼려는 것이 아니라 의도를 표현하려는 것이다.
	 * 설정자: 순회의 update 함수. 동기화: 호출 스택 값이라 공유되지 않는다. */
