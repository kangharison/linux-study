...
enum {
	...
	/* [한국어] 고정된 페이지를 아무 한도에도 계상하지 않는다.
	 * 설정자: iopt_pages 를 만들 때 페이지의 출처를 보고 정한다.
	 * 읽는 자: 페이지를 고정하고 놓을 때 어느 회계를 갱신할지 정하는 코드.
	 * 언제 쓰는가: 커널 자신이 소유한 메모리를 매핑할 때처럼, 사용자에게
	 *   물릴 근거가 없는 경우다.
	 * 0 으로 명시한 이유: 구조체를 0 으로 초기화하면 계상하지 않는 상태가 되도록
	 *   맞춘 것이다 — 실수로 잘못된 계정에 물리는 것보다 안전한 기본값이다.
	 * 왜 출처에 따라 갈리는가: 고정된 페이지가 누구의 자원을 묶어 두는지가
	 *   출처마다 다르다. 회계를 잘못 붙이면 사용자가 한도를 우회하거나,
	 *   반대로 쓰지도 않은 메모리가 한도에 잡힌다. */
	IOPT_PAGES_ACCOUNT_USER = 1,
	/* [한국어] 사용자의 잠금 메모리 한도(RLIMIT_MEMLOCK)에 계상한다.
	 * 설정자: iopt_pages 를 만들 때 페이지의 출처를 보고 정한다.
	 * 읽는 자: 페이지를 고정하고 놓을 때 어느 회계를 갱신할지 정하는 코드.
	 * 언제 쓰는가: 사용자 공간이 자기 VA 를 매핑해 달라고 요청한 경우.
	 *   그 페이지들은 스왑될 수 없게 묶이므로, mlock 과 같은 한도를 적용해야
	 *   한 프로세스가 시스템 메모리를 독차지하지 못한다.
	 * 이것이 iommufd 에서 가장 흔한 모드다.
	 * 왜 출처에 따라 갈리는가: 고정된 페이지가 누구의 자원을 묶어 두는지가
	 *   출처마다 다르다. 회계를 잘못 붙이면 사용자가 한도를 우회하거나,
	 *   반대로 쓰지도 않은 메모리가 한도에 잡힌다. */
	IOPT_PAGES_ACCOUNT_MM = 2,
	/* [한국어] 프로세스의 pinned_vm 통계에 계상한다.
	 * 설정자: iopt_pages 를 만들 때 페이지의 출처를 보고 정한다.
	 * 읽는 자: 페이지를 고정하고 놓을 때 어느 회계를 갱신할지 정하는 코드.
	 * 언제 쓰는가: 한도로 막지는 않되 얼마나 고정했는지는 보이게 하고 싶을 때.
	 * 위 USER 와의 차이: USER 는 한도를 넘으면 요청이 실패하지만, 이쪽은 통계만
	 *   올린다. VFIO 호환 경로가 옛 동작을 유지하기 위해 이 모드를 쓴다.
	 * 왜 출처에 따라 갈리는가: 고정된 페이지가 누구의 자원을 묶어 두는지가
	 *   출처마다 다르다. 회계를 잘못 붙이면 사용자가 한도를 우회하거나,
	 *   반대로 쓰지도 않은 메모리가 한도에 잡힌다. */
	IOPT_PAGES_ACCOUNT_MODE_NUM = 3,
	/* [한국어] 회계 모드의 개수 — 값의 유효성을 검사하는 상한이다.
	 * 설정자: 이 enum 자체가 정의다.
	 * 읽는 자: 사용자가 준 모드 값이 범위 안인지 확인하는 곳.
	 * 왜 필요한가: 이 값이 사용자 공간에서 올 수 있어, 범위를 벗어난 값을
	 *   그대로 쓰면 표 밖을 읽거나 정의되지 않은 동작을 한다.
	 * 값이 명시적으로 3 인 이유: 앞의 셋도 값을 명시했으므로 일관되게 적었다.
	 *   이 enum 은 uAPI 와 맞닿아 있어 값이 바뀌면 안 되고, 명시가 그것을 못 박는다. */
};
...
enum iopt_address_type {
	IOPT_ADDRESS_USER = 0,
	/* [한국어] 사용자 VA 에서 온 페이지 — 가장 흔한 경우다.
	 * 설정자: iopt_alloc_user_pages() 가 이 값으로 표시한다.
	 * 읽는 자: 페이지를 실제로 가져오는 코드가 아래 union 중 어느 필드를 읽을지
	 *   정할 때, 그리고 고정 방법(pin_user_pages)을 고를 때.
	 * 값 범위: 0. 이 값이 union 의 태그 노릇을 하므로, 표시하지 않으면 어느
	 *   필드가 유효한지 알 수 없다.
	 * 왜 출처를 구분하는가: 페이지를 얻는 방법도, 놓는 방법도, 회계도 출처마다
	 *   다르다. 사용자 VA 는 pin_user_pages 로 얻고, 아래 파일과 dmabuf 는
	 *   각각 다른 경로를 쓴다. */
	IOPT_ADDRESS_FILE,
	/* [한국어] 파일에서 온 페이지 — memfd 나 hugetlbfs 같은 메모리 파일이다.
	 * 설정자: iopt_alloc_file_pages() 가 표시한다.
	 * 읽는 자: 위 USER 와 같은 자리들.
	 * 사용자 VA 와 나누는 이유: 파일은 사용자 주소 공간에 매핑되어 있지 않아도
	 *   된다. 파일과 오프셋만으로 페이지를 얻을 수 있어, mmap 없이 매핑할 수 있고
	 *   프로세스가 사라져도 그 페이지가 유효하다.
	 * 가상 머신에서 게스트 메모리를 memfd 로 두는 구성이 이 경로를 쓴다. */
	IOPT_ADDRESS_DMABUF,
	/* [한국어] 다른 장치의 메모리 — struct page 가 없을 수도 있다.
	 * 설정자: dma-buf 를 매핑하는 경로가 표시한다.
	 * 읽는 자: 위와 같은 자리들.
	 * 가장 다루기 까다로운 출처다: dma-buf 가 가리키는 것은 시스템 메모리가
	 *   아니라 GPU 의 VRAM 같은 장치 메모리일 수 있고, 그런 메모리에는 struct
	 *   page 가 없다. 페이지 배열을 다루는 보통의 코드 경로를 그대로 쓸 수 없어
	 *   별도의 pfn_reader_dmabuf 가 필요하다.
	 * 왜 이 경로가 필요한가: 장치끼리 메모리를 직접 주고받는 P2P DMA 를 IOMMU
	 *   뒤에서 하려면, 그 메모리를 IOVA 에 매핑할 수 있어야 한다. */
};
