...
enum faulttype {
	DMA_REMAP,
	/* [한국어] DMA 번역 중에 난 폴트 — 장치가 메모리에 접근하려다 실패한 경우다.
	 * 설정자: dmar_get_fault_reason() 이 사유 코드의 범위를 보고 정한다.
	 * 읽는 자: 폴트를 출력하는 코드가 어느 사유 문자열 표를 뒤질지 정할 때.
	 * 왜 종류를 나누는가: VT-d 는 DMA 번역과 인터럽트 재매핑을 한 유닛이 함께
	 *   담당하고, 사유 코드도 두 공간으로 나뉜다. 같은 숫자가 두 공간에서 다른
	 *   뜻이라, 어느 공간의 코드인지 모르면 엉뚱한 문자열을 출력한다.
	 * 값 범위: 사유 코드가 0x30 이상이면 scalable 모드 DMA, 0x20 미만이면
	 *   레거시 DMA 다 — 둘 다 이 종류로 묶인다. */
	INTR_REMAP,
	/* [한국어] 인터럽트 재매핑 중에 난 폴트 — 인터럽트 항목을 찾거나 검증하다 실패한 경우다.
	 * 설정자/읽는 자: 위 DMA_REMAP 과 같다.
	 * 값 범위: 사유 코드의 0x20 대가 이 종류다.
	 * DMA 폴트와 성격이 크게 다르다: DMA 폴트는 매핑을 고치면 해결되는 경우가
	 *   많지만, 인터럽트 재매핑 폴트는 대개 항목을 만들지 않았거나 잘못 만든
	 *   것이라 커널 쪽 버그이거나 장치가 승인되지 않은 인터럽트를 보낸 것이다.
	 * 후자라면 격리가 실제로 작동한 증거이기도 하다. */
	UNKNOWN,
	/* [한국어] 우리가 모르는 사유 코드 — 어느 공간에도 속하지 않는 값이다.
	 * 설정자: 사유 코드가 위 두 범위 중 어디에도 들지 않을 때.
	 * 읽는 자: 출력 코드가 문자열 대신 숫자를 그대로 보여 준다.
	 * 왜 필요한가: 하드웨어가 미래에 새 사유 코드를 낼 수 있고, 그때 표 밖을
	 *   읽으면 커널이 죽는다. 모르는 것을 모른다고 다루는 편이 안전하다.
	 * 이 값이 출력에 나타나면 하드웨어가 이 드라이버보다 새롭다는 뜻이므로,
	 *   로그의 숫자를 규격에서 직접 찾아봐야 한다. */
};
