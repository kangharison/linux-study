 ...
 */
struct page_req_dsc {
	union {	/* [한국어] 비트필드로도 워드로도 읽을 수 있게 겹친다 — 해석은 필드로, 로그는 워드로 */
		struct {	/* [한국어] 앞 워드의 필드 배치 */
			u64 type:8;
	...
	union {	/* [한국어] 둘째 워드도 같은 방식으로 겹친다 */
		struct {	/* [한국어] 둘째 워드의 필드 배치 */
			u64 rd_req:1;
			/* [한국어] 읽기 권한 요청인지.
		...
		u64 qw_1;
		/* [한국어] 위 두 번째 비트필드 묶음과 같은 메모리를 덮는 u64. qw_0 과 같은 용도다. */
	};
	u64 qw_2;
