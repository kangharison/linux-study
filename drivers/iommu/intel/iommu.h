 ...
 *
 * 성능 카운터를 쓰려면 넷 다 있어야 한다. perfmon 초기화가 이 조합으로 한 번에 확인한다
 *
 ...
 * [한국어] 아래 매크로의 각 줄이 하는 일
 *
 * 레지스터가 원하는 상태가 될 때까지 도는 관용구. VT-d 의 명령은 대부분 "비트를 쓰고 상태 비트가 바뀔 때까지 기다린다" 형태라 매크로로 뺐다
 *
 * - do {
 *     매크로를 한 문장처럼 쓰게 하는 관용구
 * - cycles_t start_time = get_cycles();
 *     시작 시각. 타임아웃을 재려면 필요하다
 * - while (1) {
 *     조건이 맞을 때까지 반복
 * - sts = op(iommu->reg + offset);
 *     레지스터를 읽는다. op 는 readl/readq 중 하나로 호출자가 넘긴다
 * - if (cond)
 *     원하는 조건이 되었으면
 * - break;
 *     기다림 종료
 ...
 *     부팅을 멈춘다. IOMMU 가 응답하지 않는데 계속 진행하면 격리 상태를 알 수 없어 더 위험하다
 * - cpu_relax();
 *     바쁜 대기 중임을 CPU 에 알린다(하이퍼스레드 양보, 전력 절약)
 * - }
 *     루프의 끝
 */
...
enum {
	QI_FREE,
	/* [한국어] 이 큐 슬롯은 비어 있다 — 새 서술자를 넣을 수 있다.
	 * 설정자: 큐를 초기화할 때, 그리고 완료 처리가 끝나 슬롯을 반납할 때.
	 * 읽는 자: qi_submit_sync() 가 넣을 자리를 찾을 때.
	 * 이 상태 배열은 하드웨어가 모른다. 하드웨어는 큐의 head/tail 포인터만 보고,
	 *   개별 슬롯의 상태는 커널이 자기 필요로 따로 추적한다 — 완료를 기다리는
	 *   스레드가 어느 슬롯의 결과를 봐야 하는지 알아야 하기 때문이다.
	 * 정상 흐름: QI_FREE → QI_IN_USE → QI_DONE → QI_FREE. */
	QI_IN_USE,
	/* [한국어] 서술자가 들어가 하드웨어의 처리를 기다린다.
	 * 설정자: qi_submit_sync() 가 서술자를 큐에 쓰고 tail 을 올릴 때.
	 * 읽는 자: 완료를 기다리는 스레드, 그리고 오류 복구 경로.
	 * 이 상태에 있는 동안 그 슬롯의 메모리를 재사용하면 안 된다 — 하드웨어가
	 *   아직 읽고 있을 수 있다.
	 * 얼마나 오래 머무는가: 보통은 마이크로초 단위지만, 장치 TLB 무효화처럼
	 *   PCIe 를 왕복하는 명령은 훨씬 길어질 수 있다. */
	QI_DONE,
	/* [한국어] 하드웨어의 처리가 끝났다.
	 * 설정자: 완료 대기 서술자(wait descriptor)가 상태 메모리에 값을 쓴 것을
	 *   본 커널이 이 상태로 옮긴다.
	 * 읽는 자: 완료를 기다리던 스레드. 이 값을 보면 반환한다.
	 * 왜 하드웨어가 직접 이 배열을 갱신하지 않는가: VT-d 는 완료를 알리는 수단이
	 *   "지정한 메모리에 지정한 값을 써 달라"는 wait 서술자 하나뿐이다. 슬롯마다
	 *   상태를 써 주지는 않으므로, 커널이 그 한 값을 보고 나머지를 추론한다. */
	QI_ABORT
	/* [한국어] 오류로 중단되었다 — 이 슬롯뿐 아니라 뒤따르던 서술자들도 함께 무효다.
	 * 설정자: 하드웨어가 서술자를 거부했거나 시간 초과가 났을 때의 복구 경로.
	 * 읽는 자: 복구 경로가 무엇을 다시 제출해야 하는지 정할 때.
	 * 왜 뒤따르던 것까지 무효인가: VT-d 는 오류가 난 지점에서 큐 처리를 멈춘다.
	 *   그 뒤의 서술자들은 하드웨어가 손대지 않은 채 남아 있으므로, 복구 코드가
	 *   그것들을 모두 다시 제출해야 한다.
	 * 이 상태가 이 enum 을 필요하게 만드는 이유다. 단순히 "끝났는가"만 알면
	 *   되었다면 불리언 하나로 충분했을 것이다. */
};
...
struct iommu_flush {
	...
	/* [한국어] 컨텍스트 캐시 무효화의 구현 — 레지스터 방식과 큐 방식이 갈린다.
	 * 설정자: 유닛 초기화가 큐를 쓸 수 있는지 보고 둘 중 하나를 꽂는다.
	 * 읽는 자: 컨텍스트 항목을 고친 뒤 캐시를 비워야 하는 모든 자리.
	 * @did:  무효화할 도메인 id. 0 이면 전역이다.
	 * @sid:  대상 장치의 소스 id(BDF).
	 * @fm:   함수 마스크 — 여러 기능을 한 번에 지정할 때 쓴다.
	 * @type: 무효화 범위(전역/도메인/장치).
	 * 왜 함수 포인터인가: 큐 기반 무효화(QI)는 큐가 초기화된 뒤에만 쓸 수 있다.
	 *   그런데 부팅 초기에도 무효화가 필요해, 그때는 레지스터를 직접 쓰는 느린
	 *   방식을 쓴다. 두 구현을 이 포인터로 갈아 끼우면 호출부가 시점을 신경
	 *   쓰지 않아도 된다. */
			      u8 fm, u64 type);
	void (*flush_iotlb)(struct intel_iommu *iommu, u16 did, u64 addr,
	/* [한국어] IOTLB 무효화의 구현 — 위 flush_context 와 같은 이유로 함수 포인터다.
	 * 설정자: 유닛 초기화가 레지스터 방식 또는 큐 방식을 꽂는다.
	 * 읽는 자: 매핑을 고친 뒤 번역 캐시를 비워야 하는 모든 자리.
	 * @did:        무효화할 도메인 id.
	 * @addr:       무효화할 IOVA 의 시작.
	 * @size_order: 그 구간 크기의 로그 값(페이지 수의 지수).
	 * @type:       전역/도메인/페이지 중 어느 범위인지.
	 * 위 flush_context 와 짝을 이룬다 — 컨텍스트 항목을 고치면 두 캐시를 모두
	 *   비워야 하는 경우가 많다. 순서도 정해져 있어, 컨텍스트를 먼저 비운다. */
...
enum {
	SR_DMAR_FECTL_REG,
	/* [한국어] 서스펜드 때 저장할 레지스터 — 폴트 인터럽트 제어.
	 * 설정자/읽는 자: 아래 배열의 첨자로 쓰인다. iommu_suspend() 가 이 자리에
	 *   레지스터 값을 담고, iommu_resume() 이 되쓴다.
	 * 왜 이 레지스터를 보존하는가: 전원이 끊기면 인터럽트 마스크가 초기값으로
	 *   돌아간다. 복원하지 않으면 리줌 뒤 폴트 인터럽트가 마스크된 채로 남아,
	 *   변환 오류가 조용히 묻힌다.
	 * 이 enum 은 값이 아니라 첨자다 — 레지스터 오프셋이 아니라 저장 배열에서의
	 *   자리를 뜻하므로, 순서를 바꾸면 저장과 복원이 어긋난다. */
	SR_DMAR_FEDATA_REG,
	/* [한국어] 폴트 인터럽트 MSI 의 데이터 워드를 저장할 자리.
	 * 설정자/읽는 자: 위 FECTL 과 같은 방식.
	 * 왜 필요한가: MSI 는 "이 주소에 이 값을 써서 인터럽트를 낸다"는 방식이다.
	 *   데이터 워드는 어느 벡터를 낼지 담고 있어, 복원하지 않으면 리줌 뒤 폴트가
	 *   엉뚱한 벡터로 가거나 아예 전달되지 않는다.
	 * 아래 FEADDR/FEUADDR 과 셋이 한 벌이다 — 주소와 데이터가 모두 맞아야
	 *   MSI 가 성립한다. */
	SR_DMAR_FEADDR_REG,
	/* [한국어] 폴트 인터럽트 MSI 주소의 하위 32비트를 저장할 자리.
	 * 설정자/읽는 자: 위와 같은 방식.
	 * 왜 상하위로 나뉘는가: MSI 주소가 64비트일 수 있는데 레지스터는 32비트씩
	 *   나뉘어 있다. 하드웨어 레지스터의 배치를 그대로 따라 저장 자리도 나눈다.
	 * 이 주소는 인터럽트 컨트롤러의 도어벨을 가리킨다. 인터럽트 재매핑을 쓰면
	 *   실제 도어벨이 아니라 재매핑 표를 가리키는 값이 들어간다. */
	SR_DMAR_FEUADDR_REG,
	/* [한국어] 폴트 인터럽트 MSI 주소의 상위 32비트를 저장할 자리.
	 * 설정자/읽는 자: 위와 같은 방식.
	 * 넷을 모두 복원해야 리줌 뒤 폴트 보고가 계속 동작한다 — 하나라도 빠지면
	 *   인터럽트가 전달되지 않거나 엉뚱한 곳으로 간다.
	 * 32비트 시스템에서는 이 레지스터가 0 이지만, 저장과 복원 코드는 구분하지
	 *   않고 넷을 똑같이 다룬다. */
	MAX_SR_DMAR_REGS
	/* [한국어] 저장할 레지스터의 개수 — 배열 크기로 쓰인다.
	 * 설정자: enum 의 마지막 값이라 컴파일러가 자동으로 정한다.
	 * 읽는 자: struct intel_iommu 의 저장 배열 선언, 그리고 저장/복원 루프의 상한.
	 * 왜 이 관용구를 쓰는가: 레지스터를 하나 추가하면 위에 이름을 넣기만 하면
	 *   되고, 배열 크기와 루프 상한이 자동으로 따라간다. 숫자를 따로 적어 두면
	 *   둘이 어긋나 배열 밖을 넘어설 수 있다.
	 * 값 범위: 현재 4. */
};
 ...
 * [한국어] 아래 매크로의 각 줄이 하는 일
 *
 * 2단계 더티 추적을 쓸 수 있는가. 추적 비트(slads)뿐 아니라 워크 코히런시(smpwc)까지 요구하는 것은, 하드웨어가 남긴 비트를 CPU 가 캐시를 거치지 않고 읽어야 하기 때문이다
 *
...
struct dmar_domain {
	union {
	/* [한국어] 도메인의 "얼굴" — 종류에 따라 아래 넷 중 하나로 읽는다.
	 * 왜 union 인가: 이 드라이버는 여러 종류의 도메인(보통의 DMA, 중첩, SVA,
	 *   IDENTITY)을 하나의 struct dmar_domain 으로 표현한다. 종류마다 코어에
	 *   노출하는 얼굴이 다르지만 한 도메인이 두 종류일 수는 없어, 겹쳐 두면
	 *   구조체가 작아진다.
	 * 결정적인 제약: 넷 모두 같은 자리에서 iommu_domain 으로 시작하도록 배치되어
	 *   있어야 한다. to_dmar_domain() 이 코어가 준 iommu_domain 포인터에서
	 *   container_of 로 되짚을 때, 어느 얼굴이었는지 몰라도 되기 때문이다.
	 * 읽는 자: 도메인 종류를 아는 코드가 해당 얼굴로 접근한다. */
	...
	union {
		...
		struct {
		/* [한국어] (아래 영어 주석 참고) DMA 재매핑 도메인일 때만 유효한 추가 상태.
		 * 언제 유효한가: 이 도메인이 보통의 DMA 도메인일 때. 아래 중첩/SVA 갈래와
		 *   겹쳐 있으므로 한 도메인이 둘 이상일 수 없다.
		 * 무엇이 들어 있는가: 이 도메인 위에 얹힌 중첩 자식 도메인들의 목록과 그
		 *   목록을 지키는 락이다. 즉 이 도메인은 자식들의 부모(2단계) 노릇을 할 수 있다.
		 * 왜 익명 struct 인가: 이름을 붙이면 접근할 때마다 한 단계를 더 거쳐야 해서,
		 *   이미 union 안에 들어 있는 필드들의 경로가 더 길어진다. */
		...
		/* Nested user domain */
		struct {
		/* [한국어] (아래 영어 주석 참고) 중첩 사용자 도메인일 때만 유효한 추가 상태.
		 * 언제 유효한가: 이 도메인이 IOMMU_DOMAIN_NESTED 일 때.
		 * 무엇이 들어 있는가: 이 도메인이 얹혀 있는 부모(2단계) 도메인의 포인터다.
		 *   게스트의 DMA 는 이 도메인의 1단계 테이블로 게스트 물리 주소를 얻고,
		 *   그 주소를 부모의 2단계 테이블로 다시 옮겨야 호스트 물리 주소가 된다.
		 * 위 DMA 갈래와 겹쳐 있다는 점이 설계를 말해 준다 — 부모가 될 수 있는
		 *   도메인과 자식이 되는 도메인은 서로 다른 종류이며, 하나가 둘 다일 수 없다. */
		...
		/* SVA domain */
		struct {
		/* [한국어] (아래 영어 주석 참고) SVA 도메인일 때만 유효한 추가 상태.
		 * 언제 유효한가: 이 도메인이 IOMMU_DOMAIN_SVA 일 때.
		 * 무엇이 들어 있는가: 프로세스의 주소 공간 변경을 통보받는 mmu_notifier 다.
		 * 왜 필요한가: SVA 도메인은 자기 페이지 테이블을 갖지 않고 프로세스의 것을
		 *   그대로 가리킨다. 프로세스 쪽에서 매핑이 바뀌면(munmap, 페이지 회수, THP
		 *   분할) IOMMU 캐시도 함께 비워야 하는데, 그 시점을 알 방법이 이 notifier 뿐이다.
		 * 다른 두 갈래와 겹쳐 있으므로, SVA 도메인은 중첩의 부모도 자식도 될 수 없다. */
...
enum cache_tag_type {
	CACHE_TAG_IOTLB,
	/* [한국어] 유닛 안의 번역 캐시(IOTLB)를 가리키는 태그.
	 * 설정자: 도메인에 장치를 붙일 때 cache_tag_assign() 이 만든다.
	 * 읽는 자: 매핑을 고친 뒤 무효화를 보낼 때, 이 태그를 보고 어떤 명령을
	 *   만들지 정한다.
	 * 왜 태그가 필요한가: 한 도메인에 붙은 장치들이 여러 유닛에 흩어져 있을 수
	 *   있고, 유닛마다 캐시가 따로다. 매핑 하나를 고쳤을 때 어느 유닛의 어떤
	 *   캐시를 비워야 하는지를 미리 계산해 목록으로 들고 있는 것이 이 태그다.
	 * 값 범위: 이 종류는 유닛 안의 캐시라 무효화가 빠르다 — 아래 DEVTLB 와 대비된다. */
	CACHE_TAG_DEVTLB,
	/* [한국어] 장치 쪽 변환 캐시(Device TLB)를 가리키는 태그.
	 * 설정자/읽는 자: 위 IOTLB 태그와 같은 방식.
	 * IOTLB 와 나누는 이유: 장치 TLB 무효화는 PCIe 를 왕복하고 장치의 응답을
	 *   기다려야 해서 훨씬 느리다. 그리고 ATS 를 켠 장치에만 필요하다. 태그를
	 *   나눠 두면 필요 없는 장치에 비싼 명령을 보내지 않는다.
	 * 순서도 중요하다: IOTLB 를 먼저 비우고 DEVTLB 를 나중에 비워야, 그 사이
	 *   장치가 낡은 변환을 다시 받아 가지 않는다. */
	CACHE_TAG_NESTING_IOTLB,
	/* [한국어] 중첩 변환에서 쓰는 IOTLB 태그.
	 * 설정자/읽는 자: 위와 같은 방식.
	 * 왜 따로 있는가: 중첩에서는 게스트가 1단계를, 호스트가 2단계를 고친다.
	 *   어느 쪽이 바뀌었느냐에 따라 보내야 할 무효화 명령의 형식과 범위가 달라서,
	 *   같은 유닛·같은 장치라도 태그를 나눠 둔다.
	 * 호스트가 2단계를 고쳤을 때는 그 아래 모든 게스트 1단계 결과도 못 믿게
	 *   되므로, 이 태그의 무효화는 보통의 IOTLB 무효화보다 넓은 범위를 지운다. */
	CACHE_TAG_NESTING_DEVTLB,
	/* [한국어] 중첩 변환에서 쓰는 장치 TLB 태그.
	 * 설정자/읽는 자: 위와 같은 방식.
	 * 위 세 태그와의 관계: 이 네 값이 (유닛 캐시 / 장치 캐시) × (보통 / 중첩)의
	 *   네 조합을 이룬다. 무효화 대상 목록에 태그가 섞여 들어 있고, 무효화 경로가
	 *   그것을 훑으며 각 태그에 맞는 명령을 만든다.
	 * 중첩에서 장치 TLB 를 비울 때는 게스트가 쓰는 PASID 까지 명령에 실어야 해,
	 *   보통의 DEVTLB 무효화와 명령 형식이 다르다. */
};
