 ...
 */
int pci_request_irq(struct pci_dev *dev, unsigned int nr, irq_handler_t handler, /* NVMe: NVMe 큐의 IRQ 핸들러 등록 함수 선언(매개변수는 다음 줄에 이어짐). */
		irq_handler_t thread_fn, void *dev_id, const char *fmt, ...) /* NVMe: NVMe 큐에 대한 IRQ 핸들러 등록 함수 선언. */
{ /* NVMe: NVMe 큐 IRQ 핸들러 등록 함수 본문 시작. */
	...
	ret = request_threaded_irq(pci_irq_vector(dev, nr), handler, thread_fn, /* NVMe: NVMe 큐 인덱스를 커널 IRQ 번호로 변환하여 스레드 IRQ 등록(인자는 다음 줄에 이어짐). */
				   irqflags, devname, dev_id); /* NVMe: IRQ 플래그, 큐별 이름, dev_id를 전달하여 등록을 완료. */
...
}	/* NVMe: NVMe 큐 IRQ 핸들러 등록 함수 종료. */
EXPORT_SYMBOL(pci_request_irq);	/* NVMe: NVMe 드라이버 모듈에서 pci_request_irq 심볼을 사용할 수 있도록 낸다. */
 ...
 */
void pci_free_irq(struct pci_dev *dev, unsigned int nr, void *dev_id) /* NVMe: NVMe 큐에 등록된 IRQ 핸들러 해제 함수. */
{ /* NVMe: NVMe 큐 IRQ 핸들러 해제 함수 본문 시작. */
	kfree(free_irq(pci_irq_vector(dev, nr), dev_id)); /* NVMe: 큐 인덱스를 커널 IRQ 번호로 변환해 free_irq()를 호출하고 반환된 이름 버퍼를 해제. */
	/* NVMe: 큐 인덱스(nr)를 커널 IRQ 번호로 변환하여 free_irq() 호출.
	 *      free_irq()는 등록 시 사용했던 devname을 반환하며, 여기서
	 *      메모리를 해제한다. */
}	/* NVMe: NVMe 큐 IRQ 핸들러 해제 함수 종료. */
EXPORT_SYMBOL(pci_free_irq);	/* NVMe: NVMe 드라이버 모듈에서 pci_free_irq 심볼을 사용할 수 있도록 낸다. */
