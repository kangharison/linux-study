...
/* [한국어] BAR 0~3 을 담는 레지스터. 함수마다 8바이트씩 떨어져 있다. */
#define CDNS_PCIE_LM_EP_FUNC_BAR_CFG0(fn) \
	(CDNS_PCIE_LM_BASE + 0x0240 + (fn) * 0x0008)
/* [한국어] BAR 4~5 를 담는 레지스터. CFG0 바로 다음 4바이트다. */
#define CDNS_PCIE_LM_EP_FUNC_BAR_CFG1(fn) \
	(CDNS_PCIE_LM_BASE + 0x0244 + (fn) * 0x0008)
/* [한국어] SR-IOV 가상 함수(VF)용 같은 구조. PF 와 별도의 BAR 설정을 갖는다. */
#define CDNS_PCIE_LM_EP_VFUNC_BAR_CFG(bar, fn) \
	(((bar) < BAR_4) ? CDNS_PCIE_LM_EP_VFUNC_BAR_CFG0(fn) : CDNS_PCIE_LM_EP_VFUNC_BAR_CFG1(fn))
/* [한국어] VF 의 BAR 0~3 설정 레지스터. */
#define CDNS_PCIE_LM_EP_VFUNC_BAR_CFG0(fn) \
	(CDNS_PCIE_LM_BASE + 0x0280 + (fn) * 0x0008)
/* [한국어] VF 의 BAR 4~5 설정 레지스터. */
#define CDNS_PCIE_LM_EP_VFUNC_BAR_CFG1(fn) \
	(CDNS_PCIE_LM_BASE + 0x0284 + (fn) * 0x0008)
/* [한국어] 레지스터 안에서 BAR b 의 aperture(크기 지수) 자리. 한 BAR 이
 * 8비트를 차지하므로 b * 8 만큼 밀고, 그중 하위 5비트가 크기다.
 * 5비트로 2^0 ~ 2^31 까지 표현한다. */
#define  CDNS_PCIE_LM_EP_FUNC_BAR_CFG_BAR_APERTURE_MASK(b) \
	(GENMASK(4, 0) << ((b) * 8))
/* [한국어] 크기 지수 a 를 BAR b 의 자리로 옮겨 담는다. */
#define  CDNS_PCIE_LM_EP_FUNC_BAR_CFG_BAR_APERTURE(b, a) \
	(((a) << ((b) * 8)) & CDNS_PCIE_LM_EP_FUNC_BAR_CFG_BAR_APERTURE_MASK(b))
/* [한국어] 같은 8비트 안에서 상위 3비트가 BAR 종류(control) 자리다.
 * GENMASK(7,5) 를 b * 8 만큼 민다. */
#define  CDNS_PCIE_LM_EP_FUNC_BAR_CFG_BAR_CTRL_MASK(b) \
	(GENMASK(7, 5) << ((b) * 8))
/* [한국어] 종류 값 c 를 BAR b 의 control 자리로 옮겨 담는다. 시프트가
 * b * 8 + 5 인 것은 8비트 슬롯 안에서 다시 5비트를 건너뛰기 때문이다. */
#define  CDNS_PCIE_LM_EP_FUNC_BAR_CFG_BAR_CTRL(b, c) \
	(((c) << ((b) * 8 + 5)) & CDNS_PCIE_LM_EP_FUNC_BAR_CFG_BAR_CTRL_MASK(b))

/* Endpoint Function Configuration Register */
/* [한국어] 엔드포인트가 몇 개의 물리 함수를 노출할지 정하는 레지스터. */
#define CDNS_PCIE_LM_EP_FUNC_CFG	(CDNS_PCIE_LM_BASE + 0x02C0)

/* Root Complex BAR Configuration Register */
/* [한국어] 루트 컴플렉스 모드의 BAR 설정 레지스터. 루트 포트는 BAR 를
 * 외부에 노출하지 않지만, 인바운드 주소 변환의 창 크기를 정하는 데 같은
 * 구조를 쓴다. */
#define CDNS_PCIE_LM_RC_BAR_CFG	(CDNS_PCIE_LM_BASE + 0x0300)
/* [한국어] RC BAR0 의 크기 지수. 엔드포인트 쪽(5비트)과 달리 6비트라
 * 더 큰 창을 표현할 수 있다. */
#define  CDNS_PCIE_LM_RC_BAR_CFG_BAR0_APERTURE_MASK	GENMASK(5, 0)
/* [한국어] 크기 지수 a 를 BAR0 자리에 넣는다. */
#define  CDNS_PCIE_LM_RC_BAR_CFG_BAR0_APERTURE(a) \
	(((a) << 0) & CDNS_PCIE_LM_RC_BAR_CFG_BAR0_APERTURE_MASK)
/* [한국어] RC BAR0 의 종류(control) 자리, 비트 6~8. */
#define  CDNS_PCIE_LM_RC_BAR_CFG_BAR0_CTRL_MASK		GENMASK(8, 6)
/* [한국어] 종류 값 c 를 BAR0 자리에 넣는다. */
#define  CDNS_PCIE_LM_RC_BAR_CFG_BAR0_CTRL(c) \
	(((c) << 6) & CDNS_PCIE_LM_RC_BAR_CFG_BAR0_CTRL_MASK)
/* [한국어] RC BAR1 의 크기 지수, 비트 9~13. */
#define  CDNS_PCIE_LM_RC_BAR_CFG_BAR1_APERTURE_MASK	GENMASK(13, 9)
/* [한국어] 크기 지수 a 를 BAR1 자리에 넣는다. */
#define  CDNS_PCIE_LM_RC_BAR_CFG_BAR1_APERTURE(a) \
	(((a) << 9) & CDNS_PCIE_LM_RC_BAR_CFG_BAR1_APERTURE_MASK)
/* [한국어] RC BAR1 의 종류 자리, 비트 14~16. */
#define  CDNS_PCIE_LM_RC_BAR_CFG_BAR1_CTRL_MASK		GENMASK(16, 14)
/* [한국어] 종류 값 c 를 BAR1 자리에 넣는다. */
#define  CDNS_PCIE_LM_RC_BAR_CFG_BAR1_CTRL(c) \
	(((c) << 14) & CDNS_PCIE_LM_RC_BAR_CFG_BAR1_CTRL_MASK)
/* [한국어] 프리페치 가능 메모리 창을 켜는 비트. 이것이 꺼져 있으면 아래의
 * 32/64비트 선택은 의미가 없다. */
#define  CDNS_PCIE_LM_RC_BAR_CFG_PREFETCH_MEM_ENABLE	BIT(17)
/* [한국어] 프리페치 창을 32비트로 쓰겠다는 표시. 값이 0 이므로 아무것도
 * 세우지 않는 것이 곧 32비트 선택이다 -- 기본값을 0 으로 잡은 설계다. */
#define  CDNS_PCIE_LM_RC_BAR_CFG_PREFETCH_MEM_32BITS	0
/* [한국어] 프리페치 창을 64비트로 쓰겠다는 표시(비트 18). */
#define  CDNS_PCIE_LM_RC_BAR_CFG_PREFETCH_MEM_64BITS	BIT(18)
/* [한국어] IO 창을 켜는 비트. */
#define  CDNS_PCIE_LM_RC_BAR_CFG_IO_ENABLE		BIT(19)
/* [한국어] IO 창을 16비트 주소로 쓰겠다는 표시. 위와 같은 이유로 값이 0 이다. */
#define  CDNS_PCIE_LM_RC_BAR_CFG_IO_16BITS		0
/* [한국어] IO 창을 32비트 주소로 쓰겠다는 표시(비트 20). */
#define  CDNS_PCIE_LM_RC_BAR_CFG_IO_32BITS		BIT(20)
/* [한국어] 인바운드 주소가 BAR 범위 안인지 하드웨어가 검사하게 하는 비트.
 * 켜 두면 범위를 벗어난 접근이 조용히 엉뚱한 곳에 닿는 대신 걸러진다. */
#define  CDNS_PCIE_LM_RC_BAR_CFG_CHECK_ENABLE		BIT(31)

/* BAR control values applicable to both Endpoint Function and Root Complex */
/* [한국어] BAR 를 끈 상태. 아래 값들은 엔드포인트 BAR 설정과 RC BAR 설정
 * 양쪽에서 같은 의미로 쓰인다. */
#define  CDNS_PCIE_LM_BAR_CFG_CTRL_DISABLED		0x0
/* [한국어] 32비트 IO 공간 BAR. */
#define  CDNS_PCIE_LM_BAR_CFG_CTRL_IO_32BITS		0x1
/* [한국어] 32비트 메모리 BAR (프리페치 불가). */
#define  CDNS_PCIE_LM_BAR_CFG_CTRL_MEM_32BITS		0x4
/* [한국어] 32비트 프리페치 가능 메모리 BAR. 0x4 에 비트 0 이 더해진 값이다. */
#define  CDNS_PCIE_LM_BAR_CFG_CTRL_PREFETCH_MEM_32BITS	0x5
/* [한국어] 64비트 메모리 BAR (프리페치 불가). */
#define  CDNS_PCIE_LM_BAR_CFG_CTRL_MEM_64BITS		0x6
/* [한국어] 64비트 프리페치 가능 메모리 BAR. 0x6 에 비트 0 이 더해진 값이다.
 * 
...
/* [한국어] RC BAR 설정 레지스터 안에서 bar 번호에 맞는 자리로 '끔' 값을 민다.
...
/* [한국어] 크기 지수를 BAR 자리에 넣는다. aperture 에서 2 를 빼는 것은
...
/* [한국어] PTM(Precision Time Measurement) 제어 레지스터. 호스트와 장치가
 * 클록을 나노초 단위로 맞추는 PCIe 기능이다. */
#define CDNS_PCIE_LM_PTM_CTRL		(CDNS_PCIE_LM_BASE + 0x0DA8)
/* [한국어] PTM Responder 를 켜는 비트. 매크로 이름의 TPM 은 상류의 오타로
 ...
 */
/* [한국어] 엔드포인트 함수 fn 의 PCI 설정공간 창 시작 오프셋. 함수당 4KiB
 * (1 << 12)씩 떨어지고, GENMASK(19,12) 로 잘라 최대 256개 함수까지만 표현한다. */
#define CDNS_PCIE_EP_FUNC_BASE(fn)	(((fn) << 12) & GENMASK(19, 12))

/* [한국어] 그 창 안에서 MSI 능력 구조가 있는 오프셋. Cadence IP 가 합성 시
 * 이 자리에 고정해 둔 값이라 탐색 없이 바로 접근한다. */
#define CDNS_PCIE_EP_FUNC_MSI_CAP_OFFSET	0x90
/* [한국어] 같은 창 안의 MSI-X 능력 구조 오프셋. */
#define CDNS_PCIE_EP_FUNC_MSIX_CAP_OFFSET	0xB0
/* [한국어] 같은 창 안의 Device Capability(PCI Express 능력) 오프셋. */
#define CDNS_PCIE_EP_FUNC_DEV_CAP_OFFSET	0xC0
/* [한국어] 같은 창 안의 SR-IOV 확장 능력 오프셋. 0x100 이상이므로 확장
 * 설정공간(ECS) 영역이다. */
#define CDNS_PCIE_EP_FUNC_SRIOV_CAP_OFFSET	0x200

/* Endpoint PF Registers */
/* [한국어] 물리 함수 fn 의 ARI(Alternative Routing-ID) 능력·제어 레지스터.
 * 함수마다 4KiB(0x1000)씩 떨어진다. ARI 는 장치 번호 자리까지 함수 번호로
 * 써서 8개 제한을 256개로 넓히는 기능이다. */
#define CDNS_PCIE_CORE_PF_I_ARI_CAP_AND_CTRL(fn)	(0x144 + (fn) * 0x1000)
/* [한국어] 그 레지스터에서 '다음 함수 번호'(Next Function Number) 필드.
 * 함수들을 사슬로 이어 열거하게 하는 값이다. */
#define CDNS_PCIE_ARI_CAP_NFN_MASK			GENMASK(15, 8)

/* Root Port Registers (PCI configuration space for the root port function) */
/* [한국어] 루트 포트 함수의 PCI 설정공간 창. LM 블록과 달리 이쪽은 호스트
 * 소프트웨어가 보는 표준 설정공간 구조 그대로다. */
#define CDNS_PCIE_RP_BASE	0x00200000
/* [한국어] 그 안에서 PCI Express 능력 구조의 오프셋. 링크 상태/제어를
 * 읽을 때 이 값을 기준으로 삼는다. */
#define CDNS_PCIE_RP_CAP_OFFSET 0xC0

/* Address Translation Registers */
/* [한국어] Address Translation 블록의 기준 오프셋. AXI(CPU 쪽)와 PCIe 쪽
 * 주소를 서로 옮기는 창들이 여기 모여 있다. DesignWare 의 iATU 에 해당하는
 * Cadence 쪽 구조다. */
#define CDNS_PCIE_AT_BASE	0x00400000

/* Region r Outbound AXI to PCIe Address Translation Register 0 */
/* [한국어] 아웃바운드 창 r 의 PCI 쪽 주소 레지스터 0. 창마다 0x20 바이트씩
 * 떨어지고, (r) & 0x1F 로 잘라 최대 32개(CDNS_PCIE_MAX_OB)까지만 접근한다. */
#define CDNS_PCIE_AT_OB_REGION_PCI_ADDR0(r) \
	(CDNS_PCIE_AT_BASE + 0x0000 + ((r) & 0x1F) * 0x0020)
/* [한국어] 이 창이 덮는 주소 폭을 2의 지수로 담는 자리(하위 6비트). */
#define  CDNS_PCIE_AT_OB_REGION_PCI_ADDR0_NBITS_MASK	GENMASK(5, 0)
/* [한국어] nbits 에서 1 을 빼서 넣는다. 레지스터가 0 을 '1비트' 로 세므로
 * 지수를 그대로 넣으면 창 크기가 두 배가 된다. */
#define  CDNS_PCIE_AT_OB_REGION_PCI_ADDR0_NBITS(nbits) \
	(((nbits) - 1) & CDNS_PCIE_AT_OB_REGION_PCI_ADDR0_NBITS_MASK)
/* [한국어] 설정 트랜잭션을 만들 때 목적지 devfn 을 담는 자리(비트 12~19). */
#define  CDNS_PCIE_AT_OB_REGION_PCI_ADDR0_DEVFN_MASK	GENMASK(19, 12)
/* [한국어] devfn 을 그 자리로 옮겨 담는다. */
#define  CDNS_PCIE_AT_OB_REGION_PCI_ADDR0_DEVFN(devfn) \
	(((devfn) << 12) & CDNS_PCIE_AT_OB_REGION_PCI_ADDR0_DEVFN_MASK)
/* [한국어] 목적지 버스 번호를 담는 자리(비트 20~27). */
#define  CDNS_PCIE_AT_OB_REGION_PCI_ADDR0_BUS_MASK	GENMASK(27, 20)
/* [한국어] 버스 번호를 그 자리로 옮겨 담는다. */
#define  CDNS_PCIE_AT_OB_REGION_PCI_ADDR0_BUS(bus) \
	(((bus) << 20) & CDNS_PCIE_AT_OB_REGION_PCI_ADDR0_BUS_MASK)

/* Region r Outbound AXI to PCIe Address Translation Register 1 */
/* [한국어] 아웃바운드 창 r 의 PCI 쪽 주소 상위 32비트. ADDR0 과 짝을 이뤄
 * 64비트 주소를 만든다. */
#define CDNS_PCIE_AT_OB_REGION_PCI_ADDR1(r) \
	(CDNS_PCIE_AT_BASE + 0x0004 + ((r) & 0x1F) * 0x0020)

/* Region r Outbound PCIe Descriptor Register 0 */
/* [한국어] 아웃바운드 창 r 의 서술자 레지스터 0. '이 창의 쓰기를 어떤 종류의
 * TLP 로 바꿀 것인가' 를 정하는 자리다. */
#define CDNS_PCIE_AT_OB_REGION_DESC0(r) \
	(CDNS_PCIE_AT_BASE + 0x0008 + ((r) & 0x1F) * 0x0020)
/* [한국어] TLP 종류를 담는 하위 4비트. */
#define  CDNS_PCIE_AT_OB_REGION_DESC0_TYPE_MASK		GENMASK(3, 0)
/* [한국어] 메모리 트랜잭션. */
#define  CDNS_PCIE_AT_OB_REGION_DESC0_TYPE_MEM		0x2
/* [한국어] IO 트랜잭션. */
#define  CDNS_PCIE_AT_OB_REGION_DESC0_TYPE_IO		0x6
/* [한국어] Type 0 설정 트랜잭션 -- 링크 바로 건너편 장치를 겨눌 때. */
#define  CDNS_PCIE_AT_OB_REGION_DESC0_TYPE_CONF_TYPE0	0xA
/* [한국어] Type 1 설정 트랜잭션 -- 스위치를 거쳐 더 깊은 버스를 겨눌 때. */
#define  CDNS_PCIE_AT_OB_REGION_DESC0_TYPE_CONF_TYPE1	0xB
/* [한국어] 일반 메시지 TLP. PME_Turn_Off 같은 규약 메시지를 보낼 때 쓴다. */
#define  CDNS_PCIE_AT_OB_REGION_DESC0_TYPE_NORMAL_MSG	0xC
/* [한국어] 벤더 정의 메시지 TLP. 규약이 정하지 않은 신호를 벤더가 임의로
 * 실어 보낼 때 쓴다. */
#define  CDNS_PCIE_AT_OB_REGION_DESC0_TYPE_VENDOR_MSG	0xD
/* Bit 23 MUST be set in RC mode. */
/* [한국어] 이 창이 만드는 TLP 의 요청자 ID 를 소프트웨어가 정한 값으로
 * 고정하라는 비트. 상류 주석이 못 박아 두었듯 **RC 모드에서는 반드시 세워야
 * 한다** -- 루트 포트가 자기 RID 로 요청을 내보내야 완료 패킷이 돌아올 길을
 * 찾기 때문이다. */
#define  CDNS_PCIE_AT_OB_REGION_DESC0_HARDCODED_RID	BIT(23)
/* [한국어] 그 고정 RID 의 devfn 부분(비트 24~31). */
...
/* [한국어] 아웃바운드 창 r 의 서술자 레지스터 1. DESC0 이 담지 못한 버스
 * 번호가 여기 들어간다. */
#define CDNS_PCIE_AT_OB_REGION_DESC1(r)	\
	(CDNS_PCIE_AT_BASE + 0x000C + ((r) & 0x1F) * 0x0020)
/* [한국어] 고정 RID 의 버스 번호 부분(하위 8비트). */
#define  CDNS_PCIE_AT_OB_REGION_DESC1_BUS_MASK	GENMASK(7, 0)
/* [한국어] 버스 번호를 그대로 담는다. 시프트가 없는 것은 필드가 비트 0 에서
 * 시작하기 때문이다. */
#define  CDNS_PCIE_AT_OB_REGION_DESC1_BUS(bus) \
	((bus) & CDNS_PCIE_AT_OB_REGION_DESC1_BUS_MASK)

/* Region r AXI Region Base Address Register 0 */
/* [한국어] 아웃바운드 창 r 의 AXI(CPU) 쪽 기준 주소 레지스터 0. PCI_ADDR 쪽과
 * 짝을 이뤄 '이 CPU 주소 범위를 저 PCI 주소로 옮긴다' 는 대응을 완성한다.
 * 오프셋 0x18 로 같은 0x20 바이트 창 블록 안에 있다. */
#define CDNS_PCIE_AT_OB_REGION_CPU_ADDR0(r) \
	(CDNS_PCIE_AT_BASE + 0x0018 + ((r) & 0x1F) * 0x0020)
/* [한국어] CPU 쪽 창이 덮는 주소 폭을 담는 자리(하위 6비트). */
#define  CDNS_PCIE_AT_OB_REGION_CPU_ADDR0_NBITS_MASK	GENMASK(5, 0)
/* [한국어] PCI 쪽과 같은 이유로 nbits 에서 1 을 빼서 넣는다. */
#define  CDNS_PCIE_AT_OB_REGION_CPU_ADDR0_NBITS(nbits) \
	(((nbits) - 1) & CDNS_PCIE_AT_OB_REGION_CPU_ADDR0_NBITS_MASK)

/* Region r AXI Region Base Address Register 1 */
/* [한국어] AXI 쪽 기준 주소의 상위 32비트. 64비트 물리 주소를 쓰는 SoC 에서
 * 필요하다. */
...
/* [한국어] 아웃바운드와 같은 규칙으로 nbits 에서 1 을 뺀다. */
...
/* [한국어] AXI 링크 다운 레지스터. 링크가 끊긴 동안 AXI 쪽 접근을 어떻게
 * 처리할지를 다룬다 -- 응답 없이 멈추는 대신 오류로 끝내 시스템 정지를 막는다. */
#define CDNS_PCIE_AT_LINKDOWN (CDNS_PCIE_AT_BASE + 0x0824)

/* LTSSM Capabilities register */
/* [한국어] LTSSM(Link Training and Status State Machine) 제어·능력 레지스터.
 * 주소가 AT 가 아니라 LM 기준인 점에 유의 -- 링크 학습은 로컬 관리 영역이다. */
#define CDNS_PCIE_LTSSM_CONTROL_CAP		(CDNS_PCIE_LM_BASE + 0x0054)
/* [한국어] Detect.Quiet 상태의 최소 체류 시간을 담는 자리(비트 1~2). */
#define  CDNS_PCIE_DETECT_QUIET_MIN_DELAY_MASK	GENMASK(2, 1)
/* [한국어] 그 필드의 시작 비트(1). */
#define  CDNS_PCIE_DETECT_QUIET_MIN_DELAY_SHIFT 1
/* [한국어] delay 값을 그 자리로 옮겨 담는다. 이 값을 늘리면 링크 학습이
...
/* [한국어] 루트 포트가 쓸 수 있는 인바운드 창의 최대 번호(3). BAR 개수에
 * 대응한다. */
#define CDNS_PCIE_RP_MAX_IB	0x3
/* [한국어] 아웃바운드 창의 최대 개수(32). 위의 (r) & 0x1F 마스크와 짝이다. */
#define CDNS_PCIE_MAX_OB	32

/* Endpoint Function BAR Inbound PCIe to AXI Address Translation Register */
/* [한국어] 엔드포인트 함수 fn 의 BAR bar 에 대한 인바운드 주소 변환 레지스터 0.
...
/* [한국어] 메시지 TLP 의 라우팅 필드(비트 5~7). 이 값들은 레지스터가 아니라
 * **아웃바운드 창 안의 오프셋**에 실린다 -- 상류 주석대로, 메시지를 보내는
 * 방법이 '메시지용으로 설정한 창의 특정 오프셋에 쓰기' 이기 때문이다.
 * 그래서 라우팅과 코드가 주소 비트로 인코딩된다. */
#define CDNS_PCIE_NORMAL_MSG_ROUTING_MASK	GENMASK(7, 5)
/* [한국어] 라우팅 값을 그 자리로 옮겨 담는다. 라우팅은 메시지를 어디까지
 * 전파할지(수신자 지정, 브로드캐스트, 로컬 등)를 정한다. */
#define CDNS_PCIE_NORMAL_MSG_ROUTING(route) \
	(((route) << 5) & CDNS_PCIE_NORMAL_MSG_ROUTING_MASK)
/* [한국어] 메시지 코드 필드(비트 8~15). PME_Turn_Off 같은 메시지 종류를 담는다. */
#define CDNS_PCIE_NORMAL_MSG_CODE_MASK		GENMASK(15, 8)
/* [한국어] 코드 값을 그 자리로 옮겨 담는다. */
#define CDNS_PCIE_NORMAL_MSG_CODE(code) \
	(((code) << 8) & CDNS_PCIE_NORMAL_MSG_CODE_MASK)
/* [한국어] 데이터 없는 메시지라는 표시(비트 16). PME_Turn_Off 처럼 신호만
