...
/* [한국어] [한국어] 일반 커널 매크로용. 이 파일 자체는 특별히 쓰는 것이 없지만
 * Cadence 가 헤더 묶음을 관례적으로 포함해 두었다. */
#include <linux/kernel.h>
/* [한국어] [한국어] PCI 공통 정의용. 아래 BAR 번호 비교(BAR_3)가 여기 계통의
 * 열거값에 의존한다. */
#include <linux/pci.h>
/* [한국어] [한국어] enum pci_barno(BAR_0..BAR_5)를 제공한다. BAR_3 비교가 이것을 쓴다. */
#include <linux/pci-epf.h>
/* [한국어] [한국어] PHY 프레임워크 헤더. 다만 이 파일 안의 어떤 정의도 phy 타입을
 * 쓰지 않는다 -- Cadence 헤더 묶음을 그대로 가져온 흔적으로 보인다. */
#include <linux/phy/phy.h>
/* [한국어] [한국어] GENMASK/BIT/FIELD_PREP 를 쓰기 위해 포함한다. HPA 는 LGA 와 달리
 * 필드 채우기를 FIELD_PREP 으로 처리하는 곳이 많아 이 헤더가 특히 중요하다. */
#include <linux/bitfield.h>

/* High Performance Architecture (HPA) PCIe controller registers */
/* [한국어] [한국어] IP 레지스터 뱅크의 기준 주소 후보. **이 트리에서 참조하는 곳이
...
/* [한국어] [한국어] 엔드포인트 함수 fn 의 BAR b 설정 레지스터를 고른다. **경계가
...
/* [한국어] [한국어] BAR 0~2 를 담는 레지스터. 물리 함수마다 0x4000 씩 떨어진다. */
#define CDNS_PCIE_HPA_LM_EP_FUNC_BAR_CFG0(pfn) (0x4000 * (pfn))
/* [한국어] [한국어] BAR 3~5 를 담는 레지스터. CFG0 바로 다음 4바이트. */
#define CDNS_PCIE_HPA_LM_EP_FUNC_BAR_CFG1(pfn) ((0x4000 * (pfn)) + 0x04)
/* [한국어] [한국어] SR-IOV 가상 함수용 같은 구조. 역시 BAR_3 을 경계로 나뉜다. */
...
/* [한국어] [한국어] VF 의 BAR 0~2 설정. 같은 0x4000 블록 안에서 0x08 만큼 뒤에 있다. */
#define CDNS_PCIE_HPA_LM_EP_VFUNC_BAR_CFG0(vfn) ((0x4000 * (vfn)) + 0x08)
/* [한국어] [한국어] VF 의 BAR 3~5 설정. */
#define CDNS_PCIE_HPA_LM_EP_VFUNC_BAR_CFG1(vfn) ((0x4000 * (vfn)) + 0x0C)
/* [한국어] [한국어] 레지스터 안에서 BAR f 의 aperture(크기 지수) 자리. BAR 하나가
...
/* [한국어] [한국어] 크기 지수 a 를 BAR b 의 자리로 옮겨 담는다. 시프트 4 + b * 10 이
...
/* [한국어] [한국어] RC BAR 설정 레지스터에서 bar 번호에 맞는 슬롯으로 '끔' 값을 민다.
...
/* [한국어] [한국어] 크기 지수를 BAR 슬롯의 aperture 자리에 넣는다. **빼는 값이 7 인 점이
...
/* [한국어] [한국어] PTM(Precision Time Measurement) 제어 레지스터. 뱅크 상대 0x0520. */
#define CDNS_PCIE_HPA_LM_PTM_CTRL		0x0520
/* [한국어] [한국어] PTM Responder 를 켜는 비트. LGA 와 같은 BIT(17) 이지만 레지스터
...
/* [한국어] [한국어] 루트 포트 설정공간에서 PCI Express 능력 구조의 오프셋.
 * 이 상수를 참조하는 곳은 이 트리에 없다. */
#define CDNS_PCIE_HPA_RP_CAP_OFFSET	0xC0

/* Region r Outbound AXI to PCIe Address Translation Register 0 */
/* [한국어] [한국어] 아웃바운드 창 r 의 PCI 쪽 주소 레지스터 0. **창 하나가 0x80 바이트**
...
/* [한국어] [한국어] TLP 종류 자리(비트 24~28). LGA 는 하위 4비트였는데 HPA 는 상위로
 * 옮기고 폭도 5비트로 늘렸다. */
#define CDNS_PCIE_HPA_AT_OB_REGION_DESC0_TYPE_MASK         GENMASK(28, 24)
/* [한국어] [한국어] 메모리 트랜잭션. 종류 코드 0x0 -- LGA 의 0x2 와 값 자체가 다르다.
...
/* Region r Outbound PCIe Descriptor Register */
/* [한국어] [한국어] 아웃바운드 창 r 의 서술자 레지스터 1. 고정 RID 의 버스/devfn 이
 * 여기 담긴다. */
#define CDNS_PCIE_HPA_AT_OB_REGION_DESC1(r)        (0x100C + ((r) & 0x1F) * 0x0080)
/* [한국어] [한국어] 고정 RID 의 버스 번호 자리(비트 24~31). */
...
/* [한국어] [한국어] 고정 RID 의 devfn 자리(비트 16~23). PCI_ADDR0 쪽과 같은 자릿수라,
...
/* [한국어] [한국어] 같은 규칙으로 nbits 에서 1 을 뺀다. */
...
/* [한국어] [한국어] AXI 링크 다운 레지스터. 링크가 끊긴 동안의 AXI 접근 처리를 다룬다.
...
/* [한국어] [한국어] 확장 ROM 기준 주소 레지스터의 오프셋. 위 I_PCIE_BUS_NUMBERS 와
