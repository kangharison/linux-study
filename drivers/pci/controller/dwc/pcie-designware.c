 ...
 * NVMe 는 이 IP 가 세운 버스 위에서 열거되는 여러 장치 중 하나일 뿐이고,
 * 이 파일에는 NVMe 를 특별히 다루는 코드가 한 줄도 없다.
 *
 * 다만 이 파일이 정하는 두 가지가 그 버스에 붙은 모든 장치의 성능 상한을
 * 결정하므로, NVMe 처럼 대역폭에 민감한 장치를 볼 때 확인할 지점이 된다:
 *   - 링크 속도: dw_pcie_link_set_max_speed() 가 DT 의 max-link-speed 를
 *     LNKCAP 의 지원 속도 필드와 PCIE_LINK_WIDTH_SPEED_CONTROL 에 반영한다.
 *   - 링크 폭: dw_pcie_link_get_max_link_width() 가 하드웨어가 낼 수 있는
 *     최대 레인 수를 읽고, dw_pcie_link_set_max_link_width() 가 DT 의
 *     num-lanes 로 그것을 제한한다.
 * (앞선 판의 이 절은 이 역할을 dw_pcie_link_up() 이 한다고 적었으나,
 *  그 함수는 링크가 올라왔는지만 판정한다. 위 두 계열이 실제 담당이다.)
 */
