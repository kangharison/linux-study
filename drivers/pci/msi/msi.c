 ...
 **/
static int pci_msi_supported(struct pci_dev *dev, int nvec) /* NVMe: pci_msi_supported 함수 선언. NVMe PCIe 장치의 MSI/MSI-X 동작과 관련. */
{ /* NVMe: pci_msi_supported 함수 본문 시작. */
 ...
 *   할당된 IRQ 벡터를 반납한다.
 */
static void pcim_msi_release(void *pcidev) /* NVMe: pcim_msi_release 함수 선언. NVMe PCIe 장치의 MSI/MSI-X 동작과 관련. */
{ /* NVMe: pcim_msi_release 함수 본문 시작. */
 ...
 */
static int pcim_setup_msi_release(struct pci_dev *dev) /* NVMe: pcim_setup_msi_release 함수 선언. NVMe PCIe 장치의 MSI/MSI-X 동작과 관련. */
{ /* NVMe: pcim_setup_msi_release 함수 본문 시작. */
...
} /* NVMe: pcim_setup_msi_release 함수 종료. */
 ...
 */
static int pci_setup_msi_context(struct pci_dev *dev) /* NVMe: pci_setup_msi_context 함수 선언. NVMe PCIe 장치의 MSI/MSI-X 동작과 관련. */
{ /* NVMe: pci_setup_msi_context 함수 본문 시작. */
...
} /* NVMe: pci_setup_msi_context 함수 종료. */
 ...
 */
void pci_msi_update_mask(struct msi_desc *desc, u32 clear, u32 set) /* NVMe: pci_msi_update_mask 함수 선언. NVMe PCIe 장치의 MSI/MSI-X 동작과 관련. */
{ /* NVMe: pci_msi_update_mask 함수 본문 시작. */
...
} /* NVMe: pci_msi_update_mask 함수 종료. */
 ...
 * 하위 레벨에서 사용될 수 있다.
 */
void pci_msi_mask_irq(struct irq_data *data) /* NVMe: pci_msi_mask_irq 함수 선언. NVMe PCIe 장치의 MSI/MSI-X 동작과 관련. */
{ /* NVMe: pci_msi_mask_irq 함수 본문 시작. */
...
} /* NVMe: pci_msi_mask_irq 함수 종료. */
EXPORT_SYMBOL_GPL(pci_msi_mask_irq); /* NVMe: pci_msi_mask_irq 심볼을 외부 모듈에서 사용할 수 있도록 EXPORT. */
 ...
 */
void pci_msi_unmask_irq(struct irq_data *data) /* NVMe: pci_msi_unmask_irq 함수 선언. NVMe PCIe 장치의 MSI/MSI-X 동작과 관련. */
{ /* NVMe: pci_msi_unmask_irq 함수 본문 시작. */
...
} /* NVMe: pci_msi_unmask_irq 함수 종료. */
EXPORT_SYMBOL_GPL(pci_msi_unmask_irq); /* NVMe: pci_msi_unmask_irq 심볼을 외부 모듈에서 사용할 수 있도록 EXPORT. */
 ...
 */
void __pci_read_msi_msg(struct msi_desc *entry, struct msi_msg *msg) /* NVMe: __pci_read_msi_msg 함수 선언. NVMe PCIe 장치의 MSI/MSI-X 동작과 관련. */
{ /* NVMe: __pci_read_msi_msg 함수 본문 시작. */
	...
	if (entry->pci.msi_attrib.is_msix) { /* NVMe: MSI-X 벡터인 경우. */
		...
		pci_read_config_dword(dev, pos + PCI_MSI_ADDRESS_LO, /* NVMe: PCI config dword 읽기 호출의 연속 줄. */
				      &msg->address_lo); /* NVMe: config space에서 MSI address 하위 32bit 읽기. */
		if (entry->pci.msi_attrib.is_64) { /* NVMe: 64bit MSI capable 장치인 경우. */
			pci_read_config_dword(dev, pos + PCI_MSI_ADDRESS_HI, /* NVMe: PCI config dword 읽기 호출의 연속 줄. */
		...
		} /* NVMe: 조걘문/루프/낮은 수준 블록 종료. */
		msg->data = data; /* NVMe: 읽은 16bit data를 msg->data에 저장. */
	} /* NVMe: 조걘문/루프/낮은 수준 블록 종료. */
} /* NVMe: __pci_read_msi_msg 함수 종료. */
 ...
 */
static inline void pci_write_msg_msi(struct pci_dev *dev, struct msi_desc *desc, /* NVMe: pci_write_msg_msi 함수 선언. NVMe PCIe 장치의 MSI/MSI-X 동작과 관련. */
				     struct msi_msg *msg) /* NVMe: 함수 매개변수 선언의 연속 줄. */
{ /* NVMe: pci_write_msg_msi 함수 본문 시작. */
 ...
 */
static inline void pci_write_msg_msix(struct msi_desc *desc, struct msi_msg *msg) /* NVMe: pci_write_msg_msix 함수 선언. NVMe PCIe 장치의 MSI/MSI-X 동작과 관련. */
{ /* NVMe: pci_write_msg_msix 함수 본문 시작. */
 ...
 */
void __pci_write_msi_msg(struct msi_desc *entry, struct msi_msg *msg) /* NVMe: __pci_write_msi_msg 함수 선언. NVMe PCIe 장치의 MSI/MSI-X 동작과 관련. */
{ /* NVMe: __pci_write_msi_msg 함수 본문 시작. */
 ...
 *   변경할 때 사용되는 외부 인터페이스.
 */
void pci_write_msi_msg(unsigned int irq, struct msi_msg *msg) /* NVMe: pci_write_msi_msg 함수 선언. NVMe PCIe 장치의 MSI/MSI-X 동작과 관련. */
{ /* NVMe: pci_write_msi_msg 함수 본문 시작. */
	struct msi_desc *entry = irq_get_msi_desc(irq); /* NVMe: virq로부터 MSI 디스크립터 검색. */

	__pci_write_msi_msg(entry, msg); /* NVMe: 실제 message 쓰기 수행. */
} /* NVMe: 조걘문/루프/낮은 수준 블록 종료. */
EXPORT_SYMBOL_GPL(pci_write_msi_msg); /* NVMe: pci_write_msi_msg 심볼을 외부 모듈에서 사용할 수 있도록 EXPORT. */
 ...
 */
static void pci_intx_for_msi(struct pci_dev *dev, int enable) /* NVMe: pci_intx_for_msi 함수 선언. NVMe PCIe 장치의 MSI/MSI-X 동작과 관련. */
{ /* NVMe: pci_intx_for_msi 함수 본문 시작. */
 ...
 */
static void pci_msi_set_enable(struct pci_dev *dev, int enable) /* NVMe: pci_msi_set_enable 함수 선언. NVMe PCIe 장치의 MSI/MSI-X 동작과 관련. */
{ /* NVMe: pci_msi_set_enable 함수 본문 시작. */
 ...
 *   fallback할 수 있다.
 */
static int msi_setup_msi_desc(struct pci_dev *dev, int nvec, /* NVMe: msi_setup_msi_desc 함수 선언. NVMe PCIe 장치의 MSI/MSI-X 동작과 관련. */
			      struct irq_affinity_desc *masks) /* NVMe: 함수 매개변수 선언의 연속 줄. */
{ /* NVMe: msi_setup_msi_desc 함수 본문 시작. */
 ...
 */
static int msi_verify_entries(struct pci_dev *dev) /* NVMe: msi_verify_entries 함수 선언. NVMe PCIe 장치의 MSI/MSI-X 동작과 관련. */
{ /* NVMe: msi_verify_entries 함수 본문 시작. */
 ...
 */
static int __msi_capability_init(struct pci_dev *dev, int nvec, struct irq_affinity_desc *masks) /* NVMe: __msi_capability_init 함수 선언. NVMe PCIe 장치의 MSI/MSI-X 동작과 관련. */
{ /* NVMe: __msi_capability_init 함수 본문 시작. */
...
err: /* NVMe: __msi_capability_init 오류 처리 레이블. */
 ...
 */
static int msi_capability_init(struct pci_dev *dev, int nvec, /* NVMe: msi_capability_init 함수 선언. NVMe PCIe 장치의 MSI/MSI-X 동작과 관련. */
			       struct irq_affinity *affd) /* NVMe: 함수 매개변수 선언의 연속 줄. */
{ /* NVMe: msi_capability_init 함수 본문 시작. */
	...
	struct irq_affinity_desc *masks __free(kfree) = /* NVMe: affinity 마스크 배열을 스코프드 할당할 변수 선언(할당은 다음 줄). */
 ...
 *   점진적으로 개수를 조정하며 할당한다.
 */
int __pci_enable_msi_range(struct pci_dev *dev, int minvec, int maxvec, /* NVMe: __pci_enable_msi_range 함수 선언. NVMe PCIe 장치의 MSI/MSI-X 동작과 관련. */
			   struct irq_affinity *affd) /* NVMe: 함수 매개변수 선언의 연속 줄. */
{ /* NVMe: __pci_enable_msi_range 함수 본문 시작. */
 ...
 **/
int pci_msi_vec_count(struct pci_dev *dev) /* NVMe: pci_msi_vec_count 함수 선언. NVMe PCIe 장치의 MSI/MSI-X 동작과 관련. */
{ /* NVMe: pci_msi_vec_count 함수 본문 시작. */
...
EXPORT_SYMBOL(pci_msi_vec_count); /* NVMe: pci_msi_vec_count 심볼을 외부 모듈에서 사용할 수 있도록 EXPORT. */
 ...
 * 다시 쓰도록 한다.
 */
bool __weak arch_restore_msi_irqs(struct pci_dev *dev) /* NVMe: arch_restore_msi_irqs 함수 선언. NVMe PCIe 장치의 MSI/MSI-X 동작과 관련. */
{ /* NVMe: arch_restore_msi_irqs 함수 본문 시작. */
 ...
 */
void __pci_restore_msi_state(struct pci_dev *dev) /* NVMe: __pci_restore_msi_state 함수 선언. NVMe PCIe 장치의 MSI/MSI-X 동작과 관련. */
{ /* NVMe: __pci_restore_msi_state 함수 본문 시작. */
	...
	control |= PCI_MSI_FLAGS_ENABLE | /* NVMe: MSI Enable 비트와 multiple message 개수를 함께 설정하는 연속 줄. */
 ...
 */
void pci_msi_shutdown(struct pci_dev *dev) /* NVMe: pci_msi_shutdown 함수 선언. NVMe PCIe 장치의 MSI/MSI-X 동작과 관련. */
{ /* NVMe: pci_msi_shutdown 함수 본문 시작. */
 ...
 *   플래그를 원자적으로 갱신할 때 사용.
 */
static void pci_msix_clear_and_set_ctrl(struct pci_dev *dev, u16 clear, u16 set) /* NVMe: pci_msix_clear_and_set_ctrl 함수 선언. NVMe PCIe 장치의 MSI/MSI-X 동작과 관련. */
{ /* NVMe: pci_msix_clear_and_set_ctrl 함수 본문 시작. */
 ...
 */
static void __iomem *msix_map_region(struct pci_dev *dev, /* NVMe: msix_map_region 함수 선언. NVMe PCIe 장치의 MSI/MSI-X 동작과 관련. */
				     unsigned int nr_entries) /* NVMe: 함수 매개변수 선언의 연속 줄. */
{ /* NVMe: msix_map_region 함수 본문 시작. */
	...
	pci_read_config_dword(dev, dev->msix_cap + PCI_MSIX_TABLE, /* NVMe: PCI config dword 읽기 호출의 연속 줄. */
 ...
 */
void msix_prepare_msi_desc(struct pci_dev *dev, struct msi_desc *desc) /* NVMe: msix_prepare_msi_desc 함수 선언. NVMe PCIe 장치의 MSI/MSI-X 동작과 관련. */
{ /* NVMe: msix_prepare_msi_desc 함수 본문 시작. */
	...
	if (!pci_msi_domain_supports(dev, MSI_FLAG_NO_MASK, DENY_LEGACY) && /* NVMe: 레거시 마스킹 지원 여부를 추가로 확인하는 조걘문 연속 줄. */
 ...
 */
static int msix_setup_msi_descs(struct pci_dev *dev, struct msix_entry *entries, /* NVMe: msix_setup_msi_descs 함수 선언. NVMe PCIe 장치의 MSI/MSI-X 동작과 관련. */
				int nvec, struct irq_affinity_desc *masks) /* NVMe: 함수 매개변수 선언의 연속 줄. */
{ /* NVMe: msix_setup_msi_descs 함수 본문 시작. */
 ...
 */
static void msix_update_entries(struct pci_dev *dev, struct msix_entry *entries) /* NVMe: msix_update_entries 함수 선언. NVMe PCIe 장치의 MSI/MSI-X 동작과 관련. */
{ /* NVMe: msix_update_entries 함수 본문 시작. */
 ...
 */
static void msix_mask_all(void __iomem *base, int tsize) /* NVMe: msix_mask_all 함수 선언. NVMe PCIe 장치의 MSI/MSI-X 동작과 관련. */
{ /* NVMe: msix_mask_all 함수 본문 시작. */
 ...
 */
static int __msix_setup_interrupts(struct pci_dev *__dev, struct msix_entry *entries, /* NVMe: __msix_setup_interrupts 함수 선언. NVMe PCIe 장치의 MSI/MSI-X 동작과 관련. */
				   int nvec, struct irq_affinity_desc *masks) /* NVMe: 함수 매개변수 선언의 연속 줄. */
{ /* NVMe: __msix_setup_interrupts 함수 본문 시작. */
 ...
 *   상태에서 실제 설정 수행.
 */
static int msix_setup_interrupts(struct pci_dev *dev, struct msix_entry *entries, /* NVMe: msix_setup_interrupts 함수 선언. NVMe PCIe 장치의 MSI/MSI-X 동작과 관련. */
				 int nvec, struct irq_affinity *affd) /* NVMe: 함수 매개변수 선언의 연속 줄. */
{ /* NVMe: msix_setup_interrupts 함수 본문 시작. */
	struct irq_affinity_desc *masks __free(kfree) = /* NVMe: affinity 마스크 배열을 스코프드 할당할 변수 선언(할당은 다음 줄). */
 ...
 **/
static int msix_capability_init(struct pci_dev *dev, struct msix_entry *entries, /* NVMe: msix_capability_init 함수 선언. NVMe PCIe 장치의 MSI/MSI-X 동작과 관련. */
				int nvec, struct irq_affinity *affd) /* NVMe: 함수 매개변수 선언의 연속 줄. */
{ /* NVMe: msix_capability_init 함수 본문 시작. */
	 ...
	 */
	pci_msix_clear_and_set_ctrl(dev, 0, PCI_MSIX_FLAGS_MASKALL | /* NVMe: MSI-X control 레지스터 갱신 호출의 연속 줄. */
	...
	if (!pci_msi_domain_supports(dev, MSI_FLAG_NO_MASK, DENY_LEGACY)) { /* NVMe: 레거시 마스킹 지원 시 모든 MSI-X entry를 초기에 마스크해야 하는지 검사. */
		/*
...
out_unmap: /* NVMe: MSI-X table 매핑 해제 경로. */
	iounmap(dev->msix_base); /* NVMe: 매핑한 MSI-X table 영역 해제. */
out_disable: /* NVMe: MSI-X capability 비활성화 경로. */
 ...
 */
static bool pci_msix_validate_entries(struct pci_dev *dev, struct msix_entry *entries, int nvec) /* NVMe: pci_msix_validate_entries 함수 선언. NVMe PCIe 장치의 MSI/MSI-X 동작과 관련. */
{ /* NVMe: pci_msix_validate_entries 함수 본문 시작. */
 ...
 */
int __pci_enable_msix_range(struct pci_dev *dev, struct msix_entry *entries, int minvec, /* NVMe: __pci_enable_msix_range 함수 선언. NVMe PCIe 장치의 MSI/MSI-X 동작과 관련. */
			    int maxvec, struct irq_affinity *affd, int flags) /* NVMe: 함수 매개변수 선언의 연속 줄. */
{ /* NVMe: __pci_enable_msix_range 함수 본문 시작. */
	...
	if (hwsize < nvec) { /* NVMe: 하드웨어 최대 entry 수가 요청보다 적으면. */
		...
		else /* NVMe: PCI_IRQ_VIRTUAL 플래그가 없으면 하드웨어 실제 최대값으로 제한. */
 ...
 *   다시 설정하여 인터럽트 경로를 복구.
 */
void __pci_restore_msix_state(struct pci_dev *dev) /* NVMe: __pci_restore_msix_state 함수 선언. NVMe PCIe 장치의 MSI/MSI-X 동작과 관련. */
{ /* NVMe: __pci_restore_msix_state 함수 본문 시작. */
	...
	pci_msix_clear_and_set_ctrl(dev, 0, /* NVMe: MSI-X control 레지스터 갱신 호출의 연속 줄. */
 ...
 */
void pci_msix_shutdown(struct pci_dev *dev) /* NVMe: pci_msix_shutdown 함수 선언. NVMe PCIe 장치의 MSI/MSI-X 동작과 관련. */
{ /* NVMe: pci_msix_shutdown 함수 본문 시작. */
 ...
 */
void pci_free_msi_irqs(struct pci_dev *dev) /* NVMe: pci_free_msi_irqs 함수 선언. NVMe PCIe 장치의 MSI/MSI-X 동작과 관련. */
{ /* NVMe: pci_free_msi_irqs 함수 본문 시작. */
...
#ifdef CONFIG_PCIE_TPH /* NVMe: 커널에 TPH 기능이 포함된 경우에만 관련 코드를 컴파일. */
/**
 ...
 */
int pci_msix_write_tph_tag(struct pci_dev *pdev, unsigned int index, u16 tag) /* NVMe: pci_msix_write_tph_tag 함수 선언. NVMe PCIe 장치의 MSI/MSI-X 동작과 관련. */
{ /* NVMe: pci_msix_write_tph_tag 함수 본문 시작. */
...
#endif /* NVMe: CONFIG_PCIE_TPH 조건부 컴파일 종료. */
 ...
 */
struct pci_dev *msi_desc_to_pci_dev(struct msi_desc *desc) /* NVMe: msi_desc_to_pci_dev 함수 선언. NVMe PCIe 장치의 MSI/MSI-X 동작과 관련. */
{ /* NVMe: msi_desc_to_pci_dev 함수 본문 시작. */
	return to_pci_dev(desc->dev); /* NVMe: msi_desc->dev를 pci_dev로 캐스팅하여 반환. */
} /* NVMe: 조걘문/루프/낮은 수준 블록 종료. */
EXPORT_SYMBOL(msi_desc_to_pci_dev); /* NVMe: msi_desc_to_pci_dev 심볼을 외부 모듈에서 사용할 수 있도록 EXPORT. */
 ...
 */
void pci_no_msi(void) /* NVMe: pci_no_msi 함수 선언. NVMe PCIe 장치의 MSI/MSI-X 동작과 관련. */
{ /* NVMe: pci_no_msi 함수 본문 시작. */
