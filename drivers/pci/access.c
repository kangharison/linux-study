...
#include <linux/pci.h> /* NVMe: PCIe 호스트 드라이버가 PCI 장치 구조체와 접근 함수를 사용하기 위해 포함 */
#include <linux/module.h> /* NVMe: 드라이버 모듈 심볼 및 EXPORT_SYMBOL 매크로 사용 */
#include <linux/slab.h> /* NVMe: 장치 초기화 시 동적 메모리 할당(kmalloc/kzalloc)용 */
#include <linux/ioport.h> /* NVMe: BAR(doorbell) 자원 요청 및 관리용 */
#include <linux/wait.h> /* NVMe: config 접근 차단 시 태스크 대기(wait_event)용 */

#include "pci.h" /* NVMe: PCI 서브시스템 내장 헤더 (NVMe 장치와 config 접근 공유) */
...
#ifdef CONFIG_PCI_LOCKLESS_CONFIG /* NVMe: lockless config 접근 최적화가 활성화된 경우 */
...
#else /* NVMe: lockless가 비활성화되면 락 기반 config 접근 사용 */
...
#endif /* NVMe: CONFIG_PCI_LOCKLESS_CONFIG 조건 컴파일 종료 */
 ...
 *   PCIe capability 등을 읽거나 쓸 때 pci_read_config_ 계열과 pci_write_config_ 계열 경로로
 *   이 함수들이 간접 호출된다. pci_lock으로 동시 접근을 직렬화한다.
 */
#define PCI_OP_READ(size, type, len)  /* NVMe: 크기별 PCI config read 함수 생성 매크로 정의 (NVMe config 읽기 기반) */ \
int noinline pci_bus_read_config_##size  /* NVMe: 버스 레벨 config read 함수 선언 (NVMe 장치 포함) */ \
	(struct pci_bus *bus, unsigned int devfn, int pos, type *value)	 /* NVMe: 장치를 식별하는 bus/devfn과 config offset/값 포인터 */ \
{									 /* NVMe: PCIe config read 매크로 본문 시작 */ \
	unsigned long flags; /* NVMe: 인터럽트 상태 저장용 플래그 */						\
	u32 data = 0; /* NVMe: config read로 받은 원시 32비트 데이터 */						\
	int res; /* NVMe: 하위 버스 read 결과 코드 */								\
												 /* NVMe: 매크로 정의 내 공백 연속 라인 */ \
...
#define PCI_OP_WRITE(size, type, len)  /* NVMe: 크기별 PCI config write 함수 생성 매크로 정의 (NVMe config 쓰기 기반) */ \
int noinline pci_bus_write_config_##size  /* NVMe: 버스 레벨 config write 함수 선언 (NVMe 장치 포함) */ \
	(struct pci_bus *bus, unsigned int devfn, int pos, type value)	 /* NVMe: 장치를 식별하는 bus/devfn과 config offset/값 */ \
{										 /* NVMe: PCIe config write 매크로 본문 시작 */ \
 ...
 */
int pci_generic_config_read(struct pci_bus *bus, unsigned int devfn, /* NVMe: 메모리 맵된 config space에서 NVMe 장치 등을 읽는 generic read 진입점 */
			    int where, int size, u32 *val) /* NVMe: 장치 config 접근 함수 매개변수 선언 */
{ /* NVMe: 함수/매크로 본문 시작 */
 ...
 */
int pci_generic_config_write(struct pci_bus *bus, unsigned int devfn, /* NVMe: 메모리 맵된 config space에 NVMe command/BAR 등을 쓰는 generic write 진입점 */
			     int where, int size, u32 val) /* NVMe: 장치 config 접근 함수 매개변수 선언 */
{ /* NVMe: 함수/매크로 본문 시작 */
	void __iomem *addr; /* NVMe: 메모리 맵된 config space 주소 */
 ...
 *   플랫폼에서 사용될 수 있다.
 */
int pci_generic_config_read32(struct pci_bus *bus, unsigned int devfn, /* NVMe: 32비트 전용 하드웨어에서 NVMe config dword를 읽는 경로 */
			      int where, int size, u32 *val) /* NVMe: 장치 config 접근 함수 매개변수 선언 */
{ /* NVMe: 함수/매크로 본문 시작 */
	void __iomem *addr; /* NVMe: 메모리 맵된 config space 주소 */
 ...
 */
int pci_generic_config_write32(struct pci_bus *bus, unsigned int devfn, /* NVMe: 32비트 전용 하드웨어에서 NVMe config 부분 쓰기 경로 */
			       int where, int size, u32 val) /* NVMe: 장치 config 접근 함수 매개변수 선언 */
{ /* NVMe: 함수/매크로 본문 시작 */
	void __iomem *addr; /* NVMe: 메모리 맵된 config space 주소 */
	...
	if (!bus->unsafe_warn) { /* NVMe: 아직 경고를 출력하지 않은 버스라면 */
		dev_warn(&bus->dev, "%d-byte config write to %04x:%02x:%02x.%d offset %#x may corrupt adjacent RW1C bits\n", /* NVMe: 32비트 전용 하드웨어에서 부분 쓰기 시 RW1C 비트 훼손 가능성 경고 */
			 size, pci_domain_nr(bus), bus->number, /* NVMe: 경고 메시지에 bus/슬롯/함수/오프셋 정보 포함 */
 ...
 */
struct pci_ops *pci_bus_set_ops(struct pci_bus *bus, struct pci_ops *ops) /* NVMe: 버스를 포함한 PCI 버스의 저수준 ops 교체 함수 */
{ /* NVMe: 함수/매크로 본문 시작 */
 ...
 */
static noinline void pci_wait_cfg(struct pci_dev *dev) /* NVMe: 장치 config 접근 차단 시 차단 해제까지 대기 */
	__must_hold(&pci_lock) /* NVMe: pci_lock 보유 필요 (NVMe config 접근 직렬화) */
{ /* NVMe: 함수/매크로 본문 시작 */
	do { /* NVMe: config 접근 차단 해제까지 반복 대기 */
...
#define PCI_USER_READ_CONFIG(size, type)				 /* NVMe: userspace용 PCI config read 함수 생성 매크로 (NVMe sysfs/configfs 경로) */ \
int pci_user_read_config_##size						 /* NVMe: userspace용 NVMe config read 함수 선언 */ \
	(struct pci_dev *dev, int pos, type *val)			 /* NVMe: PCI 장치와 userspace config read 대상 offset/버퍼 */ \
{										 /* NVMe: userspace config read 매크로 본문 시작 */ \
	...
	ret = dev->bus->ops->read(dev->bus, dev->devfn,						 /* NVMe: bus ops를 통해 NVMe 장치 config read 수행 */ \
...
}										 /* NVMe: userspace config read 함수 본문 종료 */ \
EXPORT_SYMBOL_GPL(pci_user_read_config_##size); /* NVMe: userspace NVMe config read 함수를 GPL 모듈에 export */

/* Returns 0 on success, negative values indicate error. */
#define PCI_USER_WRITE_CONFIG(size, type)				 /* NVMe: userspace용 PCI config write 함수 생성 매크로 (NVMe 레지스터 사용자 제어) */ \
int pci_user_write_config_##size						 /* NVMe: userspace용 NVMe config write 함수 선언 */ \
	(struct pci_dev *dev, int pos, type val)			 /* NVMe: PCI 장치와 userspace config write 대상 offset/값 */ \
{										 /* NVMe: userspace config write 매크로 본문 시작 */ \
	...
	if (unlikely(dev->block_cfg_access)) /* NVMe: NVMe 장치가 차단 중인지 확인 */					\
		pci_wait_cfg(dev); /* NVMe: 차단 해제까지 대기 */						\
	ret = dev->bus->ops->write(dev->bus, dev->devfn,						 /* NVMe: bus ops를 통해 NVMe 장치 config write 수행 */ \
				   pos, sizeof(type), val); /* NVMe: 실제 config write (NVMe 레지스터 제어) */			\
	raw_spin_unlock_irq(&pci_lock); /* NVMe: 락 해제 */						\
												 /* NVMe: 매크로 정의 내 공백 연속 라인 */ \
	return pcibios_err_to_errno(ret); /* NVMe: errno로 변환하여 반환 */					\
}										 /* NVMe: userspace config write 함수 본문 종료 */ \
EXPORT_SYMBOL_GPL(pci_user_write_config_##size); /* NVMe: userspace NVMe config write 함수를 GPL 모듈에 export */
 ...
 */
void pci_cfg_access_lock(struct pci_dev *dev) /* NVMe: 장치의 config space 읽기/쓰기를 잠금 */
{ /* NVMe: 함수/매크로 본문 시작 */
 ...
 * atomic contexts.
 */
bool pci_cfg_access_trylock(struct pci_dev *dev) /* NVMe: 원자 컨텍스트에서 NVMe config 접근 차단을 시도 */
{ /* NVMe: 함수/매크로 본문 시작 */
	unsigned long flags; /* NVMe: 인터럽트 상태 저장 */
	...
	else /* NVMe: 차단 중이 아니면 config 접근 차단 플래그 설정 */
 ...
 */
void pci_cfg_access_unlock(struct pci_dev *dev) /* NVMe: 장치의 config space 접근을 다시 허용 */
{ /* NVMe: 함수/매크로 본문 시작 */
	unsigned long flags; /* NVMe: 인터럽트 상태 저장 */
 ...
 */
static inline int pcie_cap_version(const struct pci_dev *dev) /* NVMe: 장치의 PCIe capability 버전 반환 */
{ /* NVMe: 함수/매크로 본문 시작 */
 ...
 */
bool pcie_cap_has_lnkctl(const struct pci_dev *dev) /* NVMe: endpoint가 Link Control 레지스터를 갖는지 판단 */
{ /* NVMe: 함수/매크로 본문 시작 */
	int type = pci_pcie_type(dev); /* NVMe: 현재 PCIe function 타입(endpoint/root port 등) 획득 */

	return type == PCI_EXP_TYPE_ENDPOINT || /* NVMe: 일반 PCIe endpoint (NVMe SSD) */
	       type == PCI_EXP_TYPE_LEG_END || /* NVMe: Legacy endpoint */
	       type == PCI_EXP_TYPE_ROOT_PORT || /* NVMe: Root Port (NVMe 상위 포트) */
	       type == PCI_EXP_TYPE_UPSTREAM || /* NVMe: Upstream Port */
	       type == PCI_EXP_TYPE_DOWNSTREAM || /* NVMe: Downstream Port */
	       type == PCI_EXP_TYPE_PCI_BRIDGE || /* NVMe: PCI-to-PCI Bridge */
 ...
 */
bool pcie_cap_has_lnkctl2(const struct pci_dev *dev) /* NVMe: 링크 속도/폭 협상에 사용되는 LNKCTL2 존재 여부 */
{ /* NVMe: 함수/매크로 본문 시작 */
 ...
 */
static inline bool pcie_cap_has_sltctl(const struct pci_dev *dev) /* NVMe: 장치가 연결된 downstream 포트의 Slot Control 존재 여부 */
{ /* NVMe: 함수/매크로 본문 시작 */
	return pcie_downstream_port(dev) && /* NVMe: 상위 downstream 포트이고 slot 플래그가 설정되어야 함 */
 ...
 */
bool pcie_cap_has_rtctl(const struct pci_dev *dev) /* NVMe: 장치 상위 Root Port의 Root Control 존재 여부 */
{ /* NVMe: 함수/매크로 본문 시작 */
	int type = pci_pcie_type(dev); /* NVMe: PCIe function 타입 획득 */

	return type == PCI_EXP_TYPE_ROOT_PORT || /* NVMe: Root Port (NVMe 상위 포트) */
 ...
 */
static bool pcie_capability_reg_implemented(struct pci_dev *dev, int pos) /* NVMe: 장치에서 요청한 PCIe capability 레지스터가 구현되었는지 확인 */
{ /* NVMe: 함수/매크로 본문 시작 */
	...
	case PCI_EXP_FLAGS: /* NVMe: PCIe Capabilities Register는 항상 존재 */
		return true; /* NVMe: 해당 PCIe capability 레지스터는 NVMe 장치에서 구현됨 */
	...
	case PCI_EXP_DEVSTA: /* NVMe: Device Status */
		return true; /* NVMe: 해당 PCIe capability 레지스터는 장치에서 구현됨 */
 ...
 *   link 상태를 확인한다.
 */
int pcie_capability_read_word(struct pci_dev *dev, int pos, u16 *val) /* NVMe: DEVCTL/LNKCTL 등 PCIe capability word 읽기 */
{ /* NVMe: 함수/매크로 본문 시작 */
	...
	*val = 0; /* NVMe: 기본값 0으로 초기화 */
	 ...
	 */
	if (pci_is_pcie(dev) && pcie_downstream_port(dev) && /* NVMe: 상위 downstream 포트의 Slot Status 예외 처리 */
 ...
 */
int pcie_capability_read_dword(struct pci_dev *dev, int pos, u32 *val) /* NVMe: DEVCAP/LNKCAP 등 PCIe capability dword 읽기 */
{ /* NVMe: 함수/매크로 본문 시작 */
	int ret; /* NVMe: read 반환 값 */
 ...
 */
int pcie_capability_write_word(struct pci_dev *dev, int pos, u16 val) /* NVMe: LNKCTL ASPM 비트 등 PCIe capability word 쓰기 */
{ /* NVMe: 함수/매크로 본문 시작 */
	if (pos & 1) /* NVMe: word 정렬 검사 */
		return PCIBIOS_BAD_REGISTER_NUMBER; /* NVMe: 정렬 오류 반환 */
 ...
 */
int pcie_capability_write_dword(struct pci_dev *dev, int pos, u32 val) /* NVMe: PCIe capability의 32비트 레지스터 갱신 */
{ /* NVMe: 함수/매크로 본문 시작 */
	if (pos & 3) /* NVMe: dword 정렬 검사 */
		return PCIBIOS_BAD_REGISTER_NUMBER; /* NVMe: 정렬 오류 반환 */
 ...
 */
int pcie_capability_clear_and_set_word_unlocked(struct pci_dev *dev, int pos, /* NVMe: PCIe capability word RMW (락 미사용) */
							u16 clear, u16 set) /* NVMe: 장치 config 접근 함수 매개변수 선언 */
{ /* NVMe: 함수/매크로 본문 시작 */
 ...
 */
int pcie_capability_clear_and_set_word_locked(struct pci_dev *dev, int pos, /* NVMe: PCIe capability word RMW (pcie_cap_lock 보호) */
						      u16 clear, u16 set) /* NVMe: 장치 config 접근 함수 매개변수 선언 */
{ /* NVMe: 함수/매크로 본문 시작 */
	unsigned long flags; /* NVMe: 인터럽트 상태 저장 */
	int ret; /* NVMe: 반환 값 */
 ...
 */
int pcie_capability_clear_and_set_dword(struct pci_dev *dev, int pos, /* NVMe: PCIe capability dword RMW */
						u32 clear, u32 set) /* NVMe: 장치 config 접근 함수 매개변수 선언 */
{ /* NVMe: 함수/매크로 본문 시작 */
	int ret; /* NVMe: 반환 값 */
 ...
 */
int pci_read_config_byte(const struct pci_dev *dev, int where, u8 *val) /* NVMe: 장치 config space byte 읽기 래퍼 */
{ /* NVMe: 함수/매크로 본문 시작 */
...
int pci_read_config_word(const struct pci_dev *dev, int where, u16 *val) /* NVMe: 장치 config space word 읽기 래퍼 (STATUS 등) */
{ /* NVMe: 함수/매크로 본문 시작 */
...
int pci_read_config_dword(const struct pci_dev *dev, int where, /* NVMe: 장치 config space dword 읽기 래퍼 (BAR 등) */
					u32 *val) /* NVMe: 장치 config 접근 함수 매개변수 선언 */
{ /* NVMe: 함수/매크로 본문 시작 */
	if (pci_dev_is_disconnected(dev)) { /* NVMe: NVMe 장치 분리 여부 확인 */
		PCI_SET_ERROR_RESPONSE(val); /* NVMe: 분리 시 error response */
		return PCIBIOS_DEVICE_NOT_FOUND; /* NVMe: 장치 미발견 */
 ...
 */
int pci_write_config_byte(const struct pci_dev *dev, int where, u8 val) /* NVMe: 장치 config space byte 쓰기 래퍼 */
{ /* NVMe: 함수/매크로 본문 시작 */
	if (pci_dev_is_disconnected(dev)) /* NVMe: NVMe 장치 분리 여부 확인 */
		return PCIBIOS_DEVICE_NOT_FOUND; /* NVMe: 장치 미발견 */
...
int pci_write_config_word(const struct pci_dev *dev, int where, u16 val) /* NVMe: 장치 config space word 쓰기 래퍼 */
{ /* NVMe: 함수/매크로 본문 시작 */
	if (pci_dev_is_disconnected(dev)) /* NVMe: NVMe 장치 분리 여부 확인 */
		return PCIBIOS_DEVICE_NOT_FOUND; /* NVMe: 장치 미발견 */
...
int pci_write_config_dword(const struct pci_dev *dev, int where, /* NVMe: 장치 config space dword 쓰기 래퍼 (DMA/PM 등) */
						 u32 val) /* NVMe: 장치 config 접근 함수 매개변수 선언 */
{ /* NVMe: 함수/매크로 본문 시작 */
	if (pci_dev_is_disconnected(dev)) /* NVMe: NVMe 장치 분리 여부 확인 */
		return PCIBIOS_DEVICE_NOT_FOUND; /* NVMe: 장치 미발견 */
 ...
 */
void pci_clear_and_set_config_dword(const struct pci_dev *dev, int pos, /* NVMe: command register 등 config dword RMW */
				    u32 clear, u32 set) /* NVMe: 장치 config 접근 함수 매개변수 선언 */
{ /* NVMe: 함수/매크로 본문 시작 */
