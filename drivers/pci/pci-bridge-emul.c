...
struct pci_bridge_reg_behavior {
	/* Read-only bits */
	/* [한국어] 이 dword 안에서 읽기 전용인 비트들의 마스크.
	 * 설정자: 아래 pci_regs_behavior[] / pcie_cap_regs_behavior[] 정적 표.
	 * 읽는 자: pci_bridge_emul_conf_write() 가 쓰기 요청에서 이 비트를 제외한다.
	 * 값 범위: 비트마스크. 세 마스크(ro/rw/w1c)에 없는 비트는 Reserved 로 간주돼
	 *   읽으면 0 이 나온다 -- 위 영어 주석이 인용한 PCIe 5.0 규정 그대로다.
	 * 동기화: const 정적 데이터라 잠금이 필요 없다. */
	u32 ro;
	/* Read-write bits */
	/* [한국어] 소프트웨어가 자유롭게 읽고 쓸 수 있는 비트들의 마스크.
	 * 설정자: 위와 같은 정적 표.
	 * 읽는 자: conf_write() 가 이 마스크에 든 비트만 실제로 갱신한다.
	 * 값 범위: 비트마스크. ro 와 겹치지 않는다.
	 * 동기화: const 정적 데이터. */
	u32 rw;

	/* Write-1-to-clear bits */
	/* [한국어] 1 을 써야 지워지는 비트들의 마스크. 오류·상태 플래그가 여기 속한다.
	 * 설정자: 위와 같은 정적 표.
	 * 읽는 자: conf_write() 가 이 비트에 대해서는 대입이 아니라 '1 이면 clear'
	 *   의미로 처리한다. 그래야 다른 비트를 건드리지 않고 하나만 지울 수 있다.
	 * 값 범위: 비트마스크.
	 * 동기화: const 정적 데이터. */
	u32 w1c;
};
