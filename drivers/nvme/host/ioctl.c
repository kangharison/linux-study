...
struct nvme_user_io32 {
	...
	__u16	nblocks;
	__u16	rsvd;
	/* [한국어] 메타데이터 버퍼의 유저 공간 주소. 0 이면 메타데이터를 쓰지 않는다.
	 * 설정자: 유저 공간이 ioctl 인자로 채운다.
	 * 읽는 자: nvme_submit_io() 가 nvme_map_user_request() 로 넘겨 매핑한다.
	 * 값 범위 주의: 커널이 그대로 역참조하지 않고 반드시 매핑을 거친다 --
	 *   유저 포인터를 신뢰하지 않는다는 뜻이다.
	 * 동기화: 요청 하나에만 속하므로 공유되지 않는다. */
	__u64	metadata;
	/* [한국어] 데이터 버퍼의 유저 공간 주소.
	 * 설정자: 유저 공간.
	 * 읽는 자: nvme_map_user_request() 가 이 주소를 요청에 매핑한다.
	 * 값 범위: nblocks 가 가리키는 크기만큼 유효해야 한다. 부족하면 매핑이 실패한다.
	 * 동기화: 요청 단위. */
	__u64	addr;
	/* [한국어] 시작 LBA(Starting Logical Block Address).
	 * 설정자: 유저 공간.
	 * 읽는 자: nvme_submit_io() 가 SQE 의 cdw10/11 에 그대로 옮긴다.
	 * 값 범위: 네임스페이스 용량 안이어야 한다. 벗어나면 컨트롤러가
	 *   LBA Out of Range 로 거절한다 -- 커널이 미리 걸러 주지 않는다.
	 * 동기화: 요청 단위. */
	__u64	slba;
	/* [한국어] Dataset Management 필드. 접근 빈도·지연 힌트를 담는다.
	 * 설정자: 유저 공간.
	 * 읽는 자: SQE 의 cdw13 으로 그대로 전달된다.
	 * 값 범위: NVMe 스펙의 DSM 인코딩. 0 이면 힌트 없음.
	 * 동기화: 요청 단위. */
	__u32	dsmgmt;
	/* [한국어] 보호 정보(PI)의 초기 참조 태그.
	 * 설정자: 유저 공간.
	 * 읽는 자: SQE 의 cdw14. 컨트롤러가 각 블록의 참조 태그와 대조한다.
	 * 값 범위: PI 를 쓰지 않는 네임스페이스에서는 무시된다.
	 * 동기화: 요청 단위. */
	__u32	reftag;
	/* [한국어] 보호 정보의 애플리케이션 태그.
	 * 설정자: 유저 공간.
	 * 읽는 자: SQE 의 cdw15 하위 16비트.
	 * 값 범위: 아래 appmask 로 걸러진 비트만 검사 대상이 된다.
	 * 동기화: 요청 단위. */
	__u16	apptag;
	/* [한국어] 애플리케이션 태그 검사 마스크. 1 인 비트만 대조한다.
	 * 설정자: 유저 공간.
	 * 읽는 자: SQE 의 cdw15 상위 16비트.
	 * 왜 마스크가 따로 있나: 애플리케이션이 태그의 일부만 의미 있게 쓰고 나머지를
	 *   자유롭게 두고 싶을 때, 검사 범위를 좁히기 위해서다.
	 * 동기화: 요청 단위. */
	__u16	appmask;
