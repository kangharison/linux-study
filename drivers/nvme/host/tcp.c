...
#define pr_fmt(fmt) KBUILD_MODNAME ": " fmt	/* [한국어] 모든 pr_ 로그에 "nvme_tcp: " 를 붙인다 */
#include <linux/module.h>	/* [한국어] 모듈 등록·파라미터 */
#include <linux/init.h>	/* [한국어] __init/__exit 표시 */
#include <linux/slab.h>	/* [한국어] kmalloc 계열 */
#include <linux/err.h>	/* [한국어] 오류 포인터 규약 */
#include <linux/crc32.h>	/* [한국어] 다이제스트가 쓰는 CRC32C */
#include <linux/nvme-tcp.h>	/* [한국어] NVMe/TCP PDU 형식 정의 */
#include <linux/nvme-keyring.h>	/* [한국어] TLS PSK 를 찾을 키링 */
#include <net/sock.h>	/* [한국어] 소켓 콜백(data_ready, write_space, state_change) */
#include <net/tcp.h>	/* [한국어] TCP_NODELAY 등 소켓 옵션 */
#include <net/tls.h>	/* [한국어] TLS 소켓 상태 조회 */
#include <net/tls_prot.h>	/* [한국어] TLS 레코드 종류 — 경보와 데이터를 구별한다 */
#include <net/handshake.h>	/* [한국어] 핸드셰이크를 사용자 공간 데몬에 위임하는 넷링크 API */
#include <linux/blk-mq.h>	/* [한국어] 태그셋과 요청 API */
#include <net/busy_poll.h>	/* [한국어] 폴링 큐가 소켓을 직접 훑을 때 */
#include <trace/events/sock.h>	/* [한국어] 소켓 추적점 */

#include "nvme.h"	/* [한국어] 코어 API */
#include "fabrics.h"	/* [한국어] Connect·옵션·재연결 공통 코드 */

struct nvme_tcp_queue;	/* [한국어] 전방 선언 — 아래 구조체들이 서로를 가리킨다 */
 ...
 */
static int so_priority;	/* [한국어] 위 영어 주석대로 NIC 마다 의미가 달라 기본값을 두지 않는다 */
module_param(so_priority, int, 0644);
MODULE_PARM_DESC(so_priority, "nvme tcp socket optimize priority");	/* [한국어] 소켓 우선순위 — NIC 의 큐 선택이나 QoS 에 영향을 준다 */

/*
 * Use the unbound workqueue for nvme_tcp_wq, then we can set the cpu affinity
 * from sysfs.
 */
static bool wq_unbound;	/* [한국어] 위 영어 주석대로 I/O 워크를 특정 CPU 에 묶을지 여부 */
module_param(wq_unbound, bool, 0644);
MODULE_PARM_DESC(wq_unbound, "Use unbound workqueue for nvme-tcp IO context (default false)");	/* [한국어] 묶으면 캐시 지역성이 좋고, 풀면 부하가 흩어진다 */

/*
 * TLS handshake timeout
 */
static int tls_handshake_timeout = 10;	/* [한국어] 핸드셰이크를 사용자 공간 데몬이 하므로 응답이 없을 수 있다 */
#ifdef CONFIG_NVME_TCP_TLS	/* [한국어] TLS 를 끈 빌드에서는 이 파라미터가 의미 없다 */
module_param(tls_handshake_timeout, int, 0644);
MODULE_PARM_DESC(tls_handshake_timeout,
		 "nvme TLS handshake timeout in seconds (default 10)");
#endif

static atomic_t nvme_tcp_cpu_queues[NR_CPUS];	/* [한국어] CPU 마다 몇 개의 큐가 배정됐는지 — 새 큐를 가장 한가한 CPU 에 붙이기 위해 */

#ifdef CONFIG_DEBUG_LOCK_ALLOC	/* [한국어] lockdep 을 켠 빌드에서만 필요하다 */
 ...
 */
static struct lock_class_key nvme_tcp_sk_key[2];	/* [한국어] 위 영어 주석대로 커널 소켓과 사용자 소켓을 lockdep 이 구별하게 한다 */
static struct lock_class_key nvme_tcp_slock_key[2];	/* [한국어] 구별하지 않으면 거짓 교착 경고가 뜬다 */
...
#else
static void nvme_tcp_reclassify_socket(struct socket *sock) { }	/* [한국어] lockdep 이 없으면 할 일이 없다 */
#endif
...
struct nvme_tcp_ctrl;	/* [한국어] 전방 선언 */
...
static LIST_HEAD(nvme_tcp_ctrl_list);	/* [한국어] 중복 연결 검사와 모듈 언로드 시 일괄 삭제에 쓰인다 */
static DEFINE_MUTEX(nvme_tcp_ctrl_mutex);	/* [한국어] 그 목록을 보호한다 */
static struct workqueue_struct *nvme_tcp_wq;	/* [한국어] 송수신 워크가 도는 큐 — wq_unbound 가 성격을 정한다 */
static const struct blk_mq_ops nvme_tcp_mq_ops;	/* [한국어] 전방 선언 — 아래 정의를 앞당겨 참조한다 */
...
static inline int nvme_tcp_queue_id(struct nvme_tcp_queue *queue)	/* [한국어] 큐 배열에서의 위치가 곧 큐 번호다 */
{
	return queue - queue->ctrl->queues;	/* [한국어] 포인터 차로 인덱스를 얻는다 — 번호를 따로 저장하지 않는다 */
}

static inline bool nvme_tcp_recv_pdu_supported(enum nvme_tcp_pdu_type type)	/* [한국어] 대상이 보낼 수 있는 PDU 종류인가 */
{
	switch (type) {	/* [한국어] 호스트가 받는 방향의 PDU 만 나열한다 */
	case nvme_tcp_c2h_term:	/* [한국어] 연결 종료 통지 */
	case nvme_tcp_c2h_data:	/* [한국어] 읽기 데이터 */
	case nvme_tcp_r2t:	/* [한국어] 쓰기 데이터를 보내라는 요청 */
	case nvme_tcp_rsp:	/* [한국어] 명령 완료 */
		return true;
	default:
		return false;	/* [한국어] 호스트가 보내는 종류가 왔다 — 대상이 규약을 어겼다 */
	}
}
...
{
	if (!IS_ENABLED(CONFIG_NVME_TCP_TLS))	/* [한국어] 빌드에 없으면 항상 거짓 — 컴파일러가 관련 분기를 걷어 낸다 */
		return 0;

	return queue->tls_enabled;	/* [한국어] 핸드셰이크가 실제로 성공한 큐만 참이다 */
}
...
static inline bool nvme_tcp_tls_configured(struct nvme_ctrl *ctrl)
{
	...
	return ctrl->opts->tls || ctrl->opts->concat;	/* [한국어] 사용자가 직접 켰거나 concat 으로 키를 유도하는 구성이다 */
}

static inline struct blk_mq_tags *nvme_tcp_tagset(struct nvme_tcp_queue *queue)	/* [한국어] 완료의 command_id 로 요청을 찾을 태그셋 */
{
	u32 queue_idx = nvme_tcp_queue_id(queue);

	if (queue_idx == 0)	/* [한국어] 0번은 admin — 태그셋이 다르다 */
		return queue->ctrl->admin_tag_set.tags[queue_idx];
	return queue->ctrl->tag_set.tags[queue_idx - 1];	/* [한국어] I/O 태그셋에서는 admin 몫을 빼야 인덱스가 맞는다 */
}

static inline u8 nvme_tcp_hdgst_len(struct nvme_tcp_queue *queue)	/* [한국어] 헤더 다이제스트가 차지하는 바이트 수 */
{
	return queue->hdr_digest ? NVME_TCP_DIGEST_LENGTH : 0;	/* [한국어] 협상 결과에 따라 0 이거나 4 — PDU 오프셋 계산에 그대로 들어간다 */
}

static inline u8 nvme_tcp_ddgst_len(struct nvme_tcp_queue *queue)	/* [한국어] 데이터 다이제스트 쪽도 같은 방식 */
{
	return queue->data_digest ? NVME_TCP_DIGEST_LENGTH : 0;
}

static inline void *nvme_tcp_req_cmd_pdu(struct nvme_tcp_request *req)	/* [한국어] 요청에 딸린 PDU 버퍼의 앞쪽 */
{
	return req->pdu;	/* [한국어] 명령 PDU 는 처음부터 쓴다 */
}

static inline void *nvme_tcp_req_data_pdu(struct nvme_tcp_request *req)	/* [한국어] 같은 버퍼의 뒤쪽을 데이터 PDU 로 쓴다 */
{
	/* use the pdu space in the back for the data pdu */
	return req->pdu + sizeof(struct nvme_tcp_cmd_pdu) -	/* [한국어] 위 영어 주석대로 뒤쪽 공간을 재활용한다 — 두 PDU 가 동시에 필요하지 않기 때문이다 */
		sizeof(struct nvme_tcp_data_pdu);
}

static inline size_t nvme_tcp_inline_data_size(struct nvme_tcp_request *req)	/* [한국어] 명령과 함께 붙여 보낼 수 있는 데이터의 최대 크기 */
{
	if (nvme_is_fabrics(req->req.cmd))	/* [한국어] Fabrics 명령은 캡슐 크기가 스펙 고정값이다 */
		return NVME_TCP_ADMIN_CCSZ;
	return req->queue->cmnd_capsule_len - sizeof(struct nvme_command);	/* [한국어] I/O 큐는 협상한 캡슐 길이에서 SQE 크기를 뺀 만큼 */
}

static inline bool nvme_tcp_async_req(struct nvme_tcp_request *req)	/* [한국어] AEN 전용 요청인가 */
{
	return req == &req->queue->ctrl->async_req;	/* [한국어] 컨트롤러에 하나뿐이라 주소 비교로 충분하다 */
}

static inline bool nvme_tcp_has_inline_data(struct nvme_tcp_request *req)	/* [한국어] 명령 PDU 뒤에 데이터를 붙여 보낼 것인가 */
{
	struct request *rq;

	if (unlikely(nvme_tcp_async_req(req)))	/* [한국어] 위 영어 주석대로 AEN 은 요청이 없어 데이터도 없다 */
		return false; /* async events don't have a request */

	rq = blk_mq_rq_from_pdu(req);

	return rq_data_dir(rq) == WRITE && req->data_len &&	/* [한국어] 쓰기이고 데이터가 있고 캡슐에 들어갈 만큼 작아야 한다 */
		req->data_len <= nvme_tcp_inline_data_size(req);
}

static inline struct page *nvme_tcp_req_cur_page(struct nvme_tcp_request *req)	/* [한국어] 반복자가 가리키는 현재 페이지 */
{
	return req->iter.bvec->bv_page;	/* [한국어] sendpage 계열에 넘길 페이지 */
}

static inline size_t nvme_tcp_req_cur_offset(struct nvme_tcp_request *req)	/* [한국어] 그 페이지 안의 현재 위치 */
{
	return req->iter.bvec->bv_offset + req->iter.iov_offset;	/* [한국어] bvec 자체의 오프셋에 반복자가 진행한 만큼을 더한다 */
}

static inline size_t nvme_tcp_req_cur_length(struct nvme_tcp_request *req)	/* [한국어] 한 번에 보낼 수 있는 길이 */
{
	return min_t(size_t, iov_iter_single_seg_count(&req->iter),	/* [한국어] 현재 세그먼트의 남은 양과 PDU 의 남은 양 중 작은 쪽 */
			req->pdu_len - req->pdu_sent);
}

static inline size_t nvme_tcp_pdu_data_left(struct nvme_tcp_request *req)	/* [한국어] 이 PDU 로 아직 보내지 않은 바이트 */
{
	return rq_data_dir(blk_mq_rq_from_pdu(req)) == WRITE ?	/* [한국어] 쓰기와 읽기가 세는 기준이 다르다 */
			req->pdu_len - req->pdu_sent : 0;
}

static inline size_t nvme_tcp_pdu_last_send(struct nvme_tcp_request *req,	/* [한국어] 이번 전송으로 PDU 가 끝나는가 */
		int len)
{
	return nvme_tcp_pdu_data_left(req) <= len;	/* [한국어] 끝이면 다이제스트를 이어 붙여야 한다 */
}

/*
 * [한국어]
 * nvme_tcp_init_iter - 이 요청의 데이터를 훑을 반복자를 세운다
 *
 * @req: 대상 요청
 * @dir: ITER_SOURCE(보낼 데이터) 또는 ITER_DEST(받을 자리)
 * @return: 없음
 *
 * TCP 는 바이트 스트림이라 한 번에 다 보내지지 않는다. 그래서 어디까지
 * 보냈는지 기억할 반복자가 필요하고, 이 함수가 그것을 요청의 현재 bio 에
 * 맞춰 초기화한다.
 *
 * 두 갈래로 갈린다. Write Zeroes 나 discard 처럼 특수 페이로드를 쓰는
 * 요청은 bio 없이 벡터 하나만 있고, 보통 요청은 bio 의 bvec 배열을
 * 그대로 쓴다.
 *
 * bi_bvec_done 을 오프셋으로 넘기는 것이 중요하다. 이 bio 는 앞서 일부가
 * 이미 처리됐을 수 있고, 그 지점부터 이어야 한다.
 *
 * 실행 컨텍스트: 제출 경로와 전송 워크. 잠들지 않는다.
 *
 * 호출 체인: nvme_tcp_setup_cmd_pdu / _advance_req → [이 함수]
 */
static void nvme_tcp_init_iter(struct nvme_tcp_request *req,
		unsigned int dir)
{
	struct request *rq = blk_mq_rq_from_pdu(req);
	struct bio_vec *vec;	/* [한국어] 반복자가 훑을 벡터 배열 */
	unsigned int size;	/* [한국어] 전체 바이트 */
	int nr_bvec;	/* [한국어] 벡터 개수 */
	size_t offset;	/* [한국어] 이미 처리된 만큼 */

	if (rq->rq_flags & RQF_SPECIAL_PAYLOAD) {	/* [한국어] discard 처럼 bio 없이 벡터 하나만 있는 요청 */
		vec = &rq->special_vec;	/* [한국어] 요청 안에 박힌 단일 벡터 */
		nr_bvec = 1;
		size = blk_rq_payload_bytes(rq);
		offset = 0;	/* [한국어] 특수 페이로드는 나뉘지 않아 항상 처음부터 */
	} else {
		struct bio *bio = req->curr_bio;	/* [한국어] 여러 bio 로 이어진 요청의 현재 것 */
		struct bvec_iter bi;	/* [한국어] 개수를 세기 위한 임시 반복자 */
		struct bio_vec bv;

		vec = __bvec_iter_bvec(bio->bi_io_vec, bio->bi_iter);	/* [한국어] 현재 위치의 벡터부터 시작한다 */
		nr_bvec = 0;
		bio_for_each_bvec(bv, bio, bi) {	/* [한국어] 개수를 알아야 iov_iter_bvec 에 넘길 수 있다 */
			nr_bvec++;	/* [한국어] 반복자에 넘길 개수를 미리 세어 둔다 */
		}
		size = bio->bi_iter.bi_size;	/* [한국어] 이 bio 의 남은 바이트 */
		offset = bio->bi_iter.bi_bvec_done;	/* [한국어] 앞서 일부가 처리됐을 수 있어 그 지점부터 이어야 한다 */
	}

	iov_iter_bvec(&req->iter, dir, vec, nr_bvec, size);	/* [한국어] 반복자를 세운다 — 이후 전송이 이것만 보고 진행한다 */
		...
		int len)
{
	...
	req->pdu_sent += len;	/* [한국어] 이번 PDU 에서 보낸 양 — 다이제스트 시점 판단에 쓴다 */
	iov_iter_advance(&req->iter, len);	/* [한국어] 반복자도 함께 전진시킨다 */
	if (!iov_iter_count(&req->iter) &&	/* [한국어] 현재 bio 를 다 보냈지만 */
	    req->data_sent < req->data_len) {
		req->curr_bio = req->curr_bio->bi_next;	/* [한국어] 요청에는 아직 데이터가 남았다 — 다음 bio 로 */
		nvme_tcp_init_iter(req, ITER_SOURCE);	/* [한국어] 새 bio 에 맞춰 반복자를 다시 세운다 */
	}
}
...
static inline void nvme_tcp_send_all(struct nvme_tcp_queue *queue)	/* [한국어] NVMe/TCP 큐·PDU·소켓 경로 헬퍼 */
{
	...
	do {	/* [한국어] 위 영어 주석대로 더 보낼 것이 없을 때까지 */
		ret = nvme_tcp_try_send(queue);
	} while (ret > 0);	/* [한국어] 0 은 보낼 것이 없다는 뜻, 음수는 오류다 */
}

static inline bool nvme_tcp_queue_has_pending(struct nvme_tcp_queue *queue)	/* [한국어] 보낼 것이 남았는가 */
{
	return !list_empty(&queue->send_list) ||	/* [한국어] 두 대기열 중 하나라도 비어 있지 않으면 참 */
		!llist_empty(&queue->req_list);
}

static inline bool nvme_tcp_queue_more(struct nvme_tcp_queue *queue)	/* [한국어] MSG_MORE 를 붙여도 되는가 */
{
	return !nvme_tcp_queue_tls(queue) &&	/* [한국어] TLS 는 레코드 단위라 MSG_MORE 가 오히려 지연을 늘린다 */
		nvme_tcp_queue_has_pending(queue);
}

/*
 * [한국어]
 * nvme_tcp_queue_request - 요청을 전송 대기열에 넣고, 가능하면 그 자리에서 보낸다
 *
 * @req:  보낼 요청
 * @last: blk-mq 가 알린 마지막 요청인가. 배치의 끝이면 워크를 깨운다.
 * @return: 없음
 *
 * 대기열이 두 겹인 것이 이 함수의 핵심이다. 여러 CPU 가 동시에 넣는
 * req_list 는 락 없는 llist 이고, 전송 워크만 만지는 send_list 는 평범한
 * 목록이다. 제출 쪽이 락을 잡지 않아도 되게 하려는 구조다.
 *
 * llist_add 의 반환값이 "이것이 첫 항목이었는가"를 알려 준다. 첫 항목일
 * 때만 전송을 깨우면 되므로, 이미 대기 중이면 아무것도 하지 않는다.
 *
 * 같은 CPU 조건이 붙은 이유가 위 영어 주석에 있다. 전송 워크가 도는
 * CPU 와 지금 CPU 가 같을 때만 직접 보낸다 -- 다른 CPU 라면 뮤텍스를
 * 두고 경쟁하게 되고, 그 대기가 제출 지연이 된다. trylock 인 것도
 * 같은 이유다.
 *
 * 실행 컨텍스트: blk-mq 제출. 잠들지 않는다.
 *
 * 호출 체인: nvme_tcp_queue_rq → [이 함수] → nvme_tcp_send_all
 */
static inline void nvme_tcp_queue_request(struct nvme_tcp_request *req,
		bool last)
{
	struct nvme_tcp_queue *queue = req->queue;
	bool empty;	/* [한국어] 이 요청이 대기열의 첫 항목이었는가 */

	empty = llist_add(&req->lentry, &queue->req_list) &&	/* [한국어] 락 없는 추가 — 여러 CPU 가 동시에 넣을 수 있다 */
	 ...
	 */
	if (queue->io_cpu == raw_smp_processor_id() &&	/* [한국어] 위 영어 주석대로 같은 CPU 일 때만 직접 보낸다 */
	    empty && mutex_trylock(&queue->send_mutex)) {	/* [한국어] 첫 항목이고 전송이 놀고 있을 때만 — 기다리면 제출이 지연된다 */
		nvme_tcp_send_all(queue);	/* [한국어] 그 자리에서 보내면 워크 전환 지연이 없다 */
		mutex_unlock(&queue->send_mutex);
	}

	if (last && nvme_tcp_queue_has_pending(queue))	/* [한국어] 배치의 끝인데 남은 것이 있으면 워크를 깨운다 */
		queue_work_on(queue->io_cpu, nvme_tcp_wq, &queue->io_work);
}

/*
 * [한국어]
 * nvme_tcp_process_req_list - 락 없는 대기열을 전송 목록으로 옮긴다
 *
 * @queue: 대상 큐
 * @return: 없음
 *
 * llist_del_all 이 목록 전체를 원자적으로 떼어 온다. 그 뒤로는 이 사슬을
 * 전송 워크만 보므로 락 없이 다룰 수 있다.
 *
 * list_add 로 앞에 붙이는 것에 주의할 것. llist 는 나중에 넣은 것이
 * 앞에 오므로, 앞쪽에 계속 붙이면 원래 제출 순서가 복원된다.
 *
 * 실행 컨텍스트: 전송 워크. 잠들지 않는다.
 *
 * 호출 체인: nvme_tcp_fetch_request → [이 함수]
 */
static void nvme_tcp_process_req_list(struct nvme_tcp_queue *queue)
{
	...
	for (node = llist_del_all(&queue->req_list); node; node = node->next) {	/* [한국어] 목록 전체를 원자적으로 떼어 온다 — 이후로는 이 워크만 본다 */
		req = llist_entry(node, struct nvme_tcp_request, lentry);
		list_add(&req->entry, &queue->send_list);	/* [한국어] llist 는 역순이라 앞에 붙이면 제출 순서가 복원된다 */
	}
}

/*
 * [한국어]
 * nvme_tcp_fetch_request - 다음에 보낼 요청을 하나 꺼낸다
 *
 * @queue: 대상 큐
 * @return: 보낼 요청. 없으면 NULL.
 *
 * 전송 목록을 먼저 보고, 비어 있을 때만 락 없는 대기열을 옮겨 온다.
 * 매번 옮기지 않는 이유는 llist_del_all 이 원자적 교환이라 공짜가
 * 아니기 때문이다.
 *
 * 두 목록 노드를 모두 초기화하고 돌려주는 것이 중요하다. 이 요청은
 * 전송이 끝나기 전에 다시 대기열에 들어갈 수 있고(R2T 를 받으면 데이터를
 * 더 보내야 한다), 그때 노드가 깨끗해야 한다.
 *
 * 실행 컨텍스트: 전송 워크. 잠들지 않는다.
 *
 * 호출 체인: nvme_tcp_try_send → [이 함수]
 */
...
nvme_tcp_fetch_request(struct nvme_tcp_queue *queue)
{
	...
	req = list_first_entry_or_null(&queue->send_list,	/* [한국어] 전송 목록을 먼저 본다 */
			struct nvme_tcp_request, entry);
	if (!req) {	/* [한국어] 비었을 때만 */
		nvme_tcp_process_req_list(queue);	/* [한국어] 락 없는 대기열을 옮겨 온다 — 원자적 교환이라 매번 하지 않는다 */
		req = list_first_entry_or_null(&queue->send_list,
				struct nvme_tcp_request, entry);
		if (unlikely(!req))	/* [한국어] 정말 아무것도 없다 */
			return NULL;
	}

	list_del_init(&req->entry);	/* [한국어] R2T 를 받으면 다시 들어올 수 있어 노드를 깨끗이 둔다 */
	init_llist_node(&req->lentry);	/* [한국어] llist 노드도 마찬가지 */
	return req;
}

#define NVME_TCP_CRC_SEED (~0)	/* [한국어] CRC32C 초기값 — 스펙이 정한 값이다 */

static inline void nvme_tcp_ddgst_update(u32 *crcp,
		struct page *page, size_t off, size_t len)
{
	page += off / PAGE_SIZE;	/* [한국어] 오프셋이 페이지를 넘으면 시작 페이지부터 옮긴다 */
	off %= PAGE_SIZE;	/* [한국어] 남은 오프셋은 페이지 안쪽 위치 */
	while (len) {	/* [한국어] 여러 페이지에 걸칠 수 있다 */
		const void *vaddr = kmap_local_page(page);	/* [한국어] highmem 페이지도 있을 수 있어 임시 매핑이 필요하다 */
		size_t n = min(len, (size_t)PAGE_SIZE - off);	/* [한국어] 이 페이지에서 처리할 만큼만 */

		*crcp = crc32c(*crcp, vaddr + off, n);	/* [한국어] 누산기를 이어 간다 — 페이지 경계는 CRC 에 영향이 없다 */
		kunmap_local(vaddr);	/* [한국어] 매핑은 짧게 잡고 바로 놓는다 */
		page++;	/* [한국어] 다음 페이지 */
		off = 0;	/* [한국어] 두 번째 페이지부터는 처음부터 */
		len -= n;
	}
}

static inline __le32 nvme_tcp_ddgst_final(u32 crc)	/* [한국어] 누산이 끝난 CRC 를 선로 형식으로 */
{
	return cpu_to_le32(~crc);	/* [한국어] 스펙이 정한 최종 보수 연산과 리틀엔디안 변환 */
}

static inline __le32 nvme_tcp_hdgst(const void *pdu, size_t len)	/* [한국어] 헤더는 한 번에 있으므로 누산이 필요 없다 */
{
	return cpu_to_le32(~crc32c(NVME_TCP_CRC_SEED, pdu, len));	/* [한국어] 초기값부터 최종 보수까지 한 줄로 */
}

static inline void nvme_tcp_set_hdgst(void *pdu, size_t len)	/* [한국어] 보낼 PDU 뒤에 다이제스트를 붙인다 */
{
	*(__le32 *)(pdu + len) = nvme_tcp_hdgst(pdu, len);	/* [한국어] 헤더 바로 뒤가 다이제스트 자리다 */
}

/*
 * [한국어]
 * nvme_tcp_verify_hdgst - 받은 PDU 헤더의 다이제스트를 확인한다
 *
 * @queue:   이 PDU 가 온 큐
 * @pdu:     받은 PDU
 * @pdu_len: 다이제스트를 뺀 헤더 길이
 * @return: 0 이면 정상, -EPROTO 면 규약 위반, -EIO 면 값이 다르다
 *
 * 두 가지를 확인하며 오류의 성격이 다르다.
 *
 * 플래그가 꺼져 있으면 -EPROTO 다. 다이제스트를 쓰기로 협상해 놓고
 * 붙이지 않았다는 뜻이므로, 상대 구현이 잘못됐거나 이 연결이 우리가
 * 생각하는 그 연결이 아니다. 재시도로 나아지지 않는다.
 *
 * 값이 다르면 -EIO 다. 전송 중 손상이므로 재연결로 나아질 수 있다.
 * 두 값을 모두 로그에 남겨 손상 유형을 짐작할 수 있게 한다.
 *
 * 실행 컨텍스트: 수신 경로(소프트 IRQ 또는 워크). 잠들지 않는다.
 *
 * 호출 체인: nvme_tcp_recv_pdu → [이 함수]
 */
static int nvme_tcp_verify_hdgst(struct nvme_tcp_queue *queue,
		void *pdu, size_t pdu_len)
{
	struct nvme_tcp_hdr *hdr = pdu;	/* [한국어] 공통 헤더로 먼저 본다 */
	__le32 recv_digest;	/* [한국어] 상대가 보낸 값 */
	__le32 exp_digest;	/* [한국어] 우리가 계산한 값 */

	if (unlikely(!(hdr->flags & NVME_TCP_F_HDGST))) {	/* [한국어] 쓰기로 협상해 놓고 붙이지 않았다 */
		dev_err(queue->ctrl->ctrl.device,
			"queue %d: header digest flag is cleared\n",
			nvme_tcp_queue_id(queue));
		return -EPROTO;	/* [한국어] 재시도로 나아지지 않는 구현 오류다 */
	}

	recv_digest = *(__le32 *)(pdu + hdr->hlen);	/* [한국어] 헤더 길이만큼 건너뛴 자리 */
	exp_digest = nvme_tcp_hdgst(pdu, pdu_len);	/* [한국어] 같은 범위로 다시 계산한다 */
	if (recv_digest != exp_digest) {	/* [한국어] 전송 중 손상됐다 */
		dev_err(queue->ctrl->ctrl.device,	/* [한국어] 두 값을 모두 남겨 손상 유형을 짐작할 수 있게 */
			"header digest error: recv %#x expected %#x\n",
			le32_to_cpu(recv_digest), le32_to_cpu(exp_digest));
		return -EIO;	/* [한국어] 재연결로 나아질 수 있는 오류 */
	}
...
/*
 * [한국어]
 * nvme_tcp_check_ddgst - 데이터 다이제스트 플래그를 확인하고 누산기를 연다
 *
 * @queue: 이 PDU 가 온 큐
 * @pdu:   받은 PDU
 * @return: 0 이면 정상, -EPROTO 면 협상과 어긋난다
 *
 * 데이터 길이를 헤더에서 역산하는 것이 이 함수의 절반이다. 전체 길이에서
 * 헤더와 (있다면) 헤더 다이제스트를 빼면 남는 것이 데이터다. 그 값이
 * 0 이 아닌데 데이터 다이제스트 플래그가 없으면 협상을 어긴 것이다.
 *
 * 나머지 절반은 CRC 누산기를 초기값으로 여는 일이다. 데이터가 여러 번에
 * 나뉘어 도착하므로 받는 대로 누적하고, 마지막에 한 번 비교한다.
 *
 * 실행 컨텍스트: 수신 경로. 잠들지 않는다.
 *
 * 호출 체인: nvme_tcp_handle_c2h_data → [이 함수]
 */
static int nvme_tcp_check_ddgst(struct nvme_tcp_queue *queue, void *pdu)
{
	struct nvme_tcp_hdr *hdr = pdu;
	u8 digest_len = nvme_tcp_hdgst_len(queue);	/* [한국어] 헤더 다이제스트가 차지하는 만큼 */
	u32 len;	/* [한국어] 역산할 데이터 길이 */

	len = le32_to_cpu(hdr->plen) - hdr->hlen -	/* [한국어] 전체에서 헤더와 */
		((hdr->flags & NVME_TCP_F_HDGST) ? digest_len : 0);

	if (unlikely(len && !(hdr->flags & NVME_TCP_F_DDGST))) {	/* [한국어] 데이터가 있는데 다이제스트 플래그가 없다 */
		dev_err(queue->ctrl->ctrl.device,
			"queue %d: data digest flag is cleared\n",
		nvme_tcp_queue_id(queue));
		return -EPROTO;
	}
	queue->rcv_crc = NVME_TCP_CRC_SEED;	/* [한국어] 데이터가 나뉘어 오므로 누산기를 열어 둔다 */
		...
		unsigned int numa_node)
{
	...
	int queue_idx = (set == &ctrl->tag_set) ? hctx_idx + 1 : 0;	/* [한국어] 태그셋 포인터로 admin 인지 I/O 인지 가른다 */
	struct nvme_tcp_queue *queue = &ctrl->queues[queue_idx];
	u8 hdgst = nvme_tcp_hdgst_len(queue);	/* [한국어] 다이제스트 자리까지 포함해 잡아야 한다 */

	req->pdu = page_frag_alloc(&queue->pf_cache,	/* [한국어] 페이지 조각 할당 — 작은 버퍼 수천 개를 페이지 낭비 없이 잡는다 */
	...
	pdu = req->pdu;
	req->queue = queue;	/* [한국어] 완료 경로가 큐를 되찾는 통로 */
	nvme_req(rq)->ctrl = &ctrl->ctrl;	/* [한국어] 코어가 오류 정책을 판단할 때 */
	nvme_req(rq)->cmd = &pdu->cmd;	/* [한국어] 코어가 조립할 SQE 는 PDU 안쪽이다 — 별도 복사가 없다 */
	init_llist_node(&req->lentry);	/* [한국어] 두 대기열 노드를 깨끗한 상태로 둔다 */
		...
		unsigned int hctx_idx)
{
	...
	struct nvme_tcp_queue *queue = &ctrl->queues[hctx_idx + 1];	/* [한국어] +1 이 admin 큐 자리를 건너뛴다 */
...
{
	struct nvme_tcp_ctrl *ctrl = to_tcp_ctrl(data);
	struct nvme_tcp_queue *queue = &ctrl->queues[0];	/* [한국어] admin 은 늘 0번 */
...
/*
 * [한국어]
 * nvme_tcp_recv_state - 지금 무엇을 받는 중인지 판정한다
 *
 * @queue: 대상 큐
 * @return: PDU 헤더, 데이터, 데이터 다이제스트 중 하나
 *
 * 별도의 상태 변수를 두지 않고 남은 바이트 수로 상태를 유도하는 것이
 * 요점이다. 헤더가 남았으면 헤더를 받는 중이고, 아니면서 다이제스트가
 * 남았으면 다이제스트를 받는 중이며, 둘 다 아니면 데이터다.
 *
 * 상태와 카운터가 어긋날 수 없다는 것이 이 방식의 이점이다. 하나를
 * 갱신하고 다른 하나를 빠뜨리는 종류의 버그가 생기지 않는다.
 *
 * 순서도 의미가 있다. 헤더가 먼저 와야 데이터 길이를 알 수 있고,
 * 다이제스트는 데이터 뒤에 온다.
 *
 * 실행 컨텍스트: 수신 경로. 잠들지 않는다.
 *
 * 호출 체인: nvme_tcp_recv_skb → [이 함수]
 */
static enum nvme_tcp_recv_state
nvme_tcp_recv_state(struct nvme_tcp_queue *queue)
{
	return  (queue->pdu_remaining) ? NVME_TCP_RECV_PDU :	/* [한국어] 남은 바이트로 상태를 유도한다 — 상태와 카운터가 어긋날 수 없다 */
		(queue->ddgst_remaining) ? NVME_TCP_RECV_DDGST :	/* [한국어] 헤더가 끝났고 다이제스트가 남았다 */
		NVME_TCP_RECV_DATA;	/* [한국어] 둘 다 아니면 데이터를 받는 중 */
}

/*
 * [한국어]
 * nvme_tcp_init_recv_ctx - 다음 PDU 를 받을 준비를 한다
 *
 * @queue: 대상 큐
 * @return: 없음
 *
 * 하나의 PDU 처리가 끝날 때마다 불린다. pdu_remaining 을 응답 PDU 크기로
 * 두는 것이 요점인데, 이 값은 "적어도 이만큼은 헤더"라는 뜻이다. 실제
 * 종류와 길이는 헤더를 다 받아 봐야 알 수 있고, 그때 다시 조정된다.
 *
 * data_remaining 을 -1 로 두는 것은 "아직 모른다"의 표시다. 0 이면
 * "데이터 없음"과 구별되지 않는다.
 *
 * 실행 컨텍스트: 수신 경로. 잠들지 않는다.
 *
 * 호출 체인: 각 PDU 처리 완료 → [이 함수]
 */
static void nvme_tcp_init_recv_ctx(struct nvme_tcp_queue *queue)
{
	queue->pdu_remaining = sizeof(struct nvme_tcp_rsp_pdu) +	/* [한국어] "적어도 이만큼은 헤더" — 실제 길이는 헤더를 받아 봐야 안다 */
				nvme_tcp_hdgst_len(queue);
	queue->pdu_offset = 0;	/* [한국어] 새 PDU 의 처음부터 */
	queue->data_remaining = -1;	/* [한국어] 0 은 "데이터 없음"이라 "아직 모름"과 구별해야 한다 */
	queue->ddgst_remaining = 0;	/* [한국어] 데이터 다이제스트 여부는 헤더를 보고 정한다 */
}
...
/*
 * [한국어]
 * nvme_tcp_process_nvme_cqe - 응답 PDU 안의 완료 항목을 요청과 짝짓는다
 *
 * @queue: 응답이 온 큐
 * @cqe:   완료 항목
 * @return: 0 이면 처리했다, -EINVAL 이면 모르는 command_id 다
 *
 * req->status 를 먼저 보는 것이 요점이다. 데이터 다이제스트가 어긋났을
 * 때 수신 경로가 이미 오류를 새겨 두었을 수 있고, 그 경우 대상이 보낸
 * 성공 상태로 덮어쓰면 손상된 데이터를 정상으로 올린다. 그래서 아직
 * 성공인 경우에만 대상의 상태를 반영한다.
 *
 * 모르는 command_id 는 복구 대상이다. 그 완료를 어느 요청에도 짝지을 수
 * 없으니 이 큐의 상태를 신뢰할 수 없다.
 *
 * 실행 컨텍스트: 수신 경로. 잠들지 않는다.
 *
 * 호출 체인: nvme_tcp_handle_comp → [이 함수] → nvme_try_complete_req
 */
		...
		struct nvme_completion *cqe)
{
	...
	rq = nvme_find_rq(nvme_tcp_tagset(queue), cqe->command_id);	/* [한국어] 태그와 세대를 함께 검사한다 */
	if (!rq) {	/* [한국어] 어느 요청에도 짝지을 수 없다 */
		dev_err(queue->ctrl->ctrl.device,
			"got bad cqe.command_id %#x on queue %d\n",
			cqe->command_id, nvme_tcp_queue_id(queue));
		nvme_tcp_error_recovery(&queue->ctrl->ctrl);	/* [한국어] 큐 상태를 신뢰할 수 없어 세션을 다시 세운다 */
		return -EINVAL;
	}

	req = blk_mq_rq_to_pdu(rq);
	if (req->status == cpu_to_le16(NVME_SC_SUCCESS))	/* [한국어] 다이제스트 오류가 이미 새겨졌을 수 있다 */
		req->status = cqe->status;

	if (!nvme_try_complete_req(rq, req->status, cqe->result))	/* [한국어] 거짓이면 코어가 재시도나 페일오버를 맡았다 */
		nvme_complete_rq(rq);
	queue->nr_cqe++;	/* [한국어] 폴링이 진전 여부를 판단하는 값 */
...
/*
 * [한국어]
 * nvme_tcp_handle_c2h_data - 읽기 데이터 PDU 의 헤더를 받아들인다
 *
 * @queue: PDU 가 온 큐
 * @pdu:   C2HData PDU 헤더
 * @return: 0 이면 정상, 음수면 오류
 *
 * 세 가지를 확인한 뒤 데이터 수신 상태로 넘어간다.
 *
 * 읽기가 아닌 요청에 데이터가 오면 -EIO 다. 상대가 우리가 내지 않은
 * 전송을 하고 있다는 뜻이다.
 *
 * SUCCESS 플래그는 마지막 PDU 에만 붙을 수 있다. 그것이 "이 데이터로
 * 명령이 끝났으니 별도 응답 PDU 를 보내지 않겠다"는 최적화이기 때문이다.
 * 마지막이 아닌데 붙어 있으면 뒤따라 올 데이터가 갈 곳을 잃는다.
 *
 * 실행 컨텍스트: 수신 경로. 잠들지 않는다.
 *
 * 호출 체인: nvme_tcp_recv_pdu → [이 함수]
		...
		struct nvme_tcp_data_pdu *pdu)
{
	...
	if (!rq) {
		...
		return -ENOENT;	/* [한국어] 이 PDU 만 버린다 — 아래 오류와 달리 복구까지 가지 않는다 */
	}

	if (!blk_rq_payload_bytes(rq)) {	/* [한국어] 읽기가 아닌 요청에 데이터가 왔다 */
		dev_err(queue->ctrl->ctrl.device,
			"queue %d tag %#x unexpected data\n",
			nvme_tcp_queue_id(queue), rq->tag);
		return -EIO;	/* [한국어] 우리가 내지 않은 전송이다 */
	}

	queue->data_remaining = le32_to_cpu(pdu->data_length);	/* [한국어] 이제 데이터 수신 상태로 넘어간다 */

	if (pdu->hdr.flags & NVME_TCP_F_DATA_SUCCESS &&	/* [한국어] 응답 PDU 를 생략하겠다는 최적화 플래그 */
	    unlikely(!(pdu->hdr.flags & NVME_TCP_F_DATA_LAST))) {
		dev_err(queue->ctrl->ctrl.device,
			"queue %d tag %#x SUCCESS set but not last PDU\n",
			nvme_tcp_queue_id(queue), rq->tag);
		nvme_tcp_error_recovery(&queue->ctrl->ctrl);	/* [한국어] 뒤따라 올 데이터가 갈 곳을 잃는다 */
		return -EPROTO;
	}
		...
		struct nvme_tcp_rsp_pdu *pdu)
{
	 ...
	 */
	if (unlikely(nvme_is_aen_req(nvme_tcp_queue_id(queue),	/* [한국어] 위 영어 주석대로 AEN 은 태그를 쓰지 않아 요청을 찾을 수 없다 */
				     cqe->command_id)))
		nvme_complete_async_event(&queue->ctrl->ctrl, cqe->status,
				&cqe->result);
	else	/* [한국어] 보통 명령은 태그로 요청을 찾는다 */
...
/*
 * [한국어]
 * nvme_tcp_handle_c2h_term - 대상이 보낸 연결 종료 통지를 로그로 남긴다
 *
 * @queue: 통지가 온 큐
 * @pdu:   C2HTermReq PDU
 * @return: 없음
 *
 * 대상이 규약 위반을 발견해 연결을 끊겠다고 알리는 PDU 다. 이 함수는
 * 이유를 사람이 읽을 수 있게 남기는 일만 한다 -- 실제 해체는 이어지는
 * 소켓 종료가 처리한다.
 *
 * 길이 검사가 먼저인 이유: 이 PDU 자체가 손상됐을 수 있고, 그것을
 * 신뢰해 안쪽 필드를 읽으면 잘못된 값으로 로그를 낸다. 이미 규약이
 * 깨진 상황이라 더 조심한다.
 *
 * 실행 컨텍스트: 수신 경로. 잠들지 않는다.
 *
...
{
	u16 fes;	/* [한국어] Fatal Error Status — 대상이 밝힌 위반 종류 */
	const char *msg;	/* [한국어] 그 코드의 사람이 읽을 이름 */
	u32 plen = le32_to_cpu(pdu->hdr.plen);	/* [한국어] PDU 전체 길이. 손상 여부 판단에 쓴다 */

	static const char * const msg_table[] = {	/* [한국어] 코드를 문자열로 — 숫자만 남기면 사용자가 해석할 수 없다 */
		[NVME_TCP_FES_INVALID_PDU_HDR] = "Invalid PDU Header Field",	/* [한국어] 헤더 필드가 규약에 맞지 않았다 */
	...
	if (plen < NVME_TCP_MIN_C2HTERM_PLEN ||	/* [한국어] 이 PDU 자체가 손상됐을 수 있다 */
	    plen > NVME_TCP_MAX_C2HTERM_PLEN) {
		dev_err(queue->ctrl->ctrl.device,
			"Received a malformed C2HTermReq PDU (plen = %u)\n",
			plen);
		return;	/* [한국어] 신뢰할 수 없는 값으로 로그를 내지 않는다 */
	}

	fes = le16_to_cpu(pdu->fes);
	if (fes && fes < ARRAY_SIZE(msg_table))	/* [한국어] 표 범위 안이어야 한다 */
		msg = msg_table[fes];
	else	/* [한국어] 대상이 새 코드를 쓰거나 값이 손상됐다 */
		msg = "Unknown";

	dev_err(queue->ctrl->ctrl.device,	/* [한국어] 이유를 남긴다 — 해체는 이어지는 소켓 종료가 한다 */
...
static inline void nvme_tcp_end_request(struct request *rq, u16 status)	/* [한국어] NVMe/TCP 큐·PDU·소켓 경로 헬퍼 */
{
	...
	if (!nvme_try_complete_req(rq, cpu_to_le16(status << 1), res))	/* [한국어] 상태 코드를 스펙의 비트 위치로 옮긴다 — 최하위 비트는 phase 자리다 */
		nvme_complete_rq(rq);
}
...
/*
 * [한국어]
 * nvme_tcp_recv_ddgst - 데이터 다이제스트를 받아 검증하고 PDU 를 닫는다
 *
 * @queue:  대상 큐
 * @skb:    받은 소켓 버퍼
 * @offset: 그 안의 현재 위치. 소비한 만큼 전진시킨다.
 * @len:    남은 바이트. 소비한 만큼 줄인다.
 * @return: 0 이면 정상, 음수면 복사 오류
 *
 * 4바이트짜리 다이제스트도 여러 번에 나뉘어 올 수 있어서, 남은 양으로
 * 오프셋을 역산해 이어 붙인다. 다 모이기 전에는 0 을 돌려주고 다음
 * 호출을 기다린다.
 *
 * 값이 다르면 요청에 오류를 새기되 여기서 완료시키지 않는다. 응답 PDU 가
 * 뒤따라 오고 그 처리가 이 상태를 보기 때문이다 -- process_nvme_cqe 가
 * 이미 오류인 요청에 성공을 덮어쓰지 않는 이유가 여기 있다.
 *
 * SUCCESS 플래그가 붙어 있으면 응답 PDU 가 오지 않으므로 여기서 요청을
 * 끝낸다. 그 두 경로가 겹치지 않는 것이 이 최적화의 전제다.
 *
 * 실행 컨텍스트: 수신 경로. 잠들지 않는다.
 *
 * 호출 체인: nvme_tcp_recv_skb → [이 함수]
...
{
	struct nvme_tcp_data_pdu *pdu = (void *)queue->pdu;	/* [한국어] 이 다이제스트가 속한 데이터 PDU */
	char *ddgst = (char *)&queue->recv_ddgst;	/* [한국어] 바이트 단위로 이어 붙이기 위해 */
	size_t recv_len = min_t(size_t, *len, queue->ddgst_remaining);	/* [한국어] 이번에 받을 수 있는 만큼만 */
	off_t off = NVME_TCP_DIGEST_LENGTH - queue->ddgst_remaining;	/* [한국어] 남은 양에서 역산한 이어 붙일 위치 */
	int ret;

	ret = skb_copy_bits(skb, *offset, &ddgst[off], recv_len);	/* [한국어] 소켓 버퍼는 조각날 수 있어 전용 복사기를 쓴다 */
	if (unlikely(ret))
		return ret;

	queue->ddgst_remaining -= recv_len;	/* [한국어] 수신 상태 판정이 이 값을 본다 */
	*offset += recv_len;	/* [한국어] 호출자의 진행 위치도 함께 */
	*len -= recv_len;
	if (queue->ddgst_remaining)	/* [한국어] 아직 다 모이지 않았다 */
		return 0;	/* [한국어] 다음 호출을 기다린다 */

	if (queue->recv_ddgst != queue->exp_ddgst) {	/* [한국어] 데이터가 전송 중 손상됐다 */
		struct request *rq = nvme_cid_to_rq(nvme_tcp_tagset(queue),
					pdu->command_id);
		struct nvme_tcp_request *req = blk_mq_rq_to_pdu(rq);

		req->status = cpu_to_le16(NVME_SC_DATA_XFER_ERROR);	/* [한국어] 여기서 완료시키지 않는다 — 응답 PDU 처리가 이 상태를 본다 */
	...
	if (pdu->hdr.flags & NVME_TCP_F_DATA_SUCCESS) {	/* [한국어] 응답 PDU 가 오지 않는 최적화 경로다 */
		struct request *rq = nvme_cid_to_rq(nvme_tcp_tagset(queue),
					pdu->command_id);
		struct nvme_tcp_request *req = blk_mq_rq_to_pdu(rq);

		nvme_tcp_end_request(rq, le16_to_cpu(req->status));	/* [한국어] 그래서 여기서 끝내야 한다. 위에서 새긴 오류가 있으면 그것이 반영된다 */
		queue->nr_cqe++;
	}

	nvme_tcp_init_recv_ctx(queue);	/* [한국어] 다음 PDU 를 받을 준비 */
	return 0;
}
...
/*
 * [한국어]
 * nvme_tcp_data_ready - 소켓에 데이터가 도착했다는 통지를 받는다
 *
 * @sk: 데이터가 온 소켓
 * @return: 없음
 *
 * 여기서 직접 읽지 않고 워크를 깨우기만 하는 것이 요점이다. 이 콜백은
 * 소프트 IRQ 문맥에서 불릴 수 있어 오래 머물면 안 되고, PDU 하나를
 * 처리하는 일은 요청 완료까지 이어져 길다.
 *
 * POLLING 비트를 보는 이유: 폴링 큐는 이미 누군가 소켓을 직접 훑고
 * 있으므로 워크를 깨우면 두 경로가 같은 소켓을 두고 부딪힌다.
 *
 * sk_callback_lock 은 소켓이 우리 큐를 가리키는 동안만 이 콜백이
 * 유효하게 만든다. 큐 해체가 그 락 아래에서 포인터를 지우므로,
 * 락을 들고 읽으면 사라진 큐를 건드리지 않는다.
 *
 * 실행 컨텍스트: 소프트 IRQ. 잠들 수 없다.
 *
 * 호출 체인: TCP 스택 → sk->sk_data_ready → [이 함수] → io_work
 */
static void nvme_tcp_data_ready(struct sock *sk)
{
	struct nvme_tcp_queue *queue;	/* [한국어] 소켓에 새겨 둔 큐 */

	trace_sk_data_ready(sk);	/* [한국어] 소켓 추적점 — 지연 분석에 쓴다 */

	read_lock_bh(&sk->sk_callback_lock);	/* [한국어] 해체가 이 락 아래에서 포인터를 지우므로 들고 읽어야 안전하다 */
	queue = sk->sk_user_data;	/* [한국어] 소켓 생성 시 걸어 둔 큐 */
	if (likely(queue && queue->rd_enabled) &&	/* [한국어] 수신을 멈춘 큐에는 깨울 것이 없다 */
...
/*
 * [한국어]
 * nvme_tcp_write_space - 송신 버퍼에 자리가 생겼다는 통지를 받는다
 *
 * @sk: 여유가 생긴 소켓
 * @return: 없음
 *
 * 전송이 -EAGAIN 으로 멈췄던 요청을 다시 밀어 줄 시점이다. data_ready 와
 * 마찬가지로 여기서는 워크를 깨우기만 한다.
 *
 * TLS 인 경우 원래 콜백을 한 번 더 부르는 것에 주의할 것. 위 영어
 * 주석대로 TLS 계층이 레코드 하나를 절반만 내보낸 상태로 붙들고 있을
 * 수 있고, 그것을 이어서 내보내게 하려면 TLS 자신의 write_space 가
 * 돌아야 한다. 우리 워크만 깨우면 그 절반이 영영 나가지 않는다.
 *
 * 실행 컨텍스트: 소프트 IRQ. 잠들 수 없다.
 *
 * 호출 체인: TCP 스택 → sk->sk_write_space → [이 함수] → io_work
 */
static void nvme_tcp_write_space(struct sock *sk)
{
	...
	if (likely(queue && sk_stream_is_writeable(sk))) {	/* [한국어] 실제로 쓸 만한 여유가 생겼을 때만 */
		clear_bit(SOCK_NOSPACE, &sk->sk_socket->flags);	/* [한국어] 다음 전송이 다시 시도하도록 표시를 지운다 */
		/* Ensure pending TLS partial records are retried */
		if (nvme_tcp_queue_tls(queue))	/* [한국어] 위 영어 주석대로 TLS 가 절반 내보낸 레코드를 붙들고 있을 수 있다 */
			queue->write_space(sk);
		queue_work_on(queue->io_cpu, nvme_tcp_wq, &queue->io_work);	/* [한국어] 전송을 이어 갈 워크를 깨운다 */
	}
	read_unlock_bh(&sk->sk_callback_lock);
}
...
static inline void nvme_tcp_done_send_req(struct nvme_tcp_queue *queue)	/* [한국어] 현재 전송 중인 요청을 비운다 */
{
	queue->request = NULL;	/* [한국어] 다음 fetch 가 새 요청을 꺼내게 한다 */
}

/*
 * [한국어]
 * nvme_tcp_fail_request - 전송에 실패한 요청을 경로 오류로 끝낸다
 *
 * @req: 실패한 요청
 * @return: 없음
 *
 * AEN 과 보통 요청이 갈리는 것은 AEN 에 짝지을 블록 요청이 없기 때문이다.
 * 코어에 직접 완료를 알리는 경로를 쓴다.
 *
 * HOST_PATH_ERROR 를 쓰는 것이 중요하다. 미디어 오류가 아니라 이 경로로
 * 보내지 못했다는 뜻이므로, 다중경로가 다른 경로로 재시도할 수 있다.
 *
 * 실행 컨텍스트: 전송 워크. 잠들지 않는다.
 *
 * 호출 체인: nvme_tcp_try_send(실패) → [이 함수]
 */
static void nvme_tcp_fail_request(struct nvme_tcp_request *req)
{
	if (nvme_tcp_async_req(req)) {	/* [한국어] AEN 은 짝지을 블록 요청이 없다 */
		union nvme_result res = {};	/* [한국어] AEN 완료에는 결과 값이 없다 */

		nvme_complete_async_event(&req->queue->ctrl->ctrl,	/* [한국어] 코어에 직접 알린다 */
				cpu_to_le16(NVME_SC_HOST_PATH_ERROR), &res);
	} else {
		nvme_tcp_end_request(blk_mq_rq_from_pdu(req),	/* [한국어] 경로 오류로 끝내 다중경로가 다른 경로를 시도하게 한다 */
...
/*
 * [한국어]
 * nvme_tcp_free_async_req - AEN 전용 요청의 PDU 버퍼를 반납한다
 *
 * @ctrl: 대상 컨트롤러
 * @return: 없음
 *
 * AEN 은 blk-mq 태그를 쓰지 않으므로 태그셋 해제로는 정리되지 않는다.
 * 컨트롤러 안에 하나뿐인 요청이고, 그 버퍼를 여기서 손으로 놓는다.
 *
 * 실행 컨텍스트: admin 큐 해체. 잠들 수 있다.
 *
 * 호출 체인: nvme_tcp_destroy_admin_queue → [이 함수]
 */
static void nvme_tcp_free_async_req(struct nvme_tcp_ctrl *ctrl)
{
	struct nvme_tcp_request *async = &ctrl->async_req;	/* [한국어] 컨트롤러에 하나뿐인 AEN 요청 */

	page_frag_free(async->pdu);	/* [한국어] 태그셋이 관여하지 않아 손으로 놓는다 */
}

/*
 * [한국어]
 * nvme_tcp_alloc_async_req - AEN 전용 요청을 준비한다
 *
 * @ctrl: 대상 컨트롤러
 * @return: 0 이면 성공, -ENOMEM 이면 실패
 *
 * 태그를 쓰지 않는 대신 admin 큐에 고정된 요청 하나를 미리 잡아 둔다.
 * 보통 요청의 init_request 가 하는 일과 같지만, 태그셋이 관여하지 않아
 * 여기서 직접 한다.
 *
 * 다이제스트 자리까지 포함해 잡는 것도 같은 이유다 -- 이 PDU 도 다른
 * 명령과 똑같이 선로에 나간다.
 *
 * 실행 컨텍스트: admin 큐 구성. 잠들 수 있다.
 *
 * 호출 체인: nvme_tcp_configure_admin_queue → [이 함수]
 */
static int nvme_tcp_alloc_async_req(struct nvme_tcp_ctrl *ctrl)
{
	struct nvme_tcp_queue *queue = &ctrl->queues[0];	/* [한국어] AEN 은 admin 큐로만 나간다 */
	struct nvme_tcp_request *async = &ctrl->async_req;
	u8 hdgst = nvme_tcp_hdgst_len(queue);	/* [한국어] 이 PDU 도 다른 명령과 똑같이 다이제스트가 붙는다 */
	...
	async->queue = &ctrl->queues[0];	/* [한국어] 전송 경로가 이 포인터로 큐를 찾는다 */
	return 0;
}

/*
 * [한국어]
 * nvme_tcp_free_queue - 큐가 잡은 소켓과 버퍼를 반납한다
 *
 * @nctrl: 컨트롤러
 * @qid:   반납할 큐 번호
 * @return: 없음
 ...
 * 오류 복구와 삭제가 겹치는 것이 흔하기 때문이다.
 *
 * noreclaim 구간이 이 함수의 미묘한 지점이다. 소켓을 닫으면 TCP 가
 * 메모리를 반납하는 과정에서 회수(reclaim)를 유발할 수 있고, 그
 * 회수가 이 컨트롤러로 I/O 를 내려 보내면 지금 해체 중인 큐를 쓰려
 * 하게 된다. memalloc_noreclaim 이 그 재귀를 끊는다.
 *
 * 위 영어 주석대로 소켓 자체는 fput 이 놓는다 -- 핸드셰이크를 사용자
 * 공간 데몬에 넘기기 위해 파일로 감쌌기 때문에 파일 참조가 소유자다.
 *
 * 실행 컨텍스트: 큐 해체. 잠들 수 있다.
 *
 * 호출 체인: nvme_tcp_free_io_queues / _destroy_admin_queue → [이 함수]
 */
static void nvme_tcp_free_queue(struct nvme_ctrl *nctrl, int qid)
{
	struct nvme_tcp_ctrl *ctrl = to_tcp_ctrl(nctrl);
	struct nvme_tcp_queue *queue = &ctrl->queues[qid];
	unsigned int noreclaim_flag;	/* [한국어] 아래 재귀 방지 구간의 저장 값 */

	if (!test_and_clear_bit(NVME_TCP_Q_ALLOCATED, &queue->flags))	/* [한국어] 복구와 삭제가 겹쳐도 한 번만 해제한다 */
		return;

	page_frag_cache_drain(&queue->pf_cache);	/* [한국어] 이 큐가 쓰던 페이지 조각 캐시를 비운다 */

	noreclaim_flag = memalloc_noreclaim_save();	/* [한국어] 소켓을 닫다 회수가 일어나면 그 I/O 가 지금 해체 중인 큐로 온다 */
	/* ->sock will be released by fput() */
	fput(queue->sock->file);	/* [한국어] 핸드셰이크를 데몬에 넘기려 파일로 감쌌으므로 파일 참조가 소유자다 */
	queue->sock = NULL;	/* [한국어] 콜백들이 이 포인터를 보므로 반드시 지운다 */
	memalloc_noreclaim_restore(noreclaim_flag);	/* [한국어] 재귀 위험 구간이 끝났다 */

	kfree(queue->pdu);	/* [한국어] 수신 PDU 조립 버퍼 */
...
static bool nvme_tcp_admin_queue(struct nvme_tcp_queue *queue)	/* [한국어] 0번이면 admin — 소켓 옵션과 큐 크기가 달라진다 */
{
	return nvme_tcp_queue_id(queue) == 0;
}

static bool nvme_tcp_default_queue(struct nvme_tcp_queue *queue)	/* [한국어] 이 큐가 기본(읽기·쓰기 공용) 종류인가 */
{
	struct nvme_tcp_ctrl *ctrl = queue->ctrl;	/* [한국어] 큐 종류별 개수가 컨트롤러에 있다 */
	int qid = nvme_tcp_queue_id(queue);

	return !nvme_tcp_admin_queue(queue) &&	/* [한국어] admin 을 뺀 뒤 번호가 기본 큐 구간에 드는지 본다 */
...
/*
 * [한국어]
 * nvme_tcp_start_tls - 사용자 공간 데몬에 TLS 핸드셰이크를 맡기고 기다린다
 *
 * @nctrl: 컨트롤러
 * @queue: 핸드셰이크할 큐
 * @pskid: 쓸 PSK 의 키 일련번호
 * @return: 0 이면 성공, 음수 errno
 *
 * 커널이 직접 핸드셰이크를 하지 않는 것이 이 함수의 전제다. TLS 프로토콜
 * 협상은 복잡하고 정책이 많아 사용자 공간 데몬(tlshd)이 하고, 커널은
 * 넷링크로 요청을 올린 뒤 결과만 받는다. 소켓을 파일로 감싸 두는 것도
 * 그 데몬에게 넘기기 위해서다.
 *
 * 그래서 타임아웃이 필수다. 데몬이 없거나 죽어 있으면 응답이 영영 오지
 * 않는다. tls_err 을 -EOPNOTSUPP 로 미리 채워 두는 것도 같은 이유 --
 * 콜백이 한 번도 불리지 않은 경우와 구별하기 위해서다.
 *
 * 타임아웃 시 handshake_cancel 을 부르는 것이 중요하다. 그러지 않으면
 * 데몬이 나중에 완료 콜백을 부르는데, 그때는 이미 이 큐가 해체되고
 * 있을 수 있다.
 *
 * 실행 컨텍스트: 연결 경로. 오래 잠든다.
 *
 * 호출 체인:
 *   nvme_tcp_alloc_queue → [이 함수] → tls_client_hello_psk
 *     → (사용자 공간 tlshd) → nvme_tcp_tls_done
 */
static int nvme_tcp_start_tls(struct nvme_ctrl *nctrl,
			      struct nvme_tcp_queue *queue,
			      key_serial_t pskid)
{
	int qid = nvme_tcp_queue_id(queue);	/* [한국어] 로그에만 쓴다 */
	int ret;
	struct tls_handshake_args args;	/* [한국어] 넷링크로 데몬에 넘길 요청 */
	unsigned long tmo = tls_handshake_timeout * HZ;	/* [한국어] 데몬이 없으면 응답이 영영 오지 않는다 */
	key_serial_t keyring = nvme_keyring_id();	/* [한국어] 기본은 NVMe 전용 키링 */

	dev_dbg(nctrl->device, "queue %d: start TLS with key %x\n",	/* [한국어] 어느 키로 시도하는지 남긴다 */
		qid, pskid);
	memset(&args, 0, sizeof(args));	/* [한국어] 안 쓰는 필드가 0 이어야 한다 */
	args.ta_sock = queue->sock;	/* [한국어] 데몬이 이 소켓 위에서 핸드셰이크한다 */
	args.ta_done = nvme_tcp_tls_done;	/* [한국어] 끝나면 이 콜백이 결과를 남긴다 */
	args.ta_data = queue;	/* [한국어] 그 콜백이 큐를 되찾는 통로 */
	args.ta_my_peerids[0] = pskid;	/* [한국어] 쓸 PSK */
	args.ta_num_peerids = 1;	/* [한국어] 하나만 제안한다 */
	if (nctrl->opts->keyring)	/* [한국어] 사용자가 키링을 지정했다 */
		keyring = key_serial(nctrl->opts->keyring);
	args.ta_keyring = keyring;	/* [한국어] 데몬이 이 키링에서 키를 찾는다 */
	args.ta_timeout_ms = tls_handshake_timeout * 1000;	/* [한국어] 데몬 쪽 상한도 함께 준다 */
	queue->tls_err = -EOPNOTSUPP;	/* [한국어] 콜백이 한 번도 안 불린 경우와 구별하기 위해 */
	init_completion(&queue->tls_complete);	/* [한국어] 콜백이 이것으로 깨운다 */
	ret = tls_client_hello_psk(&args, GFP_KERNEL);	/* [한국어] 넷링크로 요청을 올린다 — 비동기다 */
	if (ret) {	/* [한국어] 요청 자체를 올리지 못했다 */
		dev_err(nctrl->device, "queue %d: failed to start TLS: %d\n",	/* [한국어] 데몬이 없거나 넷링크가 막혔다 */
			qid, ret);
		return ret;
	}
	ret = wait_for_completion_interruptible_timeout(&queue->tls_complete, tmo);	/* [한국어] 상한을 두고 기다린다 */
	if (ret <= 0) {	/* [한국어] 타임아웃이거나 시그널로 깨어났다 */
		if (ret == 0)	/* [한국어] 0 은 시간이 다 됐다는 뜻 */
			ret = -ETIMEDOUT;

		dev_err(nctrl->device,
			"queue %d: TLS handshake failed, error %d\n",
			qid, ret);
		tls_handshake_cancel(queue->sock->sk);	/* [한국어] 취소하지 않으면 데몬이 나중에 해체된 큐로 콜백을 부른다 */
	} else {
		if (queue->tls_err) {	/* [한국어] 핸드셰이크는 끝났지만 실패했다 */
			dev_err(nctrl->device,
				"queue %d: TLS handshake complete, error %d\n",
				qid, queue->tls_err);
		} else {
			dev_dbg(nctrl->device,	/* [한국어] 성공 */
				"queue %d: TLS handshake complete\n", qid);
		}
		ret = queue->tls_err;	/* [한국어] 콜백이 남긴 결과가 최종 답이다 */
	}
	return ret;
}
...
/*
 * [한국어]
 * nvme_tcp_alloc_queue - 큐 하나의 소켓을 만들어 연결하고 프로토콜을 협상한다
 *
 * @nctrl: 컨트롤러
 * @qid:   큐 번호. 0이면 admin.
 * @pskid: TLS 를 쓸 경우의 PSK 키 번호. 0이면 평문.
 * @return: 0 이면 성공, 음수 errno
 *
 * 소켓 생성부터 ICReq/ICResp 협상까지가 한 함수에 있다. 소켓 옵션마다
 * 이유가 다르다.
 *
 * syncnt=1 은 재시도를 한 번으로 줄여 연결 실패를 빨리 알아채기 위한
 * 것이고, nodelay 는 명령 PDU 가 Nagle 알고리즘에 붙들리지 않게 한다 --
 * 작은 PDU 를 모아 보내면 지연이 그대로 I/O 지연이 된다.
 *
 * no_linger 의 이유는 위 영어 주석에 있다. 연결이 끊겼을 때 TCP 가
 * 재전송을 오래 시도하며 소켓을 붙들면, 그동안 재연결을 시작할 수 없다.
 *
 * sk_allocation 을 GFP_ATOMIC 으로, sk_set_memalloc 을 거는 것은 이
 * 소켓이 스왑 경로에 놓일 수 있기 때문이다. 메모리가 부족해 스왑이
 * 필요한 상황에서 그 스왑을 나르는 소켓이 메모리를 기다리면 교착이 된다.
 *
 * 수신 버퍼를 hdgst 길이까지 포함해 잡는 것에 주의할 것. 다이제스트가
 * 켜지면 헤더 뒤에 4바이트가 더 붙고, 그것까지 한 번에 받아야 한다.
 *
 * 실행 컨텍스트: 연결/재연결 경로. 오래 잠든다.
 *
 * 호출 체인:
 *   nvme_tcp_configure_admin_queue / _io_queues → [이 함수]
 *     → kernel_connect → nvme_tcp_start_tls → nvme_tcp_init_connection
 */
				...
				key_serial_t pskid)
{
	struct nvme_tcp_ctrl *ctrl = to_tcp_ctrl(nctrl);
	struct nvme_tcp_queue *queue = &ctrl->queues[qid];
	int ret, rcv_pdu_size;	/* [한국어] 결과와 수신 버퍼 크기 */
	struct file *sock_file;	/* [한국어] 소켓을 파일로 감싼 것 — 데몬에 넘기기 위해 */

	mutex_init(&queue->queue_lock);	/* [한국어] stop/start 가 겹치는 것을 막는다 */
	queue->ctrl = ctrl;	/* [한국어] 콜백들이 이 포인터로 컨트롤러를 찾는다 */
	init_llist_head(&queue->req_list);	/* [한국어] 락 없는 제출 대기열 */
	INIT_LIST_HEAD(&queue->send_list);	/* [한국어] 전송 워크만 만지는 목록 */
	mutex_init(&queue->send_mutex);	/* [한국어] 전송을 한 번에 하나만 돌게 한다 */
	INIT_WORK(&queue->io_work, nvme_tcp_io_work);	/* [한국어] 송수신을 함께 처리하는 워크 */

	if (qid > 0)	/* [한국어] I/O 큐는 컨트롤러가 알린 캡슐 길이를 쓴다 */
		queue->cmnd_capsule_len = nctrl->ioccsz * 16;
	else
		queue->cmnd_capsule_len = sizeof(struct nvme_command) +	/* [한국어] admin 은 스펙 고정값이다 */
						NVME_TCP_ADMIN_CCSZ;

	ret = sock_create_kern(current->nsproxy->net_ns,	/* [한국어] 현재 네트워크 네임스페이스에서 만든다 */
	...
	sock_file = sock_alloc_file(queue->sock, O_CLOEXEC, NULL);	/* [한국어] 파일로 감싸야 TLS 데몬에 넘길 수 있다 */
	if (IS_ERR(sock_file)) {
		ret = PTR_ERR(sock_file);
		goto err_destroy_mutex;	/* [한국어] 파일이 없으면 소켓은 sock_release 가 아니라 이 경로가 놓는다 */
	}

	sk_net_refcnt_upgrade(queue->sock->sk);	/* [한국어] 커널 소켓이지만 네임스페이스 참조를 정식으로 든다 */
	nvme_tcp_reclassify_socket(queue->sock);	/* [한국어] lockdep 이 커널 소켓과 사용자 소켓을 구별하게 한다 */

	/* Single syn retry */
	tcp_sock_set_syncnt(queue->sock->sk, 1);	/* [한국어] 위 영어 주석대로 재시도 한 번 — 연결 실패를 빨리 알아챈다 */

	/* Set TCP no delay */
	tcp_sock_set_nodelay(queue->sock->sk);	/* [한국어] 작은 PDU 를 모아 보내면 그 지연이 I/O 지연이 된다 */
	 ...
	 */
	sock_no_linger(queue->sock->sk);	/* [한국어] 위 영어 주석대로 재전송에 붙들려 재연결이 막히는 것을 피한다 */

	if (so_priority > 0)	/* [한국어] 사용자가 지정했을 때만 */
		sock_set_priority(queue->sock->sk, so_priority);

	/* Set socket type of service */
	if (nctrl->opts->tos >= 0)	/* [한국어] 음수는 "트랜스포트 기본값"이라는 표시다 */
		ip_sock_set_tos(queue->sock->sk, nctrl->opts->tos);

	/* Set 10 seconds timeout for icresp recvmsg */
	queue->sock->sk->sk_rcvtimeo = 10 * HZ;	/* [한국어] 위 영어 주석대로 ICResp 를 기다리는 데만 쓰는 상한 */

	queue->sock->sk->sk_allocation = GFP_ATOMIC;	/* [한국어] 이 소켓이 스왑을 나를 수 있어 메모리를 기다리면 교착이 된다 */
	queue->sock->sk->sk_use_task_frag = false;	/* [한국어] 같은 이유로 태스크 조각 캐시를 쓰지 않는다 */
	queue->io_cpu = WORK_CPU_UNBOUND;	/* [한국어] 실제 CPU 는 연결이 선 뒤에 정한다 */
	queue->request = NULL;	/* [한국어] 전송 중인 요청 없음 */
	queue->data_remaining = 0;	/* [한국어] 수신 상태를 처음으로 */
	queue->ddgst_remaining = 0;
	queue->pdu_remaining = 0;
	queue->pdu_offset = 0;
	sk_set_memalloc(queue->sock->sk);	/* [한국어] 메모리 압박에서도 이 소켓은 예비 풀을 쓸 수 있다 */

	if (nctrl->opts->mask & NVMF_OPT_HOST_TRADDR) {	/* [한국어] 사용자가 출구 주소를 고정했다 */
	...
	if (nctrl->opts->mask & NVMF_OPT_HOST_IFACE) {	/* [한국어] 주소 대신 인터페이스로 출구를 고정했다 */
		char *iface = nctrl->opts->host_iface;
		sockptr_t optval = KERNEL_SOCKPTR(iface);	/* [한국어] 커널 포인터임을 명시해야 setsockopt 이 복사하지 않는다 */
	...
	queue->hdr_digest = nctrl->opts->hdr_digest;	/* [한국어] 희망 값 — 실제 사용 여부는 ICResp 가 정한다 */
	queue->data_digest = nctrl->opts->data_digest;

	rcv_pdu_size = sizeof(struct nvme_tcp_rsp_pdu) +	/* [한국어] 가장 큰 헤더에 맞춰 잡는다 */
			nvme_tcp_hdgst_len(queue);
	queue->pdu = kmalloc(rcv_pdu_size, GFP_KERNEL);	/* [한국어] 헤더를 조각조각 받아 여기서 조립한다 */
	if (!queue->pdu) {
		ret = -ENOMEM;
		goto err_sock;
	}
	...
	ret = kernel_connect(queue->sock, (struct sockaddr_unsized *)&ctrl->addr,	/* [한국어] 3-way 핸드셰이크가 여기서 끝난다 */
		sizeof(ctrl->addr), 0);
	if (ret) {
		dev_err(nctrl->device,
	...
	if (nvme_tcp_tls_configured(nctrl) && pskid) {	/* [한국어] 위 영어 주석대로 PSK 가 있을 때만 */
		ret = nvme_tcp_start_tls(nctrl, queue, pskid);	/* [한국어] 이 뒤로는 모든 바이트가 TLS 를 거친다 */
		if (ret)
			goto err_init_connect;
	}

	ret = nvme_tcp_init_connection(queue);	/* [한국어] ICReq/ICResp 로 다이제스트와 데이터 정렬을 협상한다 */
	if (ret)
		goto err_init_connect;	/* [한국어] TLS 나 ICReq 협상이 실패했다 */

	set_bit(NVME_TCP_Q_ALLOCATED, &queue->flags);	/* [한국어] 해체 경로가 이 비트를 보고 정리 대상으로 삼는다 */

	return 0;	/* [한국어] 소켓이 연결되고 프로토콜이 협상됐다. NVMe Connect 는 start_queue 가 보낸다 */

err_init_connect:
	kernel_sock_shutdown(queue->sock, SHUT_RDWR);	/* [한국어] 상대에게도 끊겼음을 알린다 */
err_rcv_pdu:
	kfree(queue->pdu);	/* [한국어] 수신 조립 버퍼 */
err_sock:
	/* ->sock will be released by fput() */
	fput(queue->sock->file);	/* [한국어] 위 영어 주석대로 파일 참조가 소켓의 소유자다 */
	queue->sock = NULL;	/* [한국어] 콜백들이 이 포인터를 본다 */
err_destroy_mutex:
	mutex_destroy(&queue->send_mutex);
	mutex_destroy(&queue->queue_lock);
	return ret;
...
/*
 * [한국어]
 * nvme_tcp_restore_sock_ops - 소켓 콜백을 원래대로 되돌린다
 *
 * @queue: 대상 큐
 * @return: 없음
 *
 * sk_user_data 를 먼저 지우는 것이 순서의 요점이다. 아직 돌고 있는
 * 콜백이 그 값을 읽으므로, NULL 을 본 콜백은 아무것도 하지 않고 물러난다.
 * 쓰기 락을 잡는 것도 그 때문이다 -- 콜백들이 읽기 락을 잡으므로,
 * 이 구간이 끝나면 어느 콜백도 옛 값을 보지 않는다.
 *
 * 실행 컨텍스트: 큐 해체. 잠들지 않는다.
 *
 * 호출 체인: __nvme_tcp_stop_queue → [이 함수]
 */
static void nvme_tcp_restore_sock_ops(struct nvme_tcp_queue *queue)
{
	struct socket *sock = queue->sock;

	write_lock_bh(&sock->sk->sk_callback_lock);	/* [한국어] 콜백들은 읽기 락을 잡는다 — 이 구간이 끝나면 옛 값을 보는 콜백이 없다 */
	sock->sk->sk_user_data  = NULL;	/* [한국어] 먼저 지워야 아직 도는 콜백이 물러난다 */
	sock->sk->sk_data_ready = queue->data_ready;	/* [한국어] 저장해 둔 원래 콜백들 */
...
/*
 * [한국어]
 * __nvme_tcp_stop_queue - 소켓을 닫고 워크가 끝나기를 기다린다
 *
 * @queue: 대상 큐
 * @return: 없음
 *
 * 세 줄의 순서가 전부다. shutdown 이 대기 중인 송수신을 깨워 오류로
 * 반환시키고, 콜백을 되돌려 새 깨움이 없게 하고, 그다음 워크가 끝나기를
 * 기다린다.
 *
 * cancel_work_sync 가 마지막인 이유: 앞의 두 단계 없이 기다리면 워크가
 * 소켓에서 블로킹된 채로 영영 끝나지 않는다.
 *
 * 실행 컨텍스트: 큐 해체. 오래 잠들 수 있다.
 *
 * 호출 체인: nvme_tcp_stop_queue_nowait → [이 함수]
 */
static void __nvme_tcp_stop_queue(struct nvme_tcp_queue *queue)
{
	kernel_sock_shutdown(queue->sock, SHUT_RDWR);	/* [한국어] 대기 중인 송수신을 깨워 오류로 반환시킨다 */
	nvme_tcp_restore_sock_ops(queue);	/* [한국어] 새 깨움이 오지 않게 한다 */
...
static void nvme_tcp_stop_queue_nowait(struct nvme_ctrl *nctrl, int qid)	/* [한국어] NVMe/TCP 큐·PDU·소켓 경로 헬퍼 */
{
	...
	if (!test_bit(NVME_TCP_Q_ALLOCATED, &queue->flags))	/* [한국어] 만들어지지 않은 큐다 */
		return;

	if (test_and_clear_bit(NVME_TCP_Q_IO_CPU_SET, &queue->flags))	/* [한국어] 이 큐가 차지하던 CPU 몫을 돌려준다 */
		atomic_dec(&nvme_tcp_cpu_queues[queue->io_cpu]);

	mutex_lock(&queue->queue_lock);	/* [한국어] 복구와 삭제가 동시에 들어올 수 있다 */
	if (test_and_clear_bit(NVME_TCP_Q_LIVE, &queue->flags))	/* [한국어] 실제 종료는 한 번만 */
		__nvme_tcp_stop_queue(queue);
	/* Stopping the queue will disable TLS */
	queue->tls_enabled = false;	/* [한국어] 위 영어 주석대로 재연결에서 다시 협상해야 한다 */
	mutex_unlock(&queue->queue_lock);
}

/*
 * [한국어]
 * nvme_tcp_wait_queue - 소켓의 송신 버퍼가 비기를 기다린다
 *
 * @nctrl: 컨트롤러
 * @qid:   대상 큐
 * @return: 없음
 *
 * stop 이 소켓을 닫아도 커널의 송신 큐에 아직 바이트가 남아 있을 수 있다.
 * 그 바이트가 참조하는 페이지는 요청의 것이므로, 비기를 기다리지 않고
 * 요청을 반납하면 네트워크 스택이 반환된 메모리를 전송한다.
 *
 * 상한을 두는 것은 상대가 아예 응답하지 않는 경우 때문이다. 200ms 를
 * 넘기면 경고만 남기고 넘어간다 -- 영원히 기다리는 것보다는 낫다.
 *
 * stop 과 wait 을 나눈 이유는 큐가 많을 때다. 전부 stop 한 뒤 전부
 * 기다리면 대기가 겹쳐 전체 시간이 큐 수에 비례하지 않는다.
 *
 * 실행 컨텍스트: 큐 해체. 잠든다.
 *
 * 호출 체인: nvme_tcp_stop_queue / _stop_io_queues → [이 함수]
 */
static void nvme_tcp_wait_queue(struct nvme_ctrl *nctrl, int qid)
{
	struct nvme_tcp_ctrl *ctrl = to_tcp_ctrl(nctrl);
	struct nvme_tcp_queue *queue = &ctrl->queues[qid];
	int timeout = 100;	/* [한국어] 2ms 씩 100회 — 200ms 상한 */

	while (timeout > 0) {
		if (!test_bit(NVME_TCP_Q_ALLOCATED, &queue->flags) ||	/* [한국어] 이미 해제됐거나 */
		    !sk_wmem_alloc_get(queue->sock->sk))
			return;	/* [한국어] 송신 버퍼가 비었다 — 이제 요청 페이지를 반납해도 안전하다 */
		msleep(2);	/* [한국어] 짧게 자며 다시 확인한다 */
		timeout -= 2;
	}
	dev_warn(nctrl->device,	/* [한국어] 상대가 응답하지 않는다 — 영원히 기다리는 것보다는 낫다 */
...
/*
 * [한국어]
 * nvme_tcp_setup_sock_ops - 소켓 콜백을 이 드라이버 것으로 바꾼다
 *
 * @queue: 대상 큐
 * @return: 없음
 *
 * 원래 콜백을 저장한 뒤 우리 것을 거는 것이 요점이다. 저장해 두는 이유는
 * 두 가지 -- 해체 시 되돌려야 하고, TLS 인 경우 write_space 에서 원래
 * 것을 한 번 더 불러야 하기 때문이다.
 *
 * sk_user_data 를 먼저 세우는 것도 순서다. 콜백을 걸자마자 불릴 수
 * 있으므로, 그때 큐 포인터가 이미 있어야 한다.
 *
 * sk_ll_usec 는 busy poll 힌트다. 1마이크로초라도 스핀하면 인터럽트
 * 왕복을 피할 수 있는 경우가 있다.
 *
 * 실행 컨텍스트: 큐 시작. 잠들지 않는다.
 *
 * 호출 체인: nvme_tcp_start_queue → [이 함수]
 */
static void nvme_tcp_setup_sock_ops(struct nvme_tcp_queue *queue)
{
	write_lock_bh(&queue->sock->sk->sk_callback_lock);	/* [한국어] 콜백이 바뀌는 동안 아무도 읽지 못하게 */
	queue->sock->sk->sk_user_data = queue;	/* [한국어] 콜백을 걸자마자 불릴 수 있어 먼저 세운다 */
	queue->state_change = queue->sock->sk->sk_state_change;	/* [한국어] 해체 시 되돌리려고 저장한다 */
	queue->data_ready = queue->sock->sk->sk_data_ready;
	queue->write_space = queue->sock->sk->sk_write_space;	/* [한국어] TLS 경로가 이것을 한 번 더 부른다 */
	queue->sock->sk->sk_data_ready = nvme_tcp_data_ready;
	queue->sock->sk->sk_state_change = nvme_tcp_state_change;
	queue->sock->sk->sk_write_space = nvme_tcp_write_space;
#ifdef CONFIG_NET_RX_BUSY_POLL	/* [한국어] busy poll 을 켠 빌드에서만 */
	queue->sock->sk->sk_ll_usec = 1;	/* [한국어] 잠깐이라도 스핀하면 인터럽트 왕복을 피할 수 있다 */
#endif
...
static void nvme_tcp_free_admin_queue(struct nvme_ctrl *ctrl)	/* [한국어] NVMe/TCP 큐·PDU·소켓 경로 헬퍼 */
{
	if (to_tcp_ctrl(ctrl)->async_req.pdu) {	/* [한국어] AEN 요청을 잡아 둔 경우에만 */
		...
		to_tcp_ctrl(ctrl)->async_req.pdu = NULL;	/* [한국어] 재연결에서 이중 해제가 되지 않도록 */
	}
...
static void nvme_tcp_free_io_queues(struct nvme_ctrl *ctrl)	/* [한국어] NVMe/TCP 큐·PDU·소켓 경로 헬퍼 */
{
	...
	for (i = 1; i < ctrl->queue_count; i++)	/* [한국어] 0번은 admin 이라 건너뛴다 */
...
static void nvme_tcp_stop_io_queues(struct nvme_ctrl *ctrl)	/* [한국어] NVMe/TCP 큐·PDU·소켓 경로 헬퍼 */
{
	...
	for (i = 1; i < ctrl->queue_count; i++)	/* [한국어] 먼저 모두 닫고 */
		nvme_tcp_stop_queue_nowait(ctrl, i);
	for (i = 1; i < ctrl->queue_count; i++)	/* [한국어] 그다음 모두 기다린다 — 대기가 겹쳐 전체 시간이 줄어든다 */
				    ...
				    int first, int last)
{
	...
	for (i = first; i < last; i++) {	/* [한국어] 구간을 받는 이유: 재연결에서 큐가 늘면 새 구간만 시작한다 */
...
out_stop_queues:
	for (i--; i >= first; i--)	/* [한국어] 실패한 큐는 start_queue 가 이미 닫았다 */
...
static int nvme_tcp_alloc_io_queues(struct nvme_ctrl *ctrl)	/* [한국어] NVMe/TCP 큐·PDU·소켓 경로 헬퍼 */
{
	...
	nr_io_queues = nvmf_nr_io_queues(ctrl->opts);	/* [한국어] 사용자가 요청한 세 종류의 합 */
	ret = nvme_set_queue_count(ctrl, &nr_io_queues);	/* [한국어] 컨트롤러가 허락한 수로 덮어쓴다 */
	if (ret)
		return ret;

	if (nr_io_queues == 0) {	/* [한국어] I/O 큐 없이는 쓸모가 없다 */
		dev_err(ctrl->device,
			"unable to set any I/O queues\n");
		return -ENOMEM;
	}

	ctrl->queue_count = nr_io_queues + 1;	/* [한국어] admin 몫을 더한다 */
	dev_info(ctrl->device,	/* [한국어] 실제로 몇 개가 됐는지 남긴다 — 요청과 다를 수 있다 */
		"creating %d I/O queues.\n", nr_io_queues);

	nvmf_set_io_queues(ctrl->opts, nr_io_queues,	/* [한국어] 허락받은 수를 읽기·쓰기·폴링으로 나눈다 */
...
/*
 * [한국어]
 * nvme_tcp_teardown_admin_queue - admin 큐를 접는다
 *
 * @ctrl:   컨트롤러
 * @remove: 태그셋까지 없앨 것인가. 재연결이면 false.
 * @return: 없음
 *
 * quiesce → sync → stop → cancel 이 이 드라이버의 해체 관용구다. 새
 * 명령을 막고, 진행 중 완료 처리를 배출시키고, 소켓을 닫고, 남은 요청을
 * 취소로 완료시켜 태그를 반납한다.
 *
 * remove 일 때만 unquiesce 하는 이유: 태그셋을 없애려면 멈춘 큐에 남은
 * 요청이 빠져나가야 한다. 재연결이라면 멈춘 채로 두어야 새 세션이 설
 * 때까지 요청이 기다린다.
 *
 * TLS PSK 를 지우는 것이 이 함수만의 일이다. concat 으로 유도한 키는
 * 이 세션에만 유효하므로, 다음 연결은 인증부터 다시 해 새 키를 만들어야
 * 한다.
 *
 * 실행 컨텍스트: 오류 복구/삭제. 오래 잠든다.
 *
 * 호출 체인: nvme_tcp_teardown_ctrl → [이 함수]
 */
static void nvme_tcp_teardown_admin_queue(struct nvme_ctrl *ctrl,
		bool remove)
{
	nvme_quiesce_admin_queue(ctrl);	/* [한국어] 새 명령을 막는다 */
	blk_sync_queue(ctrl->admin_q);	/* [한국어] 진행 중 완료 처리를 배출시킨다 */
	nvme_tcp_stop_queue(ctrl, 0);	/* [한국어] 소켓을 닫고 송신 버퍼가 비기를 기다린다 */
	nvme_cancel_admin_tagset(ctrl);	/* [한국어] 남은 요청을 취소로 완료시켜 태그를 반납한다 */
	if (remove) {	/* [한국어] 태그셋까지 없앨 때만 */
		nvme_unquiesce_admin_queue(ctrl);	/* [한국어] 멈춘 큐에 남은 요청이 태그셋 해제를 막는다 */
		nvme_remove_admin_tag_set(ctrl);
	}
	nvme_tcp_free_admin_queue(ctrl);	/* [한국어] AEN 요청과 소켓을 반납한다 */
	if (ctrl->tls_pskid) {	/* [한국어] concat 으로 유도한 키가 있다 */
		dev_dbg(ctrl->device, "Wipe negotiated TLS_PSK %08x\n",	/* [한국어] 일련번호만 남긴다 — 키 내용은 로그에 흘리지 않는다 */
			ctrl->tls_pskid);
		ctrl->tls_pskid = 0;	/* [한국어] 이 세션에만 유효하므로 다음 연결은 인증부터 다시 한다 */
	}
}

/*
 * [한국어]
 * nvme_tcp_teardown_io_queues - I/O 큐를 모두 접는다
 *
 * @ctrl:   컨트롤러
 * @remove: 태그셋까지 없앨 것인가
 * @return: 없음
 *
 * admin 과 같은 관용구를 I/O 큐 전체에 적용한다. queue_count <= 1 검사가
 * 앞에 있는 것은 admin 만 세운 상태에서도 이 경로가 불릴 수 있기 때문이다.
 *
 * sync 가 quiesce 다음인 것이 중요하다. 멈추기 전에 기다리면 그 사이
 * 들어온 요청 때문에 끝나지 않는다.
 *
 * 실행 컨텍스트: 오류 복구/삭제. 오래 잠든다.
 *
...
static void nvme_tcp_teardown_io_queues(struct nvme_ctrl *ctrl,
		bool remove)
{
	if (ctrl->queue_count <= 1)	/* [한국어] admin 만 세운 상태에서도 불린다 */
		return;
	nvme_quiesce_io_queues(ctrl);	/* [한국어] 새 I/O 를 막는다 */
	nvme_sync_io_queues(ctrl);	/* [한국어] 멈춘 뒤에 기다려야 끝난다 */
	nvme_tcp_stop_io_queues(ctrl);	/* [한국어] 모두 닫고 모두 기다린다 */
...
/*
 * [한국어]
 * nvme_tcp_reconnect_or_remove - 다시 시도할지 포기할지 정한다
 *
 * @ctrl:   컨트롤러
 * @status: 직전 시도가 남긴 오류
 * @return: 없음
 *
 * 연결 시도가 실패할 때마다 불리는 갈림길이다. CONNECTING 이 아니면
 * 리셋이나 삭제가 이미 이 컨트롤러를 가져갔다는 뜻이므로 손대지 않는다.
 *
 * WARN 이 NEW 와 LIVE 만 잡는 것에 주의할 것. 그 둘로 여기 오는 것은
 * 상태 기계가 깨진 것이지만, DELETING 이나 RESETTING 은 정상적인 경쟁이라
 * 조용히 물러난다. rdma.c 의 같은 이름 함수와 같은 구조다.
 *
 * 실행 컨텍스트: 연결/복구 워크. 예약만 한다.
 *
 * 호출 체인:
 *   nvme_tcp_reconnect_ctrl_work / _error_recovery_work → [이 함수]
 */
static void nvme_tcp_reconnect_or_remove(struct nvme_ctrl *ctrl,
		int status)
{
	enum nvme_ctrl_state state = nvme_ctrl_state(ctrl);	/* [한국어] 한 번만 읽어 아래 판단이 일관되게 한다 */

	/* If we are resetting/deleting then do nothing */
	if (state != NVME_CTRL_CONNECTING) {	/* [한국어] 위 영어 주석대로 다른 경로가 이 컨트롤러를 가져갔다 */
		WARN_ON_ONCE(state == NVME_CTRL_NEW || state == NVME_CTRL_LIVE);	/* [한국어] 그 둘은 상태 기계가 깨진 것이고, DELETING·RESETTING 은 정상 경쟁이다 */
		return;
	}

	if (nvmf_should_reconnect(ctrl, status)) {	/* [한국어] ctrl_loss_tmo 안이고 재시도할 만한 오류인가 */
		dev_info(ctrl->device, "Reconnecting in %d seconds...\n",	/* [한국어] 사용자가 장치가 왜 안 보이는지 알 수 있게 */
			ctrl->opts->reconnect_delay);
		queue_delayed_work(nvme_wq, &to_tcp_ctrl(ctrl)->connect_work,	/* [한국어] 간격을 두고 다시 시도한다 */
				ctrl->opts->reconnect_delay * HZ);
	} else {
		dev_info(ctrl->device, "Removing controller (%d)...\n",	/* [한국어] 왜 포기하는지 상태 코드와 함께 남긴다 */
			 status);
...
{
	return ctrl->opts->concat && ctrl->opts->tls_key && ctrl->tls_pskid;	/* [한국어] concat 으로 만든 키만 폐기 대상이다 — 사용자가 준 키는 건드리지 않는다 */
}
...
/*
 * [한국어]
 * nvme_tcp_teardown_ctrl - 컨트롤러를 계층 순서대로 내린다
 *
 * @ctrl:     컨트롤러
 ...
 * admin 큐를 통해 나가므로 순서를 바꾸면 보낼 통로가 없고, 그렇다고
 * 큐를 열어 두면 새 명령이 들어오므로 quiesce 만 걸어 둔다.
 *
 * 실행 컨텍스트: 삭제/리셋. 오래 잠든다.
 *
 * 호출 체인:
 *   nvme_tcp_delete_ctrl / _reset_ctrl_work → [이 함수]
 */
static void nvme_tcp_teardown_ctrl(struct nvme_ctrl *ctrl, bool shutdown)
{
	nvme_tcp_teardown_io_queues(ctrl, shutdown);	/* [한국어] 데이터 경로를 먼저 끊는다 */
	nvme_quiesce_admin_queue(ctrl);	/* [한국어] 새 admin 명령을 막되 큐는 살려 둔다 */
	nvme_disable_ctrl(ctrl, shutdown);	/* [한국어] 이 명령이 admin 큐로 나가므로 그 큐를 접기 전이어야 한다 */
...
static void nvme_tcp_complete_timed_out(struct request *rq)	/* [한국어] NVMe/TCP 큐·PDU·소켓 경로 헬퍼 */
{
	...
	nvme_tcp_stop_queue(ctrl, nvme_tcp_queue_id(req->queue));	/* [한국어] 먼저 닫아야 뒤늦은 완료가 이미 끝낸 요청을 건드리지 않는다 */
...
/*
 * [한국어]
 * nvme_tcp_map_data - 데이터를 어떻게 전달할지 SGL 서술자에 적는다
 *
 * @queue: 이 요청이 갈 큐
 * @rq:    대상 요청
 * @return: 항상 BLK_STS_OK
 *
 * 세 갈래가 NVMe/TCP 의 데이터 전달 방식 전부다.
 *
 * 데이터가 없으면 널 서술자. 쓰기이고 캡슐에 들어갈 만큼 작으면 인라인 --
 * 명령 PDU 뒤에 데이터를 붙여 한 번에 보내므로 왕복이 없다. 그 밖에는
 * 호스트 데이터 서술자를 적고, 대상이 R2T 로 요청할 때 보내거나(쓰기)
 * C2HData 로 받는다(읽기).
 *
 * 인라인 판정이 쓰기에만 걸리는 이유: 읽기는 애초에 보낼 데이터가 없다.
 *
 * 실행 컨텍스트: 제출 경로. 잠들지 않는다.
 *
 * 호출 체인: nvme_tcp_setup_cmd_pdu → [이 함수]
 */
static blk_status_t nvme_tcp_map_data(struct nvme_tcp_queue *queue,
			struct request *rq)
{
	struct nvme_tcp_request *req = blk_mq_rq_to_pdu(rq);
	struct nvme_tcp_cmd_pdu *pdu = nvme_tcp_req_cmd_pdu(req);	/* [한국어] 조립 중인 명령 PDU */
	struct nvme_command *c = &pdu->cmd;	/* [한국어] 그 안의 SQE */

	c->common.flags |= NVME_CMD_SGL_METABUF;	/* [한국어] TCP 는 PRP 를 쓰지 않는다 — 항상 SGL 이다 */

	if (!blk_rq_nr_phys_segments(rq))	/* [한국어] 데이터가 없는 명령 */
		nvme_tcp_set_sg_null(c);
	else if (rq_data_dir(rq) == WRITE &&	/* [한국어] 쓰기이고 캡슐에 들어갈 만큼 작으면 */
	    req->data_len <= nvme_tcp_inline_data_size(req))
		nvme_tcp_set_sg_inline(queue, c, req->data_len);
	else	/* [한국어] 그 밖에는 R2T(쓰기) 또는 C2HData(읽기)로 따로 나른다 */
...
/*
 * [한국어]
 * nvme_tcp_setup_cmd_pdu - 블록 요청을 명령 PDU 로 조립한다
 *
 * @ns: 네임스페이스(admin 이면 NULL)
 * @rq: 대상 요청
 * @return: BLK_STS_OK 이면 보낼 준비가 끝났다
 *
 * 상태를 통째로 초기화하는 것이 앞부분의 전부다. 요청은 재시도로 다시
 * 여기 올 수 있고, 그때 이전 전송의 진행 상태가 남아 있으면 중간부터
 * 보내려 한다.
 *
 * PDU 길이 계산이 뒷부분이다. hlen 은 헤더, hdgst 는 헤더 다이제스트,
 * pdu_len 은 인라인 데이터, ddgst 는 데이터 다이제스트 -- 넷을 더한
 * 것이 plen 이고, 대상은 그 값만큼 읽는다.
 *
 * pdo(데이터 오프셋)가 인라인이 있을 때만 채워지는 것에 주의할 것.
 * 데이터가 없으면 0 이며, 대상은 그것을 "데이터 없음"으로 읽는다.
 *
 * 실행 컨텍스트: blk-mq 제출. 잠들지 않는다.
 *
 * 호출 체인: nvme_tcp_queue_rq → [이 함수] → nvme_setup_cmd
 */
		...
		struct request *rq)
{
	...
	struct nvme_tcp_cmd_pdu *pdu = nvme_tcp_req_cmd_pdu(req);
	struct nvme_tcp_queue *queue = req->queue;
	u8 hdgst = nvme_tcp_hdgst_len(queue), ddgst = 0;	/* [한국어] 길이 계산에 들어갈 두 다이제스트 크기 */
	blk_status_t ret;

	ret = nvme_setup_cmd(ns, rq);	/* [한국어] 블록 요청을 SQE 로 번역한다 */
	if (ret)
		return ret;

	req->state = NVME_TCP_SEND_CMD_PDU;	/* [한국어] 전송 상태 기계의 시작점 */
	req->status = cpu_to_le16(NVME_SC_SUCCESS);	/* [한국어] 다이제스트 오류가 나면 수신 경로가 덮어쓴다 */
	req->offset = 0;	/* [한국어] 재시도로 다시 올 수 있어 진행 상태를 모두 지운다 */
	req->data_sent = 0;
	req->pdu_len = 0;
	req->pdu_sent = 0;
	req->h2cdata_left = 0;	/* [한국어] R2T 로 보낼 남은 양 */
	req->data_len = blk_rq_nr_phys_segments(rq) ?	/* [한국어] 세그먼트가 없으면 데이터도 없다 */
				blk_rq_payload_bytes(rq) : 0;
	req->curr_bio = rq->bio;	/* [한국어] 여러 bio 로 이어진 요청의 첫 번째 */
	if (req->curr_bio && req->data_len)	/* [한국어] 보낼 것이 있을 때만 반복자를 세운다 */
		nvme_tcp_init_iter(req, rq_data_dir(rq));

	if (rq_data_dir(rq) == WRITE &&	/* [한국어] 인라인으로 붙일 수 있는 쓰기라면 */
	    req->data_len <= nvme_tcp_inline_data_size(req))
		req->pdu_len = req->data_len;

	pdu->hdr.type = nvme_tcp_cmd;	/* [한국어] PDU 종류 */
	pdu->hdr.flags = 0;	/* [한국어] 아래에서 하나씩 세운다 */
	if (queue->hdr_digest)	/* [한국어] 협상 결과에 따라 */
		pdu->hdr.flags |= NVME_TCP_F_HDGST;
	if (queue->data_digest && req->pdu_len) {	/* [한국어] 인라인 데이터가 있을 때만 데이터 다이제스트가 붙는다 */
		pdu->hdr.flags |= NVME_TCP_F_DDGST;
		ddgst = nvme_tcp_ddgst_len(queue);	/* [한국어] 길이 계산에 반영한다 */
	}
	pdu->hdr.hlen = sizeof(*pdu);	/* [한국어] 헤더 길이 */
	pdu->hdr.pdo = req->pdu_len ? pdu->hdr.hlen + hdgst : 0;	/* [한국어] 데이터 시작 오프셋. 데이터가 없으면 0 이 "없음"을 뜻한다 */
	pdu->hdr.plen =	/* [한국어] 헤더+헤더다이제스트+데이터+데이터다이제스트 — 대상은 이만큼 읽는다 */
		cpu_to_le32(pdu->hdr.hlen + hdgst + req->pdu_len + ddgst);

	ret = nvme_tcp_map_data(queue, rq);	/* [한국어] 세 전달 방식 중 하나를 골라 SGL 에 적는다 */
	if (unlikely(ret)) {
	...
	return 0;	/* [한국어] 이제 대기열에 넣어 보내면 된다 */
}

/*
 * [한국어]
 * nvme_tcp_commit_rqs - 쌓아 둔 요청을 실제로 내보내게 한다
 *
 * @hctx: 대상 하드웨어 큐
 * @return: 없음
 *
 * blk-mq 가 여러 요청을 묶어 제출할 때, 마지막에 이것을 불러 "더 없다"고
 * 알린다. queue_request 는 last 가 아니면 워크를 깨우지 않으므로, 그
 * 묶음의 끝에서 한 번만 깨우면 된다 -- 요청마다 깨우는 것보다 훨씬 싸다.
 *
 * 실행 컨텍스트: blk-mq 제출. 잠들지 않는다.
 *
 * 호출 체인: blk-mq ops->commit_rqs → [이 함수]
 */
static void nvme_tcp_commit_rqs(struct blk_mq_hw_ctx *hctx)
{
	struct nvme_tcp_queue *queue = hctx->driver_data;

	if (!llist_empty(&queue->req_list))	/* [한국어] 묶음의 끝에서 한 번만 깨운다 — 요청마다 깨우는 것보다 훨씬 싸다 */
		queue_work_on(queue->io_cpu, nvme_tcp_wq, &queue->io_work);
}
...
static void nvme_tcp_map_queues(struct blk_mq_tag_set *set)	/* [한국어] NVMe/TCP 큐·PDU·소켓 경로 헬퍼 */
{
	...
	nvmf_map_queues(set, &ctrl->ctrl, ctrl->io_queues);	/* [한국어] rdma 도 쓰는 공통 배분 로직 */
}
...
/*
 * [한국어]
 * nvme_tcp_get_address - sysfs 의 address 속성에 출발지 주소까지 덧붙인다
 *
 * @ctrl: 컨트롤러
 * @buf:  출력 버퍼
 * @size: 그 크기
 * @return: 쓴 바이트 수
 *
 * 공통 부분(traddr, trsvcid 등)은 fabrics 코어가 만들고, 여기서는 실제로
 * 어느 로컬 주소에서 나갔는지를 더한다. 사용자가 host_traddr 을 주지
 * 않았다면 커널이 라우팅으로 골랐을 것이고, 그것이 무엇인지는 이 방법으로만
 * 알 수 있다.
 *
 * 개행을 하나 지우고 다시 쓰는 것은 두 조각을 한 줄로 잇기 위해서다.
 *
 * 큐가 살아 있을 때만 묻는다. 소켓이 없으면 출발지 주소도 없다.
 *
 * 실행 컨텍스트: sysfs 읽기. 잠들 수 있다.
 *
 * 호출 체인: sysfs show → ops->get_address → [이 함수]
 */
static int nvme_tcp_get_address(struct nvme_ctrl *ctrl, char *buf, int size)
{
	struct nvme_tcp_queue *queue = &to_tcp_ctrl(ctrl)->queues[0];	/* [한국어] admin 큐의 소켓에 물어본다 */
	struct sockaddr_storage src_addr;	/* [한국어] 커널이 실제로 고른 출발지 주소 */
	int ret, len;

	len = nvmf_get_address(ctrl, buf, size);	/* [한국어] 공통 부분은 fabrics 코어가 만든다 */

	if (!test_bit(NVME_TCP_Q_LIVE, &queue->flags))	/* [한국어] 소켓이 없으면 출발지 주소도 없다 */
		return len;

	mutex_lock(&queue->queue_lock);	/* [한국어] 읽는 동안 소켓이 사라지면 안 된다 */

	ret = kernel_getsockname(queue->sock, (struct sockaddr *)&src_addr);	/* [한국어] 라우팅이 고른 주소를 되묻는다 */
	if (ret > 0) {
		if (len > 0)	/* [한국어] 두 조각을 한 줄로 잇기 위해 */
...
static const struct blk_mq_ops nvme_tcp_mq_ops = {	/* [한국어] I/O 큐용 — poll 과 map_queues 가 있다 */
	.queue_rq	= nvme_tcp_queue_rq,	/* [한국어] 제출 핫패스 */
...
static const struct blk_mq_ops nvme_tcp_admin_mq_ops = {	/* [한국어] admin 큐용 */
	.queue_rq	= nvme_tcp_queue_rq,	/* [한국어] 제출 경로는 공유한다 */
	.complete	= nvme_complete_rq,
	.init_request	= nvme_tcp_init_request,
	.exit_request	= nvme_tcp_exit_request,
...
static const struct nvme_ctrl_ops nvme_tcp_ctrl_ops = {	/* [한국어] 코어가 이 트랜스포트를 부르는 통로 */
	.name			= "tcp",	/* [한국어] sysfs 와 로그에 보이는 이름 */
	.module			= THIS_MODULE,
...
nvme_tcp_existing_controller(struct nvmf_ctrl_options *opts)
{
	...
	list_for_each_entry(ctrl, &nvme_tcp_ctrl_list, list) {	/* [한국어] 전역 목록 — 컨트롤러 수가 적어 선형으로 충분하다 */
