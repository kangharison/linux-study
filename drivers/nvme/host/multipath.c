 ...
 */
static int multipath_param_set(const char *val, const struct kernel_param *kp)	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
{
...
MODULE_PARM_DESC(multipath,	/* [한국어] 모듈 파라미터/메타데이터 */
 ...
 */
static int multipath_always_on_set(const char *val,	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
		const struct kernel_param *kp)	/* [한국어] 지역/멤버 상태 — 상위 함수·구조 아키텍처 참고 */
{
	int ret;	/* [한국어] 파싱 결과 */
	...
	if (*arg)	/* [한국어] 아키텍처 가드 — 함수 헤드 문맥 참고 */
...
module_param_cb(multipath_always_on, &multipath_always_on_ops,	/* [한국어] 모듈 파라미터/메타데이터 */
		&multipath_always_on, 0444);	/* [한국어] always_on 파라미터 등록 */
MODULE_PARM_DESC(multipath_always_on,	/* [한국어] 모듈 파라미터/메타데이터 */
 ...
 */
static int nvme_set_iopolicy(const char *val, const struct kernel_param *kp)	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
{
	if (!val)	/* [한국어] 아키텍처 가드 — 함수 헤드 문맥 참고 */
		return -EINVAL;	/* [한국어] NULL 문자열 거부 */
	if (!strncmp(val, "numa", 4))	/* [한국어] 메모리/문자열 연산 */
		iopolicy = NVME_IOPOLICY_NUMA;	/* [한국어] 모듈 파라미터 문자열→NUMA 정책 */
	else if (!strncmp(val, "round-robin", 11))	/* [한국어] 메모리/문자열 연산 */
		iopolicy = NVME_IOPOLICY_RR;	/* [한국어] 라운드로빈 정책 — 경로 간 균등 분배 */
	else if (!strncmp(val, "queue-depth", 11))	/* [한국어] 메모리/문자열 연산 */
 ...
 */
static int nvme_get_iopolicy(char *buf, const struct kernel_param *kp)	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
{
	return sprintf(buf, "%s\n", nvme_iopolicy_names[iopolicy]);	/* [한국어] 현재 전역 정책 이름+개행 */
}

module_param_call(iopolicy, nvme_set_iopolicy, nvme_get_iopolicy,	/* [한국어] 모듈 파라미터/메타데이터 */
	&iopolicy, 0644);	/* [한국어] 기본 iopolicy 문자열 get/set (0644) */
MODULE_PARM_DESC(iopolicy,	/* [한국어] 모듈 파라미터/메타데이터 */
 ...
 */
void nvme_mpath_default_iopolicy(struct nvme_subsystem *subsys)	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
{
 ...
 */
void nvme_mpath_unfreeze(struct nvme_subsystem *subsys)	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
{
	...
	list_for_each_entry(h, &subsys->nsheads, entry)	/* [한국어] 모든 ns_head 에 freeze 연산 일괄 적용 */
		if (h->disk)	/* [한국어] 아키텍처 가드 — 함수 헤드 문맥 참고 */
 ...
 */
void nvme_mpath_wait_freeze(struct nvme_subsystem *subsys)	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
{
	struct nvme_ns_head *h;	/* [한국어] head 순회 커서 */

	lockdep_assert_held(&subsys->lock);	/* [한국어] 토폴로지 일관성 전제: subsys->lock */
	list_for_each_entry(h, &subsys->nsheads, entry)	/* [한국어] 리스트 토폴로지 조작 */
		if (h->disk)	/* [한국어] 아키텍처 가드 — 함수 헤드 문맥 참고 */
 ...
 */
void nvme_mpath_start_freeze(struct nvme_subsystem *subsys)	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
{
	struct nvme_ns_head *h;	/* [한국어] head 순회 커서 */
 ...
 */
void nvme_failover_req(struct request *req)	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
{
 ...
 */
void nvme_mpath_start_request(struct request *rq)	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
{
 ...
 */
void nvme_mpath_end_request(struct request *rq)	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
{
	struct nvme_ns *ns = rq->q->queuedata;	/* [한국어] 완료된 경로 ns */

	if (nvme_req(rq)->flags & NVME_MPATH_CNT_ACTIVE)	/* [한국어] 아키텍처 가드 — 함수 헤드 문맥 참고 */
 ...
 */
void nvme_kick_requeue_lists(struct nvme_ctrl *ctrl)	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
{
 ...
 */
bool nvme_mpath_clear_current_path(struct nvme_ns *ns)	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
{
	...
	if (!head)	/* [한국어] 아키텍처 가드 — 함수 헤드 문맥 참고 */
 ...
 */
void nvme_mpath_clear_ctrl_paths(struct nvme_ctrl *ctrl)	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
{
 ...
 */
void nvme_mpath_revalidate_paths(struct nvme_ns *ns)	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
{
	...
	list_for_each_entry_srcu(ns, &head->list, siblings,	/* [한국어] 리스트 토폴로지 조작 */
		...
		if (capacity != get_capacity(ns->disk))	/* [한국어] 아키텍처 가드 — 함수 헤드 문맥 참고 */
 ...
 */
static bool nvme_path_is_disabled(struct nvme_ns *ns)	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
{
	...
	if (state != NVME_CTRL_LIVE && state != NVME_CTRL_DELETING)	/* [한국어] 아키텍처 가드 — 함수 헤드 문맥 참고 */
 ...
 */
static struct nvme_ns *__nvme_find_path(struct nvme_ns_head *head, int node)	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
{
	...
	list_for_each_entry_srcu(ns, &head->list, siblings,	/* [한국어] 리스트 토폴로지 조작 */
		...
		if (nvme_path_is_disabled(ns))	/* [한국어] 아키텍처 가드 — 함수 헤드 문맥 참고 */
			continue;	/* [한국어] 비활성 경로 스킵 */

		if (ns->ctrl->numa_node != NUMA_NO_NODE &&	/* [한국어] 아키텍처 가드 — 함수 헤드 문맥 참고 */
	...
	if (!found)	/* [한국어] 아키텍처 가드 — 함수 헤드 문맥 참고 */
 ...
 */
static struct nvme_ns *nvme_next_ns(struct nvme_ns_head *head,	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
		struct nvme_ns *ns)	/* [한국어] 지역/멤버 상태 — 상위 함수·구조 아키텍처 참고 */
{
	ns = list_next_or_null_rcu(&head->list, &ns->siblings, struct nvme_ns,	/* [한국어] ns 상수 — 상위 enum 역할 참고 */
			siblings);	/* [한국어] RCU 리스트에서 다음 형제; 끝이면 NULL */
	if (ns)	/* [한국어] 아키텍처 가드 — 함수 헤드 문맥 참고 */
		return ns;	/* [한국어] 중간 노드 */
 ...
 */
static struct nvme_ns *nvme_round_robin_path(struct nvme_ns_head *head)	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
{
	struct nvme_ns *ns, *found = NULL;	/* [한국어] 순회 커서 / RR 후보 */
	int node = numa_node_id();	/* [한국어] 현재 제출 CPU 의 NUMA 노드 — 캐시 인덱스 */
	struct nvme_ns *old = srcu_dereference(head->current_path[node],	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
					       &head->srcu);	/* [한국어] 직전 선택 경로 (RR 시작점) */

	if (unlikely(!old))	/* [한국어] 아키텍처 가드 — 함수 헤드 문맥 참고 */
		return __nvme_find_path(head, node);	/* [한국어] 캐시 miss — 전체 탐색으로 시드 */

	if (list_is_singular(&head->list)) {	/* [한국어] 경로 1개면 RR 의미 없음 */
		if (nvme_path_is_disabled(old))	/* [한국어] 아키텍처 가드 — 함수 헤드 문맥 참고 */
	...
	for (ns = nvme_next_ns(head, old);	/* [한국어] old 다음부터 한 바퀴 (old 자신은 루프 조건으로 제외) */
		...
		if (ns->ana_state == NVME_ANA_NONOPTIMIZED)	/* [한국어] 아키텍처 가드 — 함수 헤드 문맥 참고 */
 ...
 */
static struct nvme_ns *nvme_queue_depth_path(struct nvme_ns_head *head)	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
{
	...
	list_for_each_entry_srcu(ns, &head->list, siblings,	/* [한국어] 리스트 토폴로지 조작 */
		...
		if (min_depth_opt == 0)	/* [한국어] 아키텍처 가드 — 함수 헤드 문맥 참고 */
 ...
 */
static struct nvme_ns *nvme_numa_path(struct nvme_ns_head *head)	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
{
	...
	if (unlikely(!ns))	/* [한국어] 아키텍처 가드 — 함수 헤드 문맥 참고 */
		return __nvme_find_path(head, node);	/* [한국어] 캐시 비어 있음 — 전체 탐색 */
	if (unlikely(!nvme_path_is_optimized(ns)))	/* [한국어] 아키텍처 가드 — 함수 헤드 문맥 참고 */
 ...
 */
inline struct nvme_ns *nvme_find_path(struct nvme_ns_head *head)	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
{
 ...
 */
static bool nvme_available_path(struct nvme_ns_head *head)	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
{
	struct nvme_ns *ns;	/* [한국어] 형제 경로 순회 */

	if (!test_bit(NVME_NSHEAD_DISK_LIVE, &head->flags))	/* [한국어] 상태 플래그 비트 */
 ...
 */
static void nvme_ns_head_submit_bio(struct bio *bio)	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
{
	...
	if (!bio)	/* [한국어] 아키텍처 가드 — 함수 헤드 문맥 참고 */
 ...
 */
static int nvme_ns_head_open(struct gendisk *disk, blk_mode_t mode)	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
{
 ...
 */
static void nvme_ns_head_release(struct gendisk *disk)	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
{
 ...
 */
static int nvme_ns_head_get_unique_id(struct gendisk *disk, u8 id[16],	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
 ...
 */
static int nvme_ns_head_report_zones(struct gendisk *disk, sector_t sector,	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
 ...
 */
static inline struct nvme_ns_head *cdev_to_ns_head(struct cdev *cdev)	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
{
 ...
 */
static int nvme_ns_head_chr_open(struct inode *inode, struct file *file)	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
{
	if (!nvme_tryget_ns_head(cdev_to_ns_head(inode->i_cdev)))	/* [한국어] 아키텍처 가드 — 함수 헤드 문맥 참고 */
 ...
 */
static int nvme_ns_head_chr_release(struct inode *inode, struct file *file)	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
{
 ...
 */
static int nvme_add_ns_head_cdev(struct nvme_ns_head *head)	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
{
	int ret;	/* [한국어] 등록 단계 에러 누적 */

	head->cdev_device.parent = &head->subsys->dev;	/* [한국어] sysfs 부모를 서브시스템 장치로 */
	ret = dev_set_name(&head->cdev_device, "ng%dn%d",	/* [한국어] ret 상수 — 상위 enum 역할 참고 */
			   head->subsys->instance, head->instance);	/* [한국어] generic NVMe char 노드 이름 */
	if (ret)	/* [한국어] 아키텍처 가드 — 함수 헤드 문맥 참고 */
		return ret;	/* [한국어] 이름 설정 실패 */
	ret = nvme_cdev_add(&head->cdev, &head->cdev_device,	/* [한국어] ret 상수 — 상위 enum 역할 참고 */
 ...
 * 대기와 재스캔이 겹치면 데드락.
 */
static void nvme_partition_scan_work(struct work_struct *work)	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
{
 ...
 */
static void nvme_requeue_work(struct work_struct *work)	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
{
	struct nvme_ns_head *head =	/* [한국어] 지역/멤버 상태 — 상위 함수·구조 아키텍처 참고 */
 ...
 */
static void nvme_remove_head(struct nvme_ns_head *head)	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
{
 ...
 */
static void nvme_remove_head_work(struct work_struct *work)	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
{
	struct nvme_ns_head *head = container_of(to_delayed_work(work),	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
	...
	if (remove)	/* [한국어] 아키텍처 가드 — 함수 헤드 문맥 참고 */
 ...
 */
int nvme_mpath_alloc_disk(struct nvme_ctrl *ctrl, struct nvme_ns_head *head)	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
{
	...
	if (!nvme_is_unique_nsid(ctrl, head))	/* [한국어] 아키텍처 가드 — 함수 헤드 문맥 참고 */
	...
	if (head->ids.csi == NVME_CSI_ZNS)	/* [한국어] 아키텍처 가드 — 함수 헤드 문맥 참고 */
	...
	if (IS_ERR(head->disk))	/* [한국어] 에러 포인터 규약 */
 ...
 */
static void nvme_mpath_set_live(struct nvme_ns *ns)	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
{
	struct nvme_ns_head *head = ns->head;	/* [한국어] 이 경로의 multipath head */
	int rc;	/* [한국어] device_add_disk 결과 */

	if (!head->disk)	/* [한국어] 아키텍처 가드 — 함수 헤드 문맥 참고 */
	...
	if (nvme_path_is_optimized(ns)) {	/* [한국어] 아키텍처 가드 — 함수 헤드 문맥 참고 */
		...
		srcu_idx = srcu_read_lock(&head->srcu);	/* [한국어] srcu_idx 상수 — 상위 enum 역할 참고 */
		for_each_online_node(node)	/* [한국어] 집합/비트 순회 */
 ...
 */
static int nvme_parse_ana_log(struct nvme_ctrl *ctrl, void *data,	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
		int (*cb)(struct nvme_ctrl *ctrl, struct nvme_ana_group_desc *,	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
			void *))	/* [한국어] 지역/멤버 상태 — 상위 함수·구조 아키텍처 참고 */
{
	...
	for (i = 0; i < le16_to_cpu(ctrl->ana_log_buf->ngrps); i++) {	/* [한국어] 헤더 ngrps 만큼 그룹 순회 */
		...
		if (WARN_ON_ONCE(offset > ctrl->ana_log_size - sizeof(*desc)))	/* [한국어] 불변식 위반 경고 */
		...
		if (WARN_ON_ONCE(desc->grpid == 0))	/* [한국어] 불변식 위반 경고 */
		...
		if (WARN_ON_ONCE(desc->state == 0))	/* [한국어] 불변식 위반 경고 */
			return -EINVAL;	/* [한국어] state 0 무효 */
		if (WARN_ON_ONCE(desc->state > NVME_ANA_CHANGE))	/* [한국어] 불변식 위반 경고 */
			return -EINVAL;	/* [한국어] 스펙 범위 밖 상태 */

		offset += sizeof(*desc);	/* [한국어] 고정 헤더 다음 가변 nsid 배열 위치 */
		if (WARN_ON_ONCE(offset > ctrl->ana_log_size - nsid_buf_size))	/* [한국어] 불변식 위반 경고 */
 ...
 */
static inline bool nvme_state_is_live(enum nvme_ana_state state)	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
{
 ...
 */
static void nvme_update_ns_ana_state(struct nvme_ana_group_desc *desc,	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
		struct nvme_ns *ns)	/* [한국어] 지역/멤버 상태 — 상위 함수·구조 아키텍처 참고 */
{
 ...
 */
static int nvme_update_ana_state(struct nvme_ctrl *ctrl,	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
		struct nvme_ana_group_desc *desc, void *data)	/* [한국어] 지역/멤버 상태 — 상위 함수·구조 아키텍처 참고 */
{
	...
	if (desc->state == NVME_ANA_CHANGE)	/* [한국어] 아키텍처 가드 — 함수 헤드 문맥 참고 */
		(*nr_change_groups)++;	/* [한국어] 전이 중 그룹 — ANATT 타이머 필요 카운트 */

	if (!nr_nsids)	/* [한국어] 아키텍처 가드 — 함수 헤드 문맥 참고 */
 ...
 */
static int nvme_read_ana_log(struct nvme_ctrl *ctrl)	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
{
	...
	if (nr_change_groups)	/* [한국어] 아키텍처 가드 — 함수 헤드 문맥 참고 */
 ...
 */
static void nvme_ana_work(struct work_struct *work)	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
{
 ...
 */
void nvme_mpath_update(struct nvme_ctrl *ctrl)	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
{
	u32 nr_change_groups = 0;	/* [한국어] parse 콜백이 채우는 CHANGE 카운트 (여기선 타이머 미사용) */

	if (!ctrl->ana_log_buf)	/* [한국어] 아키텍처 가드 — 함수 헤드 문맥 참고 */
 ...
 * 리셋으로 세션·로그를 재동기화한다.
 */
static void nvme_anatt_timeout(struct timer_list *t)	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
{
 ...
 */
void nvme_mpath_stop(struct nvme_ctrl *ctrl)	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
{
	if (!nvme_ctrl_use_ana(ctrl))	/* [한국어] 아키텍처 가드 — 함수 헤드 문맥 참고 */
 ...
 */
static ssize_t nvme_subsys_iopolicy_show(struct device *dev,	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
		struct device_attribute *attr, char *buf)	/* [한국어] 지역/멤버 상태 — 상위 함수·구조 아키텍처 참고 */
{
 ...
 */
static void nvme_subsys_iopolicy_update(struct nvme_subsystem *subsys,	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
		int iopolicy)	/* [한국어] 지역/멤버 상태 — 상위 함수·구조 아키텍처 참고 */
{
	struct nvme_ctrl *ctrl;	/* [한국어] 서브시스템 소속 컨트롤러 순회 */
	int old_iopolicy = READ_ONCE(subsys->iopolicy);	/* [한국어] 변경 전 정책 (로그용) */

	if (old_iopolicy == iopolicy)	/* [한국어] 아키텍처 가드 — 함수 헤드 문맥 참고 */
	...
	list_for_each_entry(ctrl, &subsys->ctrls, subsys_entry)	/* [한국어] 리스트 토폴로지 조작 */
 ...
 */
static ssize_t nvme_subsys_iopolicy_store(struct device *dev,	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
		struct device_attribute *attr, const char *buf, size_t count)	/* [한국어] 지역/멤버 상태 — 상위 함수·구조 아키텍처 참고 */
{
	struct nvme_subsystem *subsys =	/* [한국어] 지역/멤버 상태 — 상위 함수·구조 아키텍처 참고 */
 ...
 */
static ssize_t ana_grpid_show(struct device *dev, struct device_attribute *attr,	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
		char *buf)	/* [한국어] 지역/멤버 상태 — 상위 함수·구조 아키텍처 참고 */
{
 ...
 */
static ssize_t ana_state_show(struct device *dev, struct device_attribute *attr,	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
		char *buf)	/* [한국어] 지역/멤버 상태 — 상위 함수·구조 아키텍처 참고 */
{
 ...
 */
static ssize_t queue_depth_show(struct device *dev,	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
		struct device_attribute *attr, char *buf)	/* [한국어] 지역/멤버 상태 — 상위 함수·구조 아키텍처 참고 */
{
	struct nvme_ns *ns = nvme_get_ns_from_dev(dev);	/* [한국어] 블록 device↔디스크 역참조 */

	if (ns->head->subsys->iopolicy != NVME_IOPOLICY_QD)	/* [한국어] 아키텍처 가드 — 함수 헤드 문맥 참고 */
 ...
 */
static ssize_t numa_nodes_show(struct device *dev, struct device_attribute *attr,	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
		char *buf)	/* [한국어] 지역/멤버 상태 — 상위 함수·구조 아키텍처 참고 */
{
	int node, srcu_idx;	/* [한국어] srcu_idx — 함수/구조 문맥의 상태 */
	...
	if (head->subsys->iopolicy != NVME_IOPOLICY_NUMA)	/* [한국어] 아키텍처 가드 — 함수 헤드 문맥 참고 */
	...
	for_each_node(node) {	/* [한국어] 집합/비트 순회 */
		...
		if (ns == current_ns)	/* [한국어] 아키텍처 가드 — 함수 헤드 문맥 참고 */
 ...
 */
static ssize_t delayed_removal_secs_show(struct device *dev,	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
		struct device_attribute *attr, char *buf)	/* [한국어] 지역/멤버 상태 — 상위 함수·구조 아키텍처 참고 */
{
 ...
 */
static ssize_t delayed_removal_secs_store(struct device *dev,	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
		struct device_attribute *attr, const char *buf, size_t count)	/* [한국어] 지역/멤버 상태 — 상위 함수·구조 아키텍처 참고 */
{
	...
	if (sec)	/* [한국어] 아키텍처 가드 — 함수 헤드 문맥 참고 */
 ...
 */
static int nvme_lookup_ana_group_desc(struct nvme_ctrl *ctrl,	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
		struct nvme_ana_group_desc *desc, void *data)	/* [한국어] 지역/멤버 상태 — 상위 함수·구조 아키텍처 참고 */
{
	struct nvme_ana_group_desc *dst = data;	/* [한국어] 찾을 grpid 가 채워진 출력 버퍼 */

	if (desc->grpid != dst->grpid)	/* [한국어] 아키텍처 가드 — 함수 헤드 문맥 참고 */
 ...
 */
void nvme_mpath_add_sysfs_link(struct nvme_ns_head *head)	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
{
	 ...
	 */
	if (!test_bit(GD_ADDED, &head->disk->state))	/* [한국어] 상태 플래그 비트 */
	...
	list_for_each_entry_srcu(ns, &head->list, siblings,	/* [한국어] 리스트 토폴로지 조작 */
		 ...
		 */
		if (!test_bit(GD_ADDED, &ns->disk->state))	/* [한국어] 상태 플래그 비트 */
		...
		if (test_and_set_bit(NVME_NS_SYSFS_ATTR_LINK, &ns->flags))	/* [한국어] 상태 플래그 비트 */
 ...
 */
void nvme_mpath_remove_sysfs_link(struct nvme_ns *ns)	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
{
	struct device *target;	/* [한국어] target — 함수/구조 문맥의 상태 */
	struct kobject *kobj;	/* [한국어] kobj — 함수/구조 문맥의 상태 */

	if (!test_bit(NVME_NS_SYSFS_ATTR_LINK, &ns->flags))	/* [한국어] 상태 플래그 비트 */
 ...
 */
void nvme_mpath_add_disk(struct nvme_ns *ns, __le32 anagrpid)	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
{
...
#ifdef CONFIG_BLK_DEV_ZONED
	if (blk_queue_is_zoned(ns->queue) && ns->head->disk)	/* [한국어] blk-mq/큐 계층 API */
 ...
 */
void nvme_mpath_remove_disk(struct nvme_ns_head *head)	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
{
	...
	if (!list_empty(&head->list))	/* [한국어] 리스트 토폴로지 조작 */
 ...
 */
void nvme_mpath_put_disk(struct nvme_ns_head *head)	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
{
	if (!head->disk)	/* [한국어] 아키텍처 가드 — 함수 헤드 문맥 참고 */
		return;	/* [한국어] disk 없음 */
 ...
 */
void nvme_mpath_init_ctrl(struct nvme_ctrl *ctrl)	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
{
 ...
 */
int nvme_mpath_init_identify(struct nvme_ctrl *ctrl, struct nvme_id_ctrl *id)	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
{
	...
	if (!multipath || !ctrl->subsys ||	/* [한국어] 아키텍처 가드 — 함수 헤드 문맥 참고 */
	...
	if (!ctrl->max_namespaces ||	/* [한국어] 아키텍처 가드 — 함수 헤드 문맥 참고 */
 ...
 */
void nvme_mpath_uninit(struct nvme_ctrl *ctrl)	/* [한국어] 함수 시그니처 — 직전 한국어 함수 블록 계약 */
{
